...
class ConversationController extends Controller
{
    public function messages_store(Request $request)
    {
        ...
        if($request->conversation_id){
            ...
            if($request->receiver_type == 'admin'){
                ...
                $receiver_id = $receiver->id;
                $fcm_token=$delivery_man->fcm_token;
            }else if($request->receiver_type == 'provider'){
        ...
        try {
            ...
            $conversation->save();
            {
                ...
                }else if($request->receiver_type == 'vendor' || $request->receiver_type == 'delivery_man' || $request->receiver_type == 'provider' || $request->receiver_type == 'serviceman'){
                    $data = [
                        'title' =>translate('messages.message'),
                        'description' =>translate('messages.message_description'),
                        'order_id' => '',
        ...
        }else if($conv->sender_type == 'provider' && $conversation->sender){
            $provider = Provider::find($conv->sender->provider_id);
            $order = Booking::where('customer_id',$request->user()->id)->where('provider_id', $provider->id)->whereIn('booking_status', ['pending','accepted','ongoing'])->count();

        }else if($conv->receiver_type == 'provider' && $conversation->receiver){
            $provider = Provider::find($conv->receiver->provider_id);
            $order = Booking::where('customer_id',$request->user()->id)->where('provider_id', $provider->id)->whereIn('booking_status', ['pending','accepted','ongoing'])->count();

        }else if($conv->sender_type == 'serviceman' && $conversation->sender){
            $serviceman = Serviceman::find($conv->sender->serviceman_id);
            $order = Booking::where('customer_id',$request->user()->id)->where('serviceman_id', $serviceman->id)->whereIn('booking_status', ['pending','accepted','ongoing'])->count();
        }else if($conv->receiver_type == 'serviceman' && $conversation->receiver){
            $serviceman = Serviceman::find($conv->receiver->serviceman_id);
            $order = Booking::where('customer_id',$request->user()->id)->where('serviceman_id', $serviceman->id)->whereIn('booking_status', ['pending','accepted','ongoing'])->count();
        }
        else{
            $order=1;
    ...
    public function messages(Request $request)
    {
        ...
        if($request->conversation_id){
            ...
            $vd = Provider::find($request->provider_id);
            if(!$provider){
                $provider = new UserInfo();
                $provider->provider_id = $vd->id;
                $provider->f_name = $vd->company_name;
                $provider->l_name = '';
                $provider->phone = $vd->phone;
                $provider->email = $vd->email;
                $provider->image = $vd->logo;
                $provider->save();
            }else{
                $provider->f_name = $vd->company_name;
                $provider->l_name = '';
            ...
            $conversation = Conversation::with(['sender','receiver','last_message'])->WhereConversation($user->id,$provider->id)->first();
        }else if($request->serviceman_id){
            $dm = UserInfo::where('serviceman_id', $request->serviceman_id)->first();
            if(!$dm){
                $user2 = Serviceman::find($request->serviceman_id);
                $dm = new UserInfo();
                $dm->serviceman_id = $user2->id;
                $dm->f_name = $user2->first_name;
                $dm->l_name = $user2->last_name;
                $dm->phone = $user2->phone;
                $dm->email = $user2->email;
                $dm->image = $user2->image;
                $dm->save();
        ...
        if(isset($conversation)){
            if($conversation->sender_type == 'vendor' && $conversation->sender){
            ...
            }else if($conversation->sender_type == 'provider' && $conversation->sender){
                $vd = Provider::find($conversation->sender->provider_id);
                $order = Booking::where('customer_id',$request->user()->id)->where('provider_id', $vd->id)->whereIn('booking_status', ['pending','accepted','ongoing'])->count();
                ...
                $order = Booking::where('customer_id',$user->user_id)->where('serviceman_id', $user2->id)->whereIn('booking_status', ['pending','accepted','confirmed'])->count();
    ...
    public function dm_auto_messages_store(Request $request)
    {
        ...
        $automated_message = QuestionAnswer::where('question_answer_for', DRIVER)->where('id', $request->question_id)->first();
        ...
        try {
            ...
            $message->message = $automated_message->answer;
