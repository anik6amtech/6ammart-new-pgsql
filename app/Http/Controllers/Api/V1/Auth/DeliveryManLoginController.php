...
class DeliveryManLoginController extends Controller
{
    ...
    public function __construct(
        ...
        protected ReferralDriverServiceInterface   $referralDriverService,
    )
    {
    }
    ...
    public function login(Request $request)
    {
        ...
        if (auth('delivery_men')->attempt($data)) {
            ...
            $delivery_man->save();
            $this->refer_code_check($delivery_man);
            $riderDetail = RiderDetail::where('user_id', $delivery_man->id)->first();
            if(empty($riderDetail)){
                $riderDetail = new RiderDetail();
                $riderDetail->user_id = $delivery_man->id;
                $riderDetail->save();
            }
    ...
    public function store(Request $request)
    {
        ...
        if (array_key_exists('referral_code', $request->all()) && $request->referral_code) {
            $referralUser = DeliveryMan::where('ref_code', $request->referral_code)->first();
            if (!$referralUser) {
        ...
        if (!$firstLevel) {
            return response()->json(['errors' => [LEVEL_403['message']]],403);
        }
        ...
        $serve_for = json_decode($request->serve_for,true);
        ...
        $dm->is_delivery = in_array('delivery', $serve_for) ? 1 : 0;
        $dm->is_ride = in_array('ride', $serve_for) ? 1 : 0;
        $dm->user_level_id = $firstLevel->id;

        $dm->save();

        /* $dm->levelHistory()->create([
            'user_level_id' => $dm['user_level_id'],
            'user_type' => DRIVER
        ]); */

        $dm->riderDetails()->create([
            'is_online' => false,
            'availabilityStatus' => 'unavailable'
        ]);

        $dm->ref_code = Helpers::generate_dm_referer_code($dm);
        $dm->save();

        if (array_key_exists('referral_code', $request->all()) && $request->referral_code && $referralUser && $dm) {
            if (get_cache('driver_referral_earning_status')) {
                $referralDriverData = [
                    'driver_id' => $dm->id,
                    'ref_by' => $referralUser->id,
                    'ref_by_earning_amount' => (double)get_cache('driver_share_code_earning'),
                    'driver_earning_amount' => (double)get_cache('driver_use_code_earning'),
                    'is_used' => true
                ];
                ...
                if ($referralDriver?->driver_earning_amount > 0) {
                    $this->driverReferralEarningTransaction($dm, $referralDriver?->driver_earning_amount);
    ...
    private function refer_code_check($user){
        if($user->ref_code == null && isset($user->id)){
            $ref_code = Helpers::generate_dm_referer_code($user);
            DB::table('delivery_men')->where('id', $user->id)->update(['ref_code' => $ref_code]);
        }
        return true;
    }
}
