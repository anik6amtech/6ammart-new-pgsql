...
use Modules\Service\Entities\CartModule\Cart as CartModuleCart;
    ...
    private function check_guest_cart($user, $guest_id){
        if($guest_id && isset($user->id)){
            ...
            if(addon_published_status('Service')){
                CartModuleCart::where(['customer_id' => $user->id, 'is_guest' => 0])
                    ->when(CartModuleCart::where(['customer_id' => $guest_id, 'is_guest' => 1])->exists(),function ($query) {
                        $query->delete();
                    }); 

                CartModuleCart::where('customer_id', $guest_id)->where('is_guest', 1)->update(['customer_id' => $user->id, 'is_guest' => 0]);
                AddedToCart::where('user_id', $guest_id)->where('is_guest', 1)->update(['user_id' => $user->id, 'is_guest' => 0]);
            }
