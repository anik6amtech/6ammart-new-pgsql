...
class DeliverymanController extends Controller
{
    ...
    public function __construct(UserLastLocationServiceInterface $userLastLocationService, TimeTrackServiceInterface $timeTrackService)
    {
    ...
    public function get_profile(Request $request)
    {
        ...
        if(addon_published_status('RideShare')){
            ...
            $paidAmount = $dm->transactions?->where('attribute', 'admin_cash_collect')
                ->where('account' , 'payable_balance')->sum('debit') ?? 0;
            $levelUpRewardAmount = $dm->transactions?->where('attribute', 'level_reward')->sum('credit') ?? 0;
            ...
            $dm['paid_amount'] = $paidAmount;
            $dm['level_up_reward_amount'] = $levelUpRewardAmount;
            ...
            $dm['total_referral_earning'] = $this->getUserAccount($dm->id, DRIVER)->referral_earn ?? 0;
            $dm['time_track'] = TimeTrackResource::make($dm->latestTrack);
    ...
    public function get_order(Request $request)
    {
        ...
        ->where(['id' => $request['order_id']])->Notpos()->first();
        if(!$order)
        {
            return response()->json([
                'errors' => [
    ...
    public function request_withdraw(Request $request)
    {
        ...
        $method = DisbursementWithdrawalMethod::find($request['method_id']);
        $main_method = $method->withdrawalMethod;
        $fields = array_column($main_method->method_fields, 'input_name');
        $values = json_decode($method->method_fields, true);
        ...
        if ($w?->total_earning >= $request['amount']) {
            ...
            try
            {
                $attribute = WithdrawRequest::create($data);
                $w?->increment('pending_withdraw', $request['amount']);
                $this->withdrawRequestWithoutAdjustTransaction($dm, $request->amount, $attribute);
    ...
    public function withdraw_list(Request $request)
    {
        ...
        $status_reverse_map = array_flip($status_map);

        $dm = DeliveryMan::where('auth_token', $request['token'])->first();

        $status_values = null;

        if (isset($request['status']) && is_array($request['status']) && !in_array('All', $request['status'])) {
            $status_values = [];
            foreach ($request['status'] as $status_label) {
                if (isset($status_map[$status_label])) {
                    $status_values[] = $status_map[$status_label];
                }
            }
        }

        $withdraw_requests = WithdrawRequest::where('delivery_man_id', $dm->id)
        ->when(!empty($status_values), function ($query) use ($status_values) {
            return $query->whereIn('approved', $status_values);
        })
        ->latest()
        ->paginate($request['limit'], ['*'], 'page', $request['offset']);

        $data = [
            'total_size' => $withdraw_requests->total(),
            'limit' => $request['limit'],
            'offset' => $request['offset'],
            'data' => WithdrawRequestResource::collection($withdraw_requests),
        ];
    ...
    {
        $reviews = DMReview::with(['customer'])->where(['delivery_man_id' => auth('delivery_men')->id()])->active()->get();
