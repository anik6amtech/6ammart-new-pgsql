...
class SubscriptionController extends Controller
{
    public function package_view(Request $request)
    {
        ...
        $packages = SubscriptionPackage::where('status', 1)
            ...
            // ->where('module_type', $module?->module_type == 'service' && addon_published_status('Service') ? 'service' : 'all')
            ->latest()
            ->get();
    ...
    public function cancelSubscription(Request $request)
    {
        ...
        StoreSubscription::where(['id' => $request->subscription_id, 'store_id' => $request->store_id])->where('store_type', 'store')->update([
            'is_canceled' => 1,
            'canceled_by' => 'store',
        ]);
