...
class LoginController extends Controller
{
    ...
    public function login($login_url)
    {
        ...
        $loginTypes = [
            ...
            'provider' => 'provider_login_url',
            'provider_employee' => 'provider_employee_login_url'
        ];
        $siteDirections = [
            ...
            'provider' => session()?->get('provider_site_direction') ?? $direction ?? 'ltr',
            'provider_employee' => session()?->get('provider_employee_site_direction') ?? $direction ?? 'ltr',
        ];
        $locals = [
            ...
            'provider' => session()?->get('provider_local') ?? $lang ?? 'en',
            'provider_employee' => session()?->get('provider_local') ?? $lang ?? 'en'
        ];
        $role = null;
    ...
    public function submit(Request $request)
    {
        ...
        elseif ($request->role == 'vendor') {
            ...
            if ($provider) {
                ...
                if($provider->is_active == 0) {
                    return redirect()->back()->withInput($request->only('email', 'remember'))
                        ->withErrors([translate('messages.provider_is_inactive')]);
                }

                if($provider->is_approved == 0) {
                    return redirect()->back()->withInput($request->only('email', 'remember'))
                        ->withErrors([translate('messages.provider_is_not_approved')]);
                }

                /* if($provider->is_suspended == 1) {
                    return redirect()->back()->withInput($request->only('email', 'remember'))
                        ->withErrors([translate('messages.provider_is_suspended')]);
                } */
            }
        } elseif ($request->role == 'provider_employee') {
            $employee = ProviderEmployee::where('email', $request->email)->first();
            if ($employee) {
                if ( $employee?->provider?->is_active == 0 && $employee?->provider?->is_approved == 0 && $employee?->provider?->is_suspended == 1) {
                    return redirect()->back()->withInput($request->only('email', 'remember'))
                        ->withErrors([translate('messages.Provider_is_inactive')]);
                }
            }
        }
        // dump($request->role, $request->email, $request->password, $request->remember);
        ...
        if ($data == 'provider') {
            ...
            if ($request->role === 'provider_employee') {
                $employee = ProviderEmployee::where('email', $request->email)->first();
                $employee->is_logged_in = 1;
                $employee->save();
            }
...
//        dd(auth());
        if (auth('vendor')?->check()) {
            ...
            $user_link = Helpers::get_login_url('provider_login_url');
            auth()->guard('provider')->logout();
        } elseif (auth('provider_employee')->check()) {
            $user_link = Helpers::get_login_url('provider_employee_login_url');
            auth()->guard('provider_employee')->logout();
        } else {
