...
class SubscriptionController extends Controller
{
    ...
    StoreSubscription::where(['store_id' => Helpers::get_store_id(), 'id'=>$request->subscription_id])->where('store_type', 'store')->update([
            'is_canceled' => 1,
            'canceled_by' => 'store',
        ]);
    ...
    public function switchToCommission($id){
        ...
        StoreSubscription::where(['store_id' => Helpers::get_store_id()])->where('store_type', 'store')->update([
            'status' => 0,
        ]);
        return response()->json(200);
