...
//            'tax' => 'required',
        ...
        $validator->sometimes(
            ...
            'required',
            function () use ($module) {
                return $module && $module->module_type !== 'service';
            }
        );
        ...
        if (!$module->module_type == 'service'){
            ...
            $provider->module_id = $request->module_id;
            ...
            $provider->service_time_type = $request->delivery_time_type;
            $provider->business_model = ($request->business_plan == "subscription-base") ? 'subscription' : 'commission';
            $provider->save();

            Helpers::add_or_update_translations(request: $request, key_data: 'name', name_field: 'name', model_name: 'Provider', data_id: $provider->id, data_value: $provider->name);
            Helpers::add_or_update_translations(request: $request, key_data: 'address', name_field: 'address', model_name: 'Provider', data_id: $provider->id, data_value: $provider->address);
...
//                    Mail::to($admin['email'])->send(new StoreRegistration('pending', $provider->company_name));
//                }
//
//            }catch(\Exception $ex){
//                info($ex->getMessage());
//            }
            ...
            if (Helpers::subscription_check()) {
                if ($request->business_plan == 'subscription-base' && $request->package_id != null ) {
                    ...
                    return view('service::provider.register-subscription-payment',[
                        'package_id'=> $request->package_id,
                        'provider_id' => $provider->id,
                        'free_trial_settings'=>$free_trial_settings,
                        'payment_methods' => Helpers::getActivePaymentGateways(),
                ...
                elseif($request->business_plan == 'commission-base' ){
                    ...
                    $provider->save();
                    return view('service::provider.register-complete',[
                        'type'=>'commission'
                    ]);
                }
                else{
    ...
    public function get_all_modules(Request $request){
        /* $dataSetting = (int) DataSetting::where('key', 'provider_self_registration')
        ...
        $module_data = Module::Active()->whereHas('zones', function($query)use ($request){
            ...
            /* ->when(!$dataSetting, function($query){
                $query->where('module_type', '!=', 'service');
            }) */
            ->where('modules.module_type', '!=', 'ride-share')
