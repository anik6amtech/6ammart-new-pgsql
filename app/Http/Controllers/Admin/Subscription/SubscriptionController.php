...
class SubscriptionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $packages=  SubscriptionPackage::withcount('currentSubscribers')
            ...
            ->when($request?->module == 2, function($query){
                $query->where('module_type', 'service');
            })
            ->when(!in_array($request?->module, [1, 2]), function($query){
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'scheduled_service' => 'nullable|boolean',
            'service_request' => 'nullable|boolean',
            'advertisement' => 'nullable|boolean',
            'reports_and_analytics' => 'nullable|boolean',
            'bidding' => 'nullable|boolean',
        ...
        $package->scheduled_service = $request->scheduled_service ?? 0;
        $package->service_request = $request->service_request ?? 0;
        $package->advertisement = $request->advertisement ?? 0;
        $package->reports_and_analytics = $request->reports_and_analytics ?? 0;
        $package->bidding = $request->bidding ?? 0;
        ...
        if($package->module_type == 'rental') {
            $mType = 1;
        } else if($package->module_type == 'service') {
            $mType = 2;
        } else {
            $mType = 'all';
        }
        return redirect()->route('admin.business-settings.subscriptionackage.index',[ 'module' => $mType ]);
    }
    ...
    public function update(SubscriptionPackage $subscriptionackage, Request $request)
    {
        ...
        $subscriptionackage->scheduled_service = $request->scheduled_service ?? 0;
        $subscriptionackage->service_request = $request->service_request ?? 0;
        $subscriptionackage->advertisement = $request->advertisement ?? 0;
        $subscriptionackage->reports_and_analytics = $request->reports_and_analytics ?? 0;
        $subscriptionackage->bidding = $request->bidding ?? 0;
    ...
    public function subscriberList(Request $request){
        ...
        if ($request->type == 'store'){
            ...
            $subscribers= Provider::OfStatus(1)->has('store_sub_update_application')
                ->whereIn('business_model' ,['subscription','unsubscribed'])->with([
                    'store_sub_update_application.package'
                ])->withCount('store_all_sub_trans')
        ...
        $data['expired_subscription']= $total_inactive_subscription;
            ...
            if ($request->type == 'provider'){
                return view('admin-views.subscription.subscriber.provider_list', compact('subscribers', 'data'));
            }
    ...
    public function switchToCommission($id){
        ...
        StoreSubscription::where(['store_id' => $id])->where('store_type', 'store')->update([
            'status' => 0,
        ]);
        return response()->json(200);
    ...
    public function packageView($id,$store_id){
        $store_subscription= StoreSubscription::where('store_id', $store_id)->where('store_type', 'store')->with(['package'])->latest()->first();
