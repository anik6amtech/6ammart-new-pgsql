    ...
    public function index(Request $request)
    {
        ...
        if (is_numeric($searchKeyword) && $searchKeyword > 0) {
            ...
            if ($currentModuleType == 'ride-share') {
                $ride = RideRequest::find($searchKeyword);
                if ($ride) {
                    $rideRoutes = $adminRoutes->filter(function ($route) {
                        return (str_contains($route->uri(), 'ride/details')) && !str_contains($route->uri(), 'transactions/ride/details/')  && !str_contains($route->uri(), 'expense-report') && !str_contains($route->uri(), 'export');
                    });

                    if (isset($rideRoutes)) {
                        foreach ($rideRoutes as $route) {
                            $validRoutes[] = $this->filterRoute(model: $ride, route: $route, type: 'ride', prefix: 'Ride', name: $ride->id, searchKeyword: $ride->id);
                        }
                    }
                }
                ...
                if ($coupon) {
                    ...
                    $couponRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/ride-share/promotion/coupon-setup/edit');
                    });
                ...
                //discount
                $discount = DiscountSetup::when(is_numeric($currentModuleId), function ($query) use ($currentModuleId) {
                    return $query->where('module_id', $currentModuleId);
                })->find($searchKeyword);
                if ($discount) {

                    $discountRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/ride-share/promotion/discount-setup') && !str_contains($route->uri(), 'status');
                    });

                    if (isset($discountRoutes)) {
                        foreach ($discountRoutes as $route) {
                            $validRoutes[] = $this->filterRoute(model: $discount, route: $route, prefix: 'Discount');
                        }
                    }
                }
            ...
            if ($currentModuleType == 'service') {
                $provider = Provider::find($searchKeyword);
                if ($provider) {
                    $providerRoutes = $adminRoutes->filter(function ($route) {
                        return (str_contains($route->uri(), 'service/provider/details')) && !str_contains($route->uri(), 'transactions/service/provider/details/') && !str_contains($route->uri(), 'expense-report') && !str_contains($route->uri(), 'export');
                    });

                    if (isset($providerRoutes)) {
                        foreach ($providerRoutes as $route) {
                            $validRoutes[] = $this->filterRoute(model: $provider, route: $route, type: 'provider', prefix: 'Provider', name: $provider->id, searchKeyword: $provider->id);
                        }
                    }
                }

                //post & booking
                $post = Post::when(is_numeric($currentModuleId), function ($query) use ($currentModuleId) {
                    return $query->where('module_id', $currentModuleId);
                })->find($searchKeyword);
                if ($post) {

                    $postRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/service/booking/post/details');
                    });
                ...
                if ($posts) {
                    $postsRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/service/booking/post/details');
                    });
                ...
                $booking = Booking::when(is_numeric($currentModuleId), function ($query) use ($currentModuleId) {
                    return $query->where('module_id', $currentModuleId);
                })->find($searchKeyword);
                if ($booking) {
                    ...
                    if ($booking->is_repeated == 0){
                        $bookingRoutes = $adminRoutes->filter(function ($route) {
                            return str_contains($route->uri(), 'admin/service/booking/details')  && !str_contains($route->uri(), 'post') && !str_contains($route->uri(), 'rebooking') && !str_contains($route->uri(), 'repeat-single-details');
                        });
                    }else{
                        $bookingRoutes = $adminRoutes->filter(function ($route) {
                            return str_contains($route->uri(), 'admin/service/booking/repeat-details')  && !str_contains($route->uri(), 'post') && !str_contains($route->uri(), 'rebooking') && !str_contains($route->uri(), 'repeat-single-details');
                        });
                    }
                ...
                //coupon
                $coupon = \Modules\Service\Entities\PromotionManagement\Coupon::
                    when(is_numeric($currentModuleId), function ($query) use ($currentModuleId) {
                        return $query->where('module_id', $currentModuleId);
                    })
                    ->find($searchKeyword);
                ...
                if ($coupon) {
                    ...
                    $couponRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/service/coupon/edit');
                    });
                ...
                if ($banner) {
                    ...
                    $bannerRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/service/banner') && !str_contains($route->uri(), 'status');
                    });
                ...
                //discount
                $discount = Discount::when(is_numeric($currentModuleId), function ($query) use ($currentModuleId) {
                    return $query->where('module_id', $currentModuleId);
                })->find($searchKeyword);
                if ($discount) {

                    $discountRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/service/discount') && !str_contains($route->uri(), 'status');
                    });
                ...
                //campaign
                $campaign = \Modules\Service\Entities\PromotionManagement\Campaign::
//                when(is_numeric($currentModuleId), function ($query) use ($currentModuleId) {
//                    return $query->where('module_id', $currentModuleId);
//                })->
                find($searchKeyword);
                if ($campaign) {
                    ...
                    $campaignRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/service/campaign') && !str_contains($route->uri(), 'status');
                    });

                    if (isset($campaignRoutes)) {
                        foreach ($campaignRoutes as $route) {
                            $validRoutes[] = $this->filterRoute(model: $campaign, route: $route, prefix: 'Campaign');
                        }
                    }
                }
                ...
                //category
                $category = \Modules\Service\Entities\CategoryManagement\Category::ofType('main')->when(is_numeric($currentModuleId), function ($query) use ($currentModuleId) {
                    return $query->where('module_id', $currentModuleId);
                })->find($searchKeyword);
                if ($category) {

                    $categoryRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/service/category') && !str_contains($route->uri(), 'status');
                    });

                    if (isset($categoryRoutes)) {
                        foreach ($categoryRoutes as $route) {
                            $validRoutes[] = $this->filterRoute(model: $category, route: $route, prefix: 'Category');
                        }
                    }
                }

                //sub-category
                $category = \Modules\Service\Entities\CategoryManagement\Category::ofType('sub')->when(is_numeric($currentModuleId), function ($query) use ($currentModuleId) {
                    return $query->where('module_id', $currentModuleId);
                })->find($searchKeyword);
                if ($category) {

                    $categoryRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/service/sub-category') && !str_contains($route->uri(), 'status');
                    });

                    if (isset($categoryRoutes)) {
                        foreach ($categoryRoutes as $route) {
                            $validRoutes[] = $this->filterRoute(model: $category, route: $route, prefix: 'Sub Category');
                        }
                    }
                }

                //service
                $service = Service::when(is_numeric($currentModuleId), function ($query) use ($currentModuleId) {
                    return $query->where('module_id', $currentModuleId);
                })->find($searchKeyword);
                if ($service) {

                    $serviceRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/service/service') && !str_contains($route->uri(), 'status');
                    });
            ...
            if ($currentModuleType == 'ride-share') {
                $rides = RideRequest::where(function ($query) use ($searchKeyword) {
                    $query->where('payment_method', 'LIKE', '%' . $searchKeyword . '%')
                        ->orWhere('payment_status', 'LIKE', '%' . $searchKeyword . '%')
                        ...
                        ->orWhereHas('driver', function ($query) use ($searchKeyword) {
                            return $query->where('f_name', 'LIKE', '%' . $searchKeyword . '%')
                                ->orWhere('l_name', 'LIKE', '%' . $searchKeyword . '%')
                                ->orWhere('email', 'LIKE', '%' . $searchKeyword . '%')
                                ->orWhere('phone', 'LIKE', '%' . $searchKeyword . '%')
                        ...
                        ->orWhereHas('vehicle', function ($query) use ($searchKeyword) {
                            return $query->where('name', 'LIKE', '%' . $searchKeyword . '%')
                                ->orWhere('vin_number', 'LIKE', '%' . $searchKeyword . '%');
                        });
                })->get();
                if ($rides) {
                    $rideRoutes = $adminRoutes->filter(function ($route) {
                    ...
                    if (isset($rideRoutes)) {
                        foreach ($rides as $ride) {
                            foreach ($rideRoutes as $route) {
                                $validRoutes[] = $this->filterRoute(model: $ride, route: $route, prefix: 'Ride', searchKeyword: $ride->id);
                            }
                        }
                    }
                }
                ...
                where(function ($query) use ($searchKeyword) {
                    $query->where('name', 'LIKE', '%' . $searchKeyword . '%')
                        ->orWhere('coupon_code', 'LIKE', '%' . $searchKeyword . '%');
                })->get();
                ...
                if ($coupons) {
                    ...
                    if (isset($couponRoutes)) {
                        foreach ($coupons as $coupon) {
                            foreach ($couponRoutes as $route) {
                                $validRoutes[] = $this->filterRoute(model: $coupon, route: $route, prefix: 'Coupon', searchKeyword: $coupon->coupon_code);
                            }
                        }
                    }
                }
                ...
                if ($discounts) {
                    ...
                    $discountRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/ride-share/promotion/discount-setup') && !str_contains($route->uri(), 'status')
                            && !str_contains($route->uri(), 'restore') && !str_contains($route->uri(), 'export') && !str_contains($route->uri(), 'log')
                            && !str_contains($route->uri(), 'trashed') && !str_contains($route->uri(), 'test');
                    });

                    if (isset($discountRoutes)) {
                        foreach ($discounts as $discount) {
                            foreach ($discountRoutes as $route) {
                                $validRoutes[] = $this->filterRoute(model: $discount, route: $route, prefix: 'Discount', searchKeyword: $discount->id);
                            }
                        }
                    }
                }
            ...
            if (in_array($currentModuleType, ['service'])) {
                //provider
                $providers = Provider::when(is_numeric($currentModuleId), function ($query) use ($currentModuleId) {
                    return $query->where(function ($query) use ($currentModuleId) {
                        $query->where('module_id', $currentModuleId);
                    });
                })
                    ->where(function ($query) use ($searchKeyword) {
                        $query->where('first_name', 'LIKE', '%' . $searchKeyword . '%')
                            ...
                            ->orWhere('company_address', 'LIKE', '%' . $searchKeyword . '%')
                ...
                if ($providers) {
                    $providerRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'service/provider')  && !str_contains($route->uri(), 'service/provider/export') && !str_contains($route->uri(), 'service/provider/status');
                    });

                    if (isset($providerRoutes)) {
                        foreach ($providers as $Brand) {
                            foreach ($providerRoutes as $route) {
                                $validRoutes[] = $this->filterRoute(model: $Brand, route: $route, prefix: 'Provider');
                            }
                        }
                    }
                }

                $bookings = Booking::with('customer')->when(is_numeric($currentModuleId), function ($query) use ($currentModuleId) {
                    return $query->where('module_id', $currentModuleId);
                })->where(function ($query) use ($searchKeyword) {
                ...
                if ($services){
                    $serviceRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/service/service') && !str_contains($route->uri(), 'faq') && !str_contains($route->uri(), 'status')
                            && !str_contains($route->uri(), 'tag') && !str_contains($route->uri(), 'export') && !str_contains($route->uri(), 'update')
                            && !str_contains($route->uri(), 'list') && !str_contains($route->uri(), 'download') && !str_contains($route->uri(), 'create');
                    });
                    if (isset($serviceRoutes)) {
                ...
                //coupon
                $coupons = \Modules\Service\Entities\PromotionManagement\Coupon::with('discount')
                    ->where('coupon_code', 'LIKE', '%' . $searchKeyword . '%')
                    ->get();

                if ($coupons){
                    $couponRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/service/coupon') && (str_contains($route->uri(), 'edit'));
                    });
                    if (isset($couponRoutes)) {
                        foreach ($coupons as $coupon){
                            foreach ($couponRoutes as $route) {
                ...
                if ($discounts){
                    $discountRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/service/discount') && (str_contains($route->uri(), 'edit'));
                    });
                    if (isset($discountRoutes)) {
                        foreach ($discounts as $discount){
                            foreach ($discountRoutes as $route) {
                ...
                //campaign
                $campaigns = \Modules\Service\Entities\PromotionManagement\Campaign::where('campaign_name', 'LIKE', '%' . $searchKeyword . '%')->get();
                if ($campaigns){
                    $campaignRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/service/campaign') && (str_contains($route->uri(), 'edit'));
                    });
                    if (isset($campaignRoutes)) {
                ...
                //category
                $categories = \Modules\Service\Entities\CategoryManagement\Category::ofType('main')->where('name', 'LIKE', '%' . $searchKeyword . '%')->get();
                if ($categories){
                    $categoryRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/service/category') && (str_contains($route->uri(), 'edit'));
                    });
                    if (isset($categoryRoutes)) {
                ...
                //sub category
                $subCategories = \Modules\Service\Entities\CategoryManagement\Category::ofType('sub')->where('name', 'LIKE', '%' . $searchKeyword . '%')->get();
                if ($subCategories){
                    $subCategoryRoutes = $adminRoutes->filter(function ($route) {
                        return str_contains($route->uri(), 'admin/serrvice/sub-category') && (str_contains($route->uri(), 'edit'));
                    });
...
//        if ($type == 'provider'){
//            $fullURL = $formattedRouteName == 'Details' ? $fullURL. '?tab=overview' : $fullURL;
//        }
