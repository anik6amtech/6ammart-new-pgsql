...
class BusinessSettingsController extends Controller
{
    ...
    public function business_index(Request $request, $tab = 'business', $subTab = null)
    {
        ...
        if ($tab == 'business') {
            ...
            if(addon_published_status('RideShare')){
                $settings = DataSetting::where('type', RIDE_SHARE_BUSINESS_SETTINGS)->get();
                return view('admin-views.business-settings.deliveryman-index', compact('settings'));
            }
        ...
        } else if ($tab == 'chatting-setup') {

            if ($subTab == 'automated-message') {
                $key = explode(' ', $request['search']);
                $messages = AutomatedMessage::orderBy('id', 'desc')
                ->when($request?->search ,function($query)use($key) {
                    foreach ($key as $value) {
                ...
                $questions = QuestionAnswer::where('question_answer_for', DRIVER)->paginate(config('default_pagination'));
                return view('admin-views.business-settings.chatting-setup-rider', compact('questions'));
            } elseif ($subTab == 'support-center') {
                $savedReplies = SupportSavedReply::paginate(config('default_pagination'));
                return view('admin-views.business-settings.chatting-setup-support-center', compact('savedReplies'));
            }
            return view('admin-views.business-settings.chatting-setup');
        }
    }
    ...
    public function update_dm(Request $request)
    {
        ...
        Helpers::businessUpdateOrInsert(['key' => 'dm_loyalty_point_status'], [
            'value' => $request['dm_loyalty_point_status']
        ]);

        Helpers::businessUpdateOrInsert(['key' => 'dm_loyalty_point_exchange_rate'], [
            'value' => $request['dm_loyalty_point_exchange_rate']
        ]);
        ...
        if(addon_published_status('RideShare')){
            ...
            Helpers::dataUpdateOrInsert(['key' => 'driver_can_review_customer','type' => RIDE_SHARE_BUSINESS_SETTINGS], [
                'value' => $request['driver_can_review_customer']
            ]);

            Helpers::dataUpdateOrInsert(['key' => 'rider_level_feature_status','type' => RIDE_SHARE_BUSINESS_SETTINGS], [
                'value' => $request['rider_level_feature_status']
            ]);

            Helpers::dataUpdateOrInsert(['key' => 'update_vehicle_approval_status','type' => RIDE_SHARE_BUSINESS_SETTINGS], [
                'value' => $request['update_vehicle_approval_status']
            ]);

            Helpers::dataUpdateOrInsert(['key' => 'driver_referral_earning_status','type' => RIDE_SHARE_BUSINESS_SETTINGS], [
                'value' => $request['driver_referral_earning_status']
            ]);

            Helpers::dataUpdateOrInsert(['key' => 'driver_share_code_earning','type' => RIDE_SHARE_BUSINESS_SETTINGS], [
                'value' => $request['driver_share_code_earning']
            ]);

            Helpers::dataUpdateOrInsert(['key' => 'driver_use_code_earning','type' => RIDE_SHARE_BUSINESS_SETTINGS], [
                'value' => $request['driver_use_code_earning']
            ]);

            Helpers::dataUpdateOrInsert(['key' => 'update_vehicle_approval','type' => RIDE_SHARE_BUSINESS_SETTINGS], [
                'value' => json_encode($request['update_vehicle_approval'])
            ]);
        }
...
// dd( $request['commission_business_model']);
    ...
    public function fcm_index(Request $request)
    {
        ...
        if ($moduleType == 'rental' && addon_published_status('Rental')) {
        ...
        }elseif ($moduleType == 'service' && addon_published_status('Service')) {
            ...
            $formHtml = \Cache::remember($cacheKey, now()->addMinutes(60), function () use ($langs, $defaultLang, $moduleType) {
                $notificationMessages = \App\Models\NotificationMessage::with('translations')
                    ->where('module_type', $moduleType)
                    ->get()
                    ->keyBy('key');

                return view('admin-views.business-settings.partials.fcm-form', [
                    'language' => $langs,
            ...
            return view('admin-views.business-settings.fcm-index-service', compact('formHtml', 'langs', 'defaultLang','language'));


        }elseif ($moduleType == 'ride-share' && addon_published_status('RideShare')) {
    ...
    public function update_fcm_messages_service(Request $request)
    {
        ...
        $notificationUserTypes = [
            'user' => NOTIFICATION_FOR_SERVICE_USER,
            'provider' => NOTIFICATION_FOR_SERVICE_PROVIDER,
            'serviceman' => NOTIFICATION_FOR_SERVICE_SERVICEMAN,
        ];
...
//        file_put_contents(base_path('.env'), implode(PHP_EOL, $lines) . PHP_EOL);
    ...
    public function update_admin_landing_page_settings(Request $request, $tab)
    {
        ...
        if ($tab == 'fixed-data') {
        ...
        } elseif ($tab == 'earning-provider-link') {
            $earning_provider_image = DataSetting::where('type', 'admin_landing_page')->where('key', 'earning_provider_image')->first();
            if ($earning_provider_image == null) {
                $earning_provider_image = new DataSetting();
            }
            $earning_provider_image->key = 'earning_provider_image';
            $earning_provider_image->type = 'admin_landing_page';
            $earning_provider_image->value = $request->has('earning_provider_image') ? Helpers::update('earning/', $earning_provider_image->value, 'png', $request->file('earning_provider_image')) : $earning_provider_image->value;
            $earning_provider_image->save();
            ...
            Helpers::dataUpdateOrInsert(['key' => 'provider_app_earning_links', 'type' => 'admin_landing_page'], [
                'value' => json_encode([
                    'playstore_url_status' => $request['playstore_url_status'],
                    'playstore_url' => $request['playstore_url'],
                    'apple_store_url_status' => $request['apple_store_url_status'],
            ...
            Toastr::success(translate('messages.provider_links_updated'));
        } elseif ($tab == 'earning-serviceman-link') {
            $earning_serviceman_image = DataSetting::where('type', 'admin_landing_page')->where('key', 'earning_serviceman_image')->first();
            if ($earning_serviceman_image == null) {
                $earning_serviceman_image = new DataSetting();
            }
            $earning_serviceman_image->key = 'earning_serviceman_image';
            $earning_serviceman_image->type = 'admin_landing_page';
            $earning_serviceman_image->value = $request->has('earning_serviceman_image') ? Helpers::update('earning/', $earning_serviceman_image->value, 'png', $request->file('earning_serviceman_image')) : $earning_serviceman_image->value;
            $earning_serviceman_image->save();
            ...
            Helpers::dataUpdateOrInsert(['key' => 'serviceman_app_earning_links', 'type' => 'admin_landing_page'], [
                'value' => json_encode([
                    'playstore_url_status' => $request['playstore_url_status'],
                    'playstore_url' => $request['playstore_url'],
                    'apple_store_url_status' => $request['apple_store_url_status'],
            ...
            Toastr::success(translate('messages.serviceman_man_links_updated'));
            ...
            Helpers::dataUpdateOrInsert(['key' => 'counter_section', 'type' => 'admin_landing_page'], [
                'value' => json_encode([
                    ...
                    'seller_count_numbers' => $request['seller_count_numbers'],
                    'deliveryman_count_numbers' => $request['deliveryman_count_numbers'],
                    'provider_count_numbers' => $request['provider_count_numbers'],
                    'serviceman_count_numbers' => $request['serviceman_count_numbers'],
    ...
    public function login_url_page()
    {
        $data = array_column(DataSetting::whereIn('key', [
            'store_employee_login_url', 'store_login_url', 'admin_employee_login_url', 'admin_login_url', 'provider_login_url', 'provider_employee_login_url'
        ])->get(['key', 'value'])->toArray(), 'value', 'key');
    ...
    public function login_url_page_update(Request $request)
    {
        ...
        $request->validate([
            ...
            'provider_login_url' => 'nullable|regex:/^[a-zA-Z0-9\-\_]+$/u|unique:data_settings,value',
            'provider_employee_login_url' => 'nullable|regex:/^[a-zA-Z0-9\-\_]+$/u|unique:data_settings,value',
        ]);
        ...
        if ($request->type == 'admin') {
            ...
            DataSetting::query()->updateOrInsert(['key' => 'provider_login_url', 'type' => 'login_provider'], [
                'value' => $request->provider_login_url
            ]);
        } elseif ($request->type == 'provider_employee') {
            DataSetting::query()->updateOrInsert(['key' => 'provider_employee_login_url', 'type' => 'login_provider_employee'], [
                'value' => $request->provider_employee_login_url
            ]);
        }
