...
class CustomerController extends Controller
{
    ...
    public function rideShareView(Request $request,$id)
    {
        $key = $request['search'];
        $customer = User::find($id);
        if (isset($customer)) {
            $total_rides_amount = RideRequest::selectRaw('sum(paid_fare) as total_ride_amount')->latest()->where(['customer_id' => $id])
                ->when(isset($key), function($query) use($key){
                    $query->Where('id', 'like', "%{$key}%");
                })->get();

            $rides = RideRequest::latest()->where(['customer_id' => $id])
            ->when(isset($key), function($query) use($key){
                $query->Where('id', 'like', "%{$key}%");
            })->paginate(config('default_pagination'));
            $moduleType = 'ride-share';
            $module_id = Module::where('module_type', 'ride-share')->first()?->id;

            $reviews = RideReview::where('review_for', 'customer')
                ->where('received_by', $id)
                ->with(['givenByDeliveryMan', 'trip', 'receivedByUser'])
                ->paginate(config('default_pagination'));

            return view('admin-views.customer.customer-ride-view', compact('customer', 'rides','total_rides_amount','moduleType','module_id','reviews'));
        }
        Toastr::error(translate('messages.customer_not_found'));
        return back();
    }
    ...
    public function serviceView(Request $request,$id)
    {
        ...
        $customer = User::find($id);
        if (isset($customer)) {
            $total_booking_amount = Booking::selectRaw('sum(total_booking_amount) as total_booking_amount')->latest()->where(['customer_id' => $id])
                ->when(isset($key), function($query) use($key){
                    $query->Where('id', 'like', "%{$key}%");
                })->get();

            $bookings = Booking::latest()->where(['customer_id' => $id])
            ->when(isset($key), function($query) use($key){
                $query->Where('id', 'like', "%{$key}%");
            })->paginate(config('default_pagination'));
            $moduleType = 'service';
            $module_id = Module::where('module_type', 'service')->first()?->id;
            return view('admin-views.customer.customer-service-view', compact('customer', 'bookings','total_booking_amount','moduleType','module_id'));
        }
        Toastr::error(translate('messages.customer_not_found'));
        return back();
    }
