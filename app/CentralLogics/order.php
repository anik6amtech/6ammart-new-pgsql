...
class OrderLogic
{
    ...
    public static function create_transaction($order, $received_by=false, $status = null)
    {
        ...
        try{
            ...
            $admin_total_commission = $comission_amount + $order->additional_charge - $admin_subsidy- $admin_coupon_discount_subsidy -$store_discount_amount - $flash_admin_discount_amount - $ref_bonus_amount;
            ...
            {
                DB::beginTransaction();
                $dm_collected_cash_without_admin_commission = 0;
                ...
                else if($received_by=='deliveryman' && $order->delivery_man && $order->delivery_man->type == 'zone_wise')
                {
                    ...
                    $dm_over_flow =  true ;
                    $dm_collected_cash_without_admin_commission = ($order->order_amount-$order->partially_paid_amount) - $admin_total_commission;
                }

                $adminWallet->save();

                $admin_user_account = (new OrderLogic)->getUserAccount($adminUserId, 'admin');
                if($order->payment_method == "cash_on_delivery"){
                    $admin_user_account->receivable_balance += $admin_total_commission;
                }else{
                    $admin_user_account->payable_balance += $order->order_amount - $admin_total_commission;
                    $admin_user_account->received_balance += $admin_total_commission;
                }
                ...
                if(isset($dmWallet)){
                    ...
                    $dm_user_account = (new OrderLogic)->getUserAccount($order->delivery_man_id, DRIVER);
                    if($order->payment_method == "cash_on_delivery"){
                        $dm_user_account->payable_balance += $admin_total_commission;
                        $dm_user_account->received_balance += $dm_collected_cash_without_admin_commission;
                    }else{
                        $dm_user_account->receivable_balance += $dm_collected_cash_without_admin_commission;
                    }
                    $dm_user_account->save();
                }
                ...
                if($order->is_guest  == 0){
                    ...
                    if(isset($order->customer->ref_by) && CustomerLogic::check_referral_availability($order->customer->id)  && $ref_status == 1){
    ...
    public static function refund_order($order)
    {
        ...
        $admin_order_commission = $order_transaction->admin_commission + $order_transaction->delivery_fee_comission;

        $admin_user_account = (new OrderLogic)->getUserAccount($adminUserId, 'admin');
        if($order->payment_method == "cash_on_delivery"){
            $admin_user_account->receivable_balance -= $admin_order_commission;
        }else{
            $admin_user_account->payable_balance -= ($order->order_amount - $admin_order_commission);
            $admin_user_account->received_balance -= $admin_order_commission;
        }
        $admin_user_account->save();
