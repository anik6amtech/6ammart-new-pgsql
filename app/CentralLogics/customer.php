...
class CustomerLogic
{
    ...
    public static function create_wallet_transaction($user_id, float $amount, $transaction_type, $referance, $update_user_account = true)
    {
        ...
        try {
            ...
            if ($update_user_account) {
                $user_account = (new CustomerLogic)->getUserAccount($user->id, CUSTOMER);
                $user_account->wallet_balance = $user->wallet_balance;
                if ($transaction_type == 'referrer') {
                    $user_account->referral_earn += $amount;
                }
                $user_account->save();
            }
    ...
    public static function create_loyalty_point_transaction($user_id, $referance, $amount, $transaction_type)
    {
        ...
        if ( in_array($transaction_type, ['order_place','trip_booking','repeat_booking_place','booking_place']) ) {
    ...
    public static function check_referral_availability($user_id)
    {
        $user = User::find($user_id);
        if (!$user) {
            return false;
        ...
        if (count($user->orders) == 0 && count($user->trips) == 0 && count($user->customerRides) == 0 && count($user->bookings) == 0) {
            return true;
        }
    ...
    public static function check_ride_referral_availability($user_id)
    {
        $user = User::find($user_id);
        if (!$user) {
            return false;
        ...
        if (count($user->orders) == 0 && count($user->trips) == 0 && count($user->bookings) == 0) {
            return true;
        }
