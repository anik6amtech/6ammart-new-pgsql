...
use Modules\Service\Mail\BusinessSettingsModule\RenewSubscriptionMail;
...
//        }
    ...
    public static function dm_rating_count($deliveryman_id, $rating)
    {
        ...
        $ride = DB::table('ride_reviews')
            ...
            ->where('received_by', $deliveryman_id)
            ->where('rating', $rating)
            ->select(DB::raw('COUNT(*) as total'));

        $union = $dm->unionAll($ride);
    ...
    public static function get_provider_id()
    {
        ...
        }elseif (auth('provider_employee')->check()) {
            return auth('provider_employee')->user()->provider_id;
        }
        return 0;
    }

    public static function get_provider_data() {
        if (auth('provider')->check()) {
            return auth('provider')->user();
        } else if (auth('provider_employee')->check()) {
            return auth('provider_employee')->user()->provider;
        }
        return 0;
    }
    ...
    public static function get_provider_loggedin_user()
    {
        if (auth('provider')->check()) {
            return auth('provider')->user();
        } else if (auth('provider_employee')->check()) {
            return auth('provider_employee')->user();
        }
        return 0;
    }
    ...
    public static function provider_employee_module_permission_check($mod_name)
    {
        if (auth('provider')->check()) {
            return true;
        } else if (auth('provider_employee')->check()) {
            $permission = auth('provider_employee')->user()->role->modules;
            if (isset($permission) && in_array($mod_name, (array)json_decode($permission)) == true) {
                return true;
            }
        }
    ...
    public static function generate_dm_referer_code() {
        $ref_code = strtoupper(Str::random(10));

        if (self::referer_dm_code_exists($ref_code)) {
            return self::generate_dm_referer_code();
        }

        return $ref_code;
    }

    public static function referer_dm_code_exists($ref_code) {
    ...
    public static function get_login_url($type){
        $data=DataSetting::whereIn('key',['store_employee_login_url','store_login_url','admin_employee_login_url','admin_login_url','provider_login_url','provider_employee_login_url'
    ...
    public static function get_full_url($path,$data,$type,$placeholder = null){
        $place_holders = [
            ...
            'provider' => asset('public/assets/admin/img/160x160/img1.jpg'),
...
//                return rtrim($awsUrl, '/') . '/' . ltrim($awsBucket . '/' . $path . '/' . $data, '/');
    ...
    public static function serviceSubscriptionConditionsCheck($provider_id ,$package_id,){
        $store = Provider::findOrFail($provider_id);
        $package = SubscriptionPackage::withoutGlobalScope('translate')->find($package_id);
        $total_food = $store->subscribed_services()->count();
        if ($package->max_product != 'unlimited' &&  $total_food >= $package->max_product  ){
            return ['disable_item_count' => $total_food - $package->max_product];
        }
        return null;
    ...
    public static function subscription_plan_chosen($store_id ,$package_id, $payment_method  ,$discount = 0,$pending_bill =0,$reference=null ,$type=null){
        ...
        try {
            ...
            elseif($store_old_subscription && $store_old_subscription->package_id == $package->id && $type == 'renew' ){
                ...
                StoreSubscription::where('store_id',$store->id)->where('store_type', 'store')->update([
                    'status' => 0,
                ]);
            ...
            } elseif(StoreSubscription::where('store_id',$store->id)->where('store_type', 'store')->where('is_trial',0)->count() > 0 || $reference == 'plan_shift_by_admin'){
    ...
    public static function service_provider_subscription_plan_chosen($provider_id ,$package_id, $payment_method  ,$discount = 0,$pending_bill =0,$reference=null ,$type=null){
        $store=Provider::find($provider_id);
        $package = SubscriptionPackage::withoutGlobalScope('translate')->find($package_id);
        $add_days=0;
        $add_orders=0;
        ...
        try {
            ...
            if (isset($store_subscription) && $type == 'renew') {
                ...
                self::serviceProviderCalculateSubscriptionRefundAmount($store);
                ...
                $store_subscription = new StoreSubscription();
                $store_subscription->store_type = 'service_provider';
                $store_subscription->total_package_renewed = 0;
            }
            ...
            $store_subscription->review=$package->review;
            $store_subscription->self_delivery=$package->self_delivery;
            $store_subscription->scheduled_service = $package->scheduled_service;
            $store_subscription->service_request = $package->service_request;
            $store_subscription->advertisement = $package->advertisement;
            $store_subscription->reports_and_analytics = $package->reports_and_analytics;
            $store_subscription->bidding = $package->bidding;
            $store_subscription->is_canceled=0;
            $store_subscription->canceled_by='none';
            ...
            $store->package_id= $package->id;
            // $store->reviews_section= 1;
            $store->business_model= 'subscription';

            $subscription_transaction= new SubscriptionTransaction();

            $subscription_transaction->store_type = 'service_provider';
            $subscription_transaction->package_id = $package->id;
            $subscription_transaction->store_id = $store->id;
            $subscription_transaction->price = $package->price;
            ...
            $subscription_transaction->payment_status = 'success';
            $subscription_transaction->created_by = in_array($payment_method, ['wallet_payment_by_admin', 'manual_payment_by_admin', 'plan_shift_by_admin']) ? 'Admin' : 'Provider';
            ...
            if(in_array($type ,['renew','free_trial'])){
                $subscription_transaction->plan_type = $type;
            } elseif(StoreSubscription::where('store_id',$store->id)->where('store_type','service_provider')->where('is_trial',0)->count() > 0 || $reference == 'plan_shift_by_admin'){
                $subscription_transaction->plan_type = 'new_plan';
            }
            ...
            $subscription_transaction->package_details=[
                ...
                'max_order'=>$package->max_order,
                'max_product'=>$package->max_product,
                'scheduled_service' => $package->scheduled_service,
                'service_request' => $package->service_request,
                'advertisement' => $package->advertisement,
                'reports_and_analytics' => $package->reports_and_analytics,
        ...
        if(data_get(self::serviceSubscriptionConditionsCheck(provider_id:$store->id,package_id:$package->id) , 'disable_item_count') > 0){
            $disable_item_count=data_get(Helpers::serviceSubscriptionConditionsCheck(provider_id:$store->id,package_id:$package->id) , 'disable_item_count');
            // $store->item_section= 0;
            $store->save();
            SubscribedService::where('provider_id',$store->id)->oldest()->take($disable_item_count)->update([
                'is_subscribed' => 0
            ]);
        ...
        if(!(in_array($payment_method,['manual_payment_by_admin','plan_shift_by_admin']) && $store_old_subscription == null )){
            self::serviceSubscriptionNotifications($store,$type);
        }

        return  $subscription_transaction->id;
    }

    public static function serviceSubscriptionNotifications($store,$type ){
        try {
            if($type == 'renew' && config('mail.status')){
                Mail::to($store->email)->send(new RenewSubscriptionMail($store));
            }
    ...
    public static function serviceProviderSubscriptionPayment($provider_id,$package_id,$payment_gateway,$url,$pending_bill=0,$type='payment',$payment_platform='web'){
        $store = Provider::where('id',$provider_id)->first();
        $package = SubscriptionPackage::where('id',$package_id)->first();
        $type == null ? 'payment' :$type ;
    ...
    public static function serviceProviderCalculateSubscriptionRefundAmount($store,$return_data=null){
        ...
        if($store_subscription && $store_subscription?->is_canceled === 0 && $store_subscription?->is_trial === 0 ){
            ...
            if (Carbon::now()->diffInDays($day_left, false) > 0) {
                ...
                if(($validity - $add_days) < $subscription_usage_max_time ){
                        ...
                        shiftRefundSubscriptionTransaction($store->id, $back_amount);
    ...
    public static function getNotificationStatusDataAdmin($user_type,$key,$module_type = null){
        if ($module_type) {
            $data = NotificationSetting::where(['type'=>$user_type,'module_type' => $module_type,'key'=>$key])->select(['mail_status','push_notification_status','sms_status'])->first();
        } else {
