...
class GenerateAdminRoute extends Command
{
    ...
    public function handle()
    {
        ...
        $formattedRoutes= $this->manuallyAddedBladePath($formattedRoutes);
        ...
        if (file_exists($jsonFilePath)) {
            ...
            if (!empty($newRoutes)) {
               ...
               $updatedRoutes= $this->manuallyAddedBladePartialsPath($updatedRoutes);
                file_put_contents($jsonFilePath, json_encode($updatedRoutes, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
            }
        } else {
            $updatedRoutes= $this->manuallyAddedBladePartialsPath($formattedRoutes);
    ...
    private function manuallyAddedBladePartialsPath(array $formattedArray): array
    {
        try {
            $bladePartials = [
    ...
    private function manuallyAddedBladePath($formattedRoutes): array
    {
        $array = [
    ...
    private function genetateRouteJsonFileFormate($formattedRoutes,$bladePath, $routeName, $uri) : array  {
        ...
        foreach ($bladePaths as $moduleType => $path) {
            ...
            if($moduleType  == ""){
                ...
                $containsRental = (
                    stripos($path, 'service') !== false ||
                    stripos($routeName, 'service') !== false ||
                    stripos($uri, 'service') !== false
                );
                if($containsRental){
                    $moduleType='service';
                }
                ...
                $containsRental = (
                    stripos($path, 'ride-share') !== false ||
                    stripos($routeName, 'ride-share') !== false ||
                    stripos($uri, 'ride-share') !== false
                );
                if($containsRental){
    ...
    private function getRouteName($actualRouteName){
        ...
        if (count($routeNameParts) >= 2) {
            ...
            $uniqueWords = array_filter($uniqueWords, function ($word) {
                return strtolower($word) !== 'service';
            });

            $uniqueWords = array_filter($uniqueWords, function ($word) {
                return strtolower($word) !== 'ride-share';
            });
