    ...
    function collect_cash_success($data){
        ...
        try {
            ...
            if($data->attribute === 'deliveryman_collect_cash_payments'){
                (new class { use TransactionTrait; })->collectCashWithoutAdjustTransaction($user_data, $account_transaction->amount);
            }
...
if (!function_exists('config_settings')) {
    ...
    if (! function_exists('service_provider_sub_success')) {
        function service_provider_sub_success($data)
        {
            $type = 'renew';
            if ($data->attribute == 'store_subscription_payment') {
                $type = 'new_plan';
            ...
            $provider = Provider::find($data->payer_id);
            config(['module.current_module_id' => $provider->module_id]);
            ...
            Helpers::service_provider_subscription_plan_chosen(provider_id: $data->payer_id, package_id: $data->attribute_id, payment_method: $data->payment_method, discount: 0, pending_bill: $pending_bill, reference: $data->attribute, type: $type);
            if ($type !== 'new_join') {
                Toastr::success($type == 'renew' ?  translate('Subscription_Package_Renewed_Successfully.') : translate('Subscription_Package_Shifted_Successfully.'));
            }
            $package = SubscriptionPackage::withoutGlobalScope('translate')->find($data->attribute_id);

            purchaseSubscriptionTransaction($package->price, $data->payer_id, 0);
...
if (!function_exists('getLanguageCode')) {
    ...
    {
        foreach (GATEWAYS_LANGUAGES as $locale) {
...
if (!function_exists('business_data')) {
    function business_data(string $key)
    {
        $config = null;

        $businessData = BusinessSetting::where('key', $key)->first();
