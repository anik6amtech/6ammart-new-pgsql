...
trait TripLogicTrait
{
    ...
    public static function create_transaction($trip, $received_by = false, $status = null)
    {
        ...
        try {
            ...
            $admin_total_commission = $comission_amount + $trip->additional_charge - $admin_coupon_discount_subsidy - $discount_on_trip  - $ref_bonus_amount;
            ...
            $admin_user_account = self::getAdminAccount($adminUserId, 'admin');
            if($trip->payment_method == "cash_payment"){
                $admin_user_account->receivable_balance += $admin_total_commission;
            }else{
                $admin_user_account->payable_balance += $trip->trip_amount - $admin_total_commission;
            ...
            if ($trip->is_guest  == 0) {
                ...
                if (isset($trip->customer->ref_by) && CustomerLogic::check_referral_availability($trip->customer->id)  && $ref_status == 1) {
    ...
    public static function getAdminAccount($user_id, $user_type){
