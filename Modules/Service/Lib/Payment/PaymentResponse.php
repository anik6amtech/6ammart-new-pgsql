...
namespace Modules\Service\Lib\Payment;
...
class PaymentResponse
{
    ...
    public static function success($data): array
    {
        ...
        $module = config('module.current_module_data') ?? Module::where('module_type','service')->first();
        ...
        if (!$request->has('post_id') || is_null($request['post_id'])) {
            ...
            $response = (new PaymentResponse)->placeBookingRequestForBidding(base64_decode($request['access_token']), $request, $tran_id, $data, $module->id ?? null);
     ...
     */
    // NOTE:: Its already managed by the way of mart
