...
trait BookingScopes
{
    ...
    public function scopeProviderPendingBookings($query, Provider $provider, $maxBookingAmount)
    {
        ...
        $packageSubscriber = StoreSubscription::where('store_id', $providerId)->where('store_type', 'service_provider')->where('status',1)->latest()->first();
        $endDate = optional($packageSubscriber)->expiry_date;
...
//        $scheduleBookingEligibility = nextBookingEligibility($providerId);
       ...
       if ($packageSubscriber && $provider->business_model === 'subscription') {
           ...
           if ($isPackageEnded > 0 && $scheduleBookingEligibility && !$canceled) {
            ...
            if ($provider->service_availability && (!$provider->is_suspended || !business_config('provider_suspend_on_exceed_cash_limit', 'service_business_settings')->value)) {
                $zone_id = $provider->zone_id;
                $subscribedSubCategories = SubscribedService::where(['provider_id' => $provider->id])->where(['is_subscribed' => 1])->pluck('sub_category_id')->toArray();
