...
{
    public function find_total_Booking_amount($customer_user_id, $post_id, $provider_id, $module_id): float
    {
        $booking_additional_charge_status = BusinessSetting::where('key', 'additional_charge_status')->first()->value ?? 0;
        $extra_fee = 0;
        if ($booking_additional_charge_status) {
            $extra_fee = BusinessSetting::where('key', 'additional_charge')->first()->value ?? 0;
        }

        if (!isset($post_id)) {
            $total_amount = cart_total($customer_user_id, $module_id);
        } else {
            //for bidding
    ...
    public function find_customer_info($customer_user_id, $service_address_id): array
    {
        ...
        $address = CustomerAddress::find($service_address_id);
