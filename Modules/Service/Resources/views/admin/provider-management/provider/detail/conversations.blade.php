...
@section('title',$provider->company_name."'s ".translate('messages.conversation'))
                    ...
                    <div class="row g-3">
                        <div class="col-lg-4 col-md-6">
                            ...
                            <div class="card">
                                ...
                                <input type="hidden" id="vendor_id" value="{{ $provider->id }}">
                                <!-- Body -->
                                <div class="card-body p-0" style="overflow-y: scroll;height: 600px" id="vendor-conversation-list">
                                    <div class="border-bottom"></div>
                                    @include('service::admin.provider-management.provider.detail.partials._conversation_list')
                                </div>
                                <!-- End Body -->
                            </div>
                            <!-- End Card -->
...
<script>
    ...
    function viewConvs(url, id_to_active, conv_id, sender_id) {
        ...
        let new_url= "{{route('admin.service.provider.details', ['id'=>$provider->id, 'tab'=> 'conversations'])}}" + '&conversation=' + conv_id+ '&user=' + sender_id;
            $.get({
                url: url,
    ...
    function loadMoreData(page) {
        $.ajax({
                url: "{{ route('admin.service.provider.message.list') }}" + '?page=' + page,
                type: "get",
    ...
    function fetch_data(page, query) {
            $.ajax({
                url: "{{ route('admin.service.provider.message.list') }}" + '?page=' + page + "&key=" + query,
                type: "get",
                data:{"user_id":user_id},
                success: function(data) {
