...
@section('title',$provider->company_name."'s ".translate('messages.settings'))
...
<div class="content container-fluid">
    ...
    <div class="tab-content">
        <div class="tab-pane fade show active" id="vendor">
            ...
            <form action="{{ route('admin.service.provider.update-settings', $provider->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                                ...
                                <div class="col-xxl-6 col-lg-7 col-md-7">
                                    <div class="d-flex align-items-center restriction-time rounded border">
                                        ...
                                        <select class="custom-select max-w-100px border-0 h--45px px-2" name="time_restriction_on_schedule_booking_type"
                                            ...
                                            <option value="hours" {{ ($settings['time_restriction_on_schedule_booking_type'] ?? '') == 'hours' ? 'selected' : '' }}>{{ translate('messages.hour') }}</option>
            ...
            {{-- @if (!config('module.'.$provider->module_type)['always_open']) --}}
                <div class="card mt-3">
                    ...
                    <div class="card-body" id="schedule">
                        @include('service::admin.provider-management.provider.detail.partials._schedule', $provider)
                    </div>
                </div>
            {{-- @endif --}}
        </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:" method="post" id="add-schedule" data-route="{{route('admin.service.provider.add-schedule')}}">
                    @csrf
