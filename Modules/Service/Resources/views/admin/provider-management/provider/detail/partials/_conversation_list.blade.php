@foreach($conversations as $conv)
    ...
    @if (isset($user ) && $conv->last_message)
        ...
        <div
            ...
            data-url="{{route('admin.service.provider.message.view',['conversation_id'=>$conv->id,'user_id'=>$user->id])}}" data-active-id="customer-{{$user->id}}" data-conv-id="{{ $conv->id }}" data-sender-id="{{ $user->id }}"
            id="customer-{{$user->id}}">
