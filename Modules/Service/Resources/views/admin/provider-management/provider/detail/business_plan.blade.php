...
@section('title',$provider->company_name."'s ".translate('messages.business_plan'))
...
<div class="content container-fluid">
    ...
    @if ($provider->business_model == 'commission' &&  \App\CentralLogics\Helpers::commission_check())
            ...
            <div class="__bg-F8F9FC-card __plan-details">
                <div class="d-flex flex-wrap flex-md-nowrap justify-content-between __plan-details-top">
                    <div class="w-100">
                        ...
                        <h4 class="title mt-2"><span class="text-180">{{ $provider->commission > 0 ?  $provider->commission :  $admin_commission }} %</span> {{ translate('messages.Commission_per_'.$orderOrTrip) }}</h4>
                        <div class="info-text ">
                            {{ translate($title . ' will pay') }} {{ $provider->commission > 0 ?  $provider->commission :  $admin_commission }}% {{ translate('commission to') }} <strong>{{ $provider->company_name }}</strong> {{ translate('from each '.$orderOrTrip.'. You will get access of all the features and options  in '.$title.' panel , app and interaction with user.') }}
                        </div>
                        <div class="mt-3">
                            <form action="{{route('admin.service.provider.commission_update',[$provider['id'] , 'tab' => 'business_plan'])}}" method="post">
                                @csrf
                                @method("post")
                                <div class="row">
                                    <div class="col-xl-6 col-xxl-5">
                                        <div>
                                            <label class="d-flex mb-1 justify-content-between switch toggle-switch-sm text-dark text-capitalize" for="comission_status">
                                                ...
                                                <input type="checkbox" class="toggle-switch-input" name="commission_status" id="comission_status" value="1" {{($provider->commission_type == 'custom')?'checked':''}}>
                                                <span class="toggle-switch-label">
                                                    <span class="toggle-switch-indicator"></span>
                                                </span>
                                            </label>
                                            <div class="d-flex flex-wrap gap-3">
                                                <input type="number" id="commission" min="0" max="10000" step="0.01" name="commission" class="form-control w-200px flex-grow-1 bg-white" required value="{{$provider->commission??'0'}}" {{isset($provider->commission)?'':'readonly'}}>
    ...
    @elseif (in_array($provider->business_model,[ 'subscription' ,'unsubscribed']) && $provider?->store_sub_update_application)
                            ...
                            <div class="col-sm-6 col-lg-4">
                                <a class="__card-2 __bg-1 flex-row align-items-center gap-4" href="#">
                                    ...
                                    <div class="w-0 flex-grow-1 py-md-3">
                                        ...
                                        <h4 class="title m-0">{{  \App\CentralLogics\Helpers::date_format($provider?->store_sub_update_application?->expiry_date_parsed) }}</h4>
                                    </div>
                                </a>
                            </div>
                            <div class="col-sm-6 col-lg-4">
                                <a class="__card-2 __bg-8 flex-row align-items-center gap-4" href="#">
                                    ...
                                    <div class="w-0 flex-grow-1 py-md-3">
                                        ...
                                        <h4 class="title m-0">{{  \App\CentralLogics\Helpers::format_currency($provider?->store_sub_update_application?->package?->price * ($provider?->store_sub_update_application?->total_package_renewed + 1) ) }}</h4>
                                    </div>
                                </a>
                            </div>
                            <div class="col-sm-6 col-lg-4">
                                <a class="__card-2 __bg-4 flex-row align-items-center gap-4" href="#">
                                    ...
                                    <div class="w-0 flex-grow-1 py-md-3">
                                        ...
                                        <h4 class="title m-0">{{ $provider?->store_sub_update_application?->total_package_renewed + 1 }}</h4>
                                    </div>
                                </a>
                            </div>
                        </div>
                ...
                <div class="card mb-3">
                    <div class="card-header border-0 align-items-center">
                        <h4 class="card-title align-items-center gap-2">
                            ...
                            <span>{{ translate('Package Overview') }}
                                ...
                                @elseif($provider?->store_sub_update_application?->status == 0)
                                <span class=" badge badge-pill badge-danger">  &nbsp; {{ translate('Expired') }}  &nbsp; </span>
                                @elseif ($provider?->store_sub_update_application?->is_canceled == 1)
                                <span class=" badge badge-pill badge-warning">  &nbsp; {{ translate('canceled') }}  &nbsp; </span>
                                @elseif($provider?->store_sub_update_application?->status == 1)
                    ...
                    <div class="card-body pt-0">
                        <div class="__bg-F8F9FC-card __plan-details">
                            <div class="d-flex flex-wrap flex-md-nowrap justify-content-between __plan-details-top">
                                <div class="left">
                                    <h3 class="name">{{ $provider?->store_sub_update_application?->package?->package_name }}</h3>
                                    <div class="font-medium text--title">{{ $provider?->store_sub_update_application?->package?->text }}</div>
                                </div>
                                <h3 class="right">{{ \App\CentralLogics\Helpers::format_currency($provider?->store_sub_update_application?->last_transcations?->price) }} /<small class="font-medium text--title">{{ $provider?->store_sub_update_application?->last_transcations?->validity }} {{ translate('messages.Days') }}</small></h3>
                            </div>
                            ...
                            <div class="check--grid-wrapper mt-3 max-w-850px">
                                ...
                                <div>
                                    <div class="d-flex align-items-center gap-2">
                                        @if ( $provider?->store_sub_update_application?->mobile_app == 1 )
                                ...
                                <div>
                                    <div class="d-flex align-items-center gap-2">
                                        @if ( $provider?->store_sub_update_application?->review == 1 )
                                        <img src="{{asset('/public/assets/admin/img/subscription-plan/check.png')}}" alt="">
                                        @else
                                        <img src="{{asset('/public/assets/admin/img/subscription-plan/check-1.png')}}" alt="">
                                        @endif
                                ...
                                <div>
                                    <div class="d-flex align-items-center gap-2">
                                        @if ( $provider?->store_sub_update_application?->chat == 1 )
                                        <img src="{{asset('/public/assets/admin/img/subscription-plan/check.png')}}" alt="">
                                        @else
                                        <img src="{{asset('/public/assets/admin/img/subscription-plan/check-1.png')}}" alt="">
                                        @endif
                                ...
                                <div>
                                    <div class="d-flex align-items-center gap-2">
                                        <img src="{{asset('/public/assets/admin/img/subscription-plan/check.png')}}" alt="">
                                        @if ( $provider?->store_sub_update_application?->max_order == 'unlimited' )
                                        <span class="form-check-label text-dark">{{ translate('messages.unlimited_bookings') }}</span>
                                        @else
                                        <span class="form-check-label text-dark"> {{ $provider?->store_sub_update_application?->package?->max_order }} {{
                                            translate('messages.Bookings') }} <small>({{ $provider?->store_sub_update_application?->max_order }} {{ translate('left') }}) </small> </span>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div>
                                    <div class="d-flex align-items-center gap-2">
                                        <img src="{{asset('/public/assets/admin/img/subscription-plan/check.png')}}" alt="">
                                        @if ( $provider?->store_sub_update_application?->max_product == 'unlimited' )
                                        <span class="form-check-label text-dark">{{ translate('messages.unlimited_service_sub_category') }}</span>
                                        @else
                                        <span class="form-check-label text-dark"> {{ $provider?->store_sub_update_application?->max_product }} {{
                                            translate('messages.service_sub_category') }} <small>({{ $provider?->store_sub_update_application?->max_product - $provider?->subscribed_services?->count() }} {{ translate('left') }}) </small></span>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div>
                                    <div class="d-flex align-items-center gap-2">
                                        @if ( $provider?->store_sub_update_application?->scheduled_service == 1 )
                                        <img src="{{asset('/public/assets/admin/img/subscription-plan/check.png')}}" alt="">
                                        @else
                                        <img src="{{asset('/public/assets/admin/img/subscription-plan/check-1.png')}}" alt="">
                                        @endif
                                        <span class="form-check-label text-dark">{{ translate('messages.scheduled_service') }}</span>
                                    </div>
                                </div>

                                <div>
                                    <div class="d-flex align-items-center gap-2">
                                        @if ( $provider?->store_sub_update_application?->service_request == 1 )
                                        <img src="{{asset('/public/assets/admin/img/subscription-plan/check.png')}}" alt="">
                                        @else
                                        <img src="{{asset('/public/assets/admin/img/subscription-plan/check-1.png')}}" alt="">
                                        @endif
                                        <span class="form-check-label text-dark">{{ translate('messages.service_request') }}</span>
                                    </div>
                                </div>

                                <div>
                                    <div class="d-flex align-items-center gap-2">
                                        @if ( $provider?->store_sub_update_application?->advertisement == 1 )
                                        <img src="{{asset('/public/assets/admin/img/subscription-plan/check.png')}}" alt="">
                                        @else
                                        <img src="{{asset('/public/assets/admin/img/subscription-plan/check-1.png')}}" alt="">
                                        @endif
                                ...
                                <div>
                                    <div class="d-flex align-items-center gap-2">
                                        @if ( $provider?->store_sub_update_application?->reports_and_analytics == 1 )
                                        <img src="{{asset('/public/assets/admin/img/subscription-plan/check.png')}}" alt="">
                                        @else
                                        <img src="{{asset('/public/assets/admin/img/subscription-plan/check-1.png')}}" alt="">
                                        @endif
                                        <span class="form-check-label text-dark">{{ translate('messages.reports_and_analytics') }}</span>
                                    </div>
                                </div>

                                <div>
                                    <div class="d-flex align-items-center gap-2">
                                        @if ( $provider?->store_sub_update_application?->bidding == 1 )
                                        <img src="{{asset('/public/assets/admin/img/subscription-plan/check.png')}}" alt="">
                                        @else
                                        <img src="{{asset('/public/assets/admin/img/subscription-plan/check-1.png')}}" alt="">
                                        @endif
                                        <span class="form-check-label text-dark">{{ translate('messages.bidding') }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="btn--container justify-content-end mt-3">
                            @if ( $provider?->store_sub_update_application?->is_canceled == 0 && $provider?->store_sub_update_application?->status == 1  )
                            <button type="button"  data-url="{{route('admin.service.provider.cancel_subscription',['id' => $provider?->id, 'subscription_id' => $provider?->store_sub_update_application?->id])}}" data-message="{{translate('If_you_cancel_the_subscription,_after_')}} {{  Carbon\Carbon::now()->diffInDays($provider?->store_sub_update_application?->expiry_date_parsed->format('Y-m-d'), false); }} {{ translate('days_the_vendor_will_no_longer_be_able_to_run_the_business_before_subscribe_a_new_plan.') }}"
                ...
                <div class="modal-body px-4 pt-0">
                    <div>
                        ...
                        <div class="plan-slider owl-theme owl-carousel owl-refresh">
                            ...
                            <div class="__plan-item hover {{ $provider->store_business_model == 'commission'  ? 'active' : ''}} ">
                                <div class="inner-div">
                                    <div class="text-center">
                                        <h3 class="title">{{ translate('Commission Base') }}</h3>
                                        <h2 class="price">{{  $provider->commission > 0 ?  $provider->commission :  $admin_commission }}%</h2>
                                    </div>
                                    <div class="py-5 mt-4">
                                        <div class="info-text text-center">
                                            {{ translate($title.' will pay') }} {{  $provider->commission > 0 ?  $provider->commission :  $admin_commission }}% {{ translate('commission to') }} {{ $provider->company_name }} {{ translate('from each '.$orderOrTrip.'. You will get access of all the features and options  in '.$title.' panel , app and interaction with user.') }}
                                        </div>
                                    </div>
                                    <div class="text-center">
                                        ...
                                        @else
                                        @php
                                        $cash_backs= \App\CentralLogics\Helpers::calculateSubscriptionRefundAmount(store:$provider ,return_data:true);
                                        @endphp

                                        <button type="button" data-url="{{route('admin.service.provider.switch_to_commission',$provider->id)}}" data-message="{{translate('You_Want_To_Migrate_To_Commission.')}} {{ data_get($cash_backs,'back_amount') > 0  ?  translate('You will get').' '. \App\CentralLogics\Helpers::format_currency(data_get($cash_backs,'back_amount')) .' '.translate('to_your_wallet_for_remaining') .' '.data_get($cash_backs,'days').' '.translate('messages.days_subscription_plan') : '' }}"  class="btn btn--primary shift_to_commission">{{ translate('Shift in this plan') }}</button>
                                        @endif
                            ...
                            <div class="__plan-item hover {{ $provider?->store_sub_update_application?->package_id == $package->id  && $provider->store_business_model != 'commission'  ? 'active' : ''}}">
                                <div class="inner-div">
                                    <div class="text-center">
                                    ...
                                    <ul class="info">
                                        ...
                                        <li>
                                            <i class="tio-checkmark-circle"></i> <span>  {{ translate('messages.Unlimited_Booking') }} </span>
                                        </li>
                                        @else
                                        <li>
                                            <i class="tio-checkmark-circle"></i> <span>  {{ $package->max_order }} {{ translate('messages.Bookings') }} </span>
                                        </li>
                                        @endif
                                        @if ($package->max_product == 'unlimited')
                                        <li>
                                            <i class="tio-checkmark-circle"></i> <span>  {{ translate('messages.Unlimited_service_sub_category') }} </span>
                                        </li>
                                        @else
                                        <li>
                                            <i class="tio-checkmark-circle"></i> <span>  {{ $package->max_product }} {{ translate('messages.service_sub_category') }} </span>
                                        </li>
                                        @endif
                                    ...
                                    <div class="text-center">
                                        ...
                                        @if ( $provider?->business_model != 'commission'  && $provider?->store_sub_update_application?->package_id == $package->id)
                                        <button data-id="{{ $package->id }}"  data-url="{{route('admin.service.provider.subscription_package_view',[$package->id,$provider->id ])}}"
    ...
    <div id="index" data-index="{{ $index ?? 0 }}"></div>
    ...
    <div id="storeId" data-id="{{ $provider->id }}"></div>
    <div id="storeSubId" data-sub-id="{{ $provider?->store_sub_update_application?->id }}"></div>
...
@push('script_2')
    <script src="{{asset('Modules/Service/public/assets/js/admin/provider-subscription.js')}}"></script>
@endpush
