    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <h1 class="page-header-title mr-3 mb-md-0">
                ...
                <span>
                    {{ translate('messages.service_provider_withdraw_transaction')}}
                </span>
            </h1>
        </div>
        <!-- Page Heading -->
                ...
                <div class="search--button-wrapper justify-content-end">
                    <form class="search-form theme-style">
                        ...
                        <div class="input-group input--group">
                            <input id="datatableSearch" name="search" type="search" value="{{ request()?->search ?? null}}" class="form-control h--40px" placeholder="{{translate('ex_:_search_provider_name')}}" aria-label="{{translate('messages.search_here')}}">
                    ...
                    <div class="hs-unfold mr-2">
                        ...
                        <div id="usersExportDropdown"
                            ...
                            <a id="export-excel" class="dropdown-item" href="{{route('admin.transactions.service.provider.withdraw_export', ['type'=>'excel',request()->getQueryString()])}}">
                                <img class="avatar avatar-xss avatar-4by3 mr-2"
                                    src="{{ asset('public/assets/admin') }}/svg/components/excel.svg"
                                    alt="Image Description">
                                {{ translate('messages.excel') }}
                            </a>
                            <a id="export-csv" class="dropdown-item" href="{{route('admin.transactions.service.provider.withdraw_export', ['type'=>'csv',request()->getQueryString()])}}">
                                <img class="avatar avatar-xss avatar-4by3 mr-2"
                    ...
                    <table id="datatable"
                        ...
                        @foreach($withdraw_req as $k=>$wr)
                            <tr>
                                ...
                                <td>
                                    @if($wr->provider)
                                    <a class="deco-none" title="{{ $wr->provider->first_name.' '.$wr->provider->last_name }}"
                                        href="{{route('admin.service.provider.details', ['id' => $wr->provider->id, 'module_id' => $wr->provider->module_id])}}">{{ Str::limit($wr->provider->first_name.' '.$wr->provider->last_name, 20, '...') }}</a>
                                    @else
...
@push('script_2')
    <script>
        ...
        $(document).on('click', '.show-approve-view', function () {
            let id = $(this).data('id');
            let url = "{{ route('admin.transactions.service.provider.withdraw_status', ['data_id']) }}";
        ...
        $('.status-filter').on('change',function () {
            ...
            $.post({
                url: '{{route('admin.transactions.service.provider.status-filter')}}',
                data: {
        ...
        function fetch_data(id) {
            $.ajax({
                url: "{{ route('admin.transactions.service.provider.getWithdrawDetails') }}" + '?withdraw_id=' + id,
                type: "get",
                beforeSend: function () {
                    $('#data-view').empty();
                    $('#loading').show()
