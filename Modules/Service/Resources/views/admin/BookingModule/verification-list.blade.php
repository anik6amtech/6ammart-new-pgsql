...
@section('title',translate('Booking verification list'))
...
@section('content')
    <div class="content container-fluid">
        ...
        <h2 class="h1 mb-sm-3 mb-2 text-capitalize d-flex align-items-center gap-2">
            {{translate('Verify Request')}} <span class="rounded py-1 fz-12px px-2 mb-0 text-title" data-bg-color="#3342571A">{{ $bookings->total() }}</span>
        </h2>
        <!-- Nav -->
            ...
            <ul class="nav nav-tabs align-items-start border-0 gap-20px nav--tabs tabs__style-border" id="myTab"
                ...
                <li class="nav-item">
                    <a class="nav-link {{$type=='pending'?'active':''}}"
                       href="{{route('admin.service.booking.list.verification', ['booking_status'=>'pending', 'type' => 'pending'])}}">
                        {{ translate('pending') }}
                    </a>
                </li>
                <li class="nav-item text">
                    <a class="nav-link {{$type=='denied'?'active':''}}"
                       href="{{route('admin.service.booking.list.verification', ['booking_status'=>'pending', 'type' => 'denied'])}}">
                        {{ translate('denied') }}
                    </a>
                </li>
            </ul>
                      ...
                      <div class="d-flex flex-wrap align-items-center gap-3">
                          <div class="hs-unfold mr-2">
                              ...
                              <div id="usersExportDropdown" class="hs-unfold-content dropdown-unfold dropdown-menu dropdown-menu-sm-right hs-unfold-content-initialized hs-unfold-css-animation animated hs-unfold-hidden" data-hs-target-height="131.516" data-hs-unfold-content="" data-hs-unfold-content-animation-in="slideInUp" data-hs-unfold-content-animation-out="fadeOut" style="animation-duration: 300ms;">
                                  ...
                                  <a id="export-excel" class="dropdown-item" href="{{ route('admin.service.booking.download.verification.list',$queryParams) }}">
                                    ...
                                    <td>
                                        <div class="table-actions d-flex gap-2">
                                            ...
                                            @if($booking->is_verified == '0')
                                                    <button type="button" data-verify="{{$booking->id}}" class="btn action-btn btn-outline-success booking-verify" style="--size: 30px">
                                                        <i class="tio-done"></i>
                                                    </button>

                                                    <button type="button" data-deny="{{$booking->id}}" style="--size: 30px"
                                                            class="btn action-btn btn-outline-danger booking-deny"
                                                        ...
                                                        <div class="modal-content">
                                                            <div class="modal-body pt-5 p-md-5">
                                                                ...
                                                                <div class="d-flex justify-content-center mb-4">
                                                                    <img width="75" height="75" src="{{asset('public/assets/admin/img/delete2.png')}}" class="rounded-circle" alt="">
                                                                </div>
    ...
    <script>
        (function ($) {
            "use strict";
                ...
                let route = '{{ route('admin.service.booking.verification_status_update', [':itemId']) }}';
