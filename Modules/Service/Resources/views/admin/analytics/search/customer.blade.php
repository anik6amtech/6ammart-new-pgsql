...
@section('content')
    <div class="main-content">
        <div class="container-fluid">
            ...
            <div class="row gy-2 g-3">
                <div class="col-lg-6">
                    <div class="card h-100">
                        <div class="card-body">
                            <div class="d-flex flex-wrap justify-content-between gap-3 mb-4">
                                <div class="">
                                    <h4 class="mb-1 text--clr">{{translate('Top 5 Customer')}}</h4>
                                    <p class="mb-1 text--clr">{{translate('According to search volume')}}</p>
                                </div>
                                <div class="select-wrap d-flex flex-wrap gap-10">
                                    <select class="form-control min-w180 h-30 top-customers__select">
                            ...
                            <div class="apex_donut-chart__customize">
                    ...
                    <div class="card h-100">
                        <div class="card-body">
                            <div class="d-flex flex-wrap justify-content-between gap-3">
                                <div class="">
                                    <h4 class="mb-1 text--clr">{{translate('Top Services')}}</h4>
                                    <p class="fs-12 m-0">{{translate('According to search volume')}}</p>
                                </div>
                                <div class="select-wrap d-flex flex-wrap gap-10">
                                    <select class="form-control py-1 min-w180 h-30 top-services__select">
                                        <option value="" disabled selected>{{translate('Select_Date_Range')}}</option>
                                        <option
                            ...
                            <div class="mt-4">
                                <div class="">
                                    <ul class="common-list top_service-max p-0 after-none gap-3 d-flex flex-column">
                ...
                <div class="card-body p-0">
                    ...
                    <div class="data-table-top py-3 px-3 d-flex align-items-center flex-wrap gap-3 justify-content-between">
                        <div class="d-flex gap-2 fw-medium">
                            <span class="opacity-75 text-dark">{{translate('Earning_List')}}: </span>
                        </div>
...
{{--                            </div>--}}
                    ...
                    <div class="table-responsive pb-0">
                        <table class="table align-middle-cus table-borderless m-0">
                            <thead class="text-nowrap z--1" data-bg-color="#F3F8F8">
                                <tr>
                                    <th class="border-0 text-nowrap">{{translate('SL')}}</th>
                                    <th class="border-0 text-nowrap">{{translate('Customer')}}</th>
                                    <th class="border-0 text-nowrap">{{translate('Search')}} {{translate('Volume')}}</th>
                                    <th class="border-0 text-nowrap">{{translate('Related')}} {{translate('Services')}}</th>
                                    <th class="border-0 text-nowrap">{{translate('Times Service')}} {{translate('Visited')}}</th>
                                    <th class="border-0 text-nowrap">{{translate('Times Added')}} {{translate('to Cart')}}</th>
                                    <th class="border-0 text-nowrap">{{translate('Total')}} {{translate('Booking')}}</th>
                                    <th class="border-0 text-nowrap text-center">{{translate('Action')}}</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="media align-items-center gap-3">
                                            <div class="avatar avatar-lg">
                                                <a href="{{route('admin.customer.view',[$customer->id, 'web_page'=>'overview'])}}">
                                            ...
                                            <div class="media-body">
                                                <h5>
                                                    <a href="{{route('admin.customer.view',[$customer->id, 'web_page'=>'overview'])}}" class="fs-14 text--clr">
                                                        {{$customer['name']}}
                                                    </a>
                                                </h5>
                                            </div>
                                    ...
                                    <td>
                                        <div class="table-actions d-flex justify-content-center">
                                            <a href="{{route('admin.customer.view',[$customer->id, 'web_page'=>'overview'])}}"
                                               type="button" class="btn action-btn btn-outline-primary" style="--size: 30px">
...
@push('script_2')
    ...
    <script>
        ...
        $(".top-customers__select").on('change', function () {
            if (this.value !== "") location.href = "{{route('admin.transactions.service.analytics.search.customer')}}" + '?date_range=' + this.value + '&date_range_2=' + '{{$queryParams['date_range_2']??'all_time'}}';
        });
        $(".top-services__select").on('change', function () {
            if (this.value !== "") location.href = "{{route('admin.transactions.service.analytics.search.customer')}}" + '?date_range=' + '{{$queryParams['date_range']??'all_time'}}' + '&date_range_2=' + this.value;
        });
    </script>
@endpush
