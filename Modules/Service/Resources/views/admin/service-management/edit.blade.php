...
<div class="content container-fluid">
    ...
    <form action="{{ route('admin.service.service.update', $service->id) }}" method="post" enctype="multipart/form-data">
        @method('put')
        @csrf
                        ...
                        <div class="lang_form" id="default-form">
                            <div class="mb-20">
                                ...
                                <input type="text" class="form-control" name="name[]" required
                                    value="{{ $service?->getRawOriginal('name') }}" placeholder="{{ translate('service_name') }}" maxlength="255"
                            ...
                            <div class="mb-20">
                                ...
                                <textarea name="short_description[]" rows="3" class="form-control" required placeholder="{{ translate('short_description') }}">{{ $service?->getRawOriginal('short_description') }}</textarea>
                            </div>
                            <div class="form-group custom-editor mb-0">
                                ...
                                <textarea class="form-control position-relative ckeditor" data-id="mail-body" required name="description[]">{{ $service?->getRawOriginal('description') }}</textarea>
                            </div>
                        ...
                        @foreach ($language as $key => $lang)
                            ...
                            <div class="d-none lang_form" id="{{ $lang }}-form">
                                ...
                                <input type="text" class="form-control" name="name[]"
                                    value="{{ $translate[$lang]['name'] ?? '' }}" placeholder="{{ translate('service_name') }}" maxlength="255"
                                    data-preview-text="preview-title">
                            </div>
                            <div class="mb-20">
                                <label class="form-label">{{ translate('short_description') }} ({{ strtoupper($lang) }})</label>
                                <textarea name="short_description[]" rows="3" class="form-control" placeholder="{{ translate('short_description') }}">{{ $translate[$lang]['short_description'] ?? '' }}</textarea>
                            </div>
                            <div class="form-group custom-editor mb-0">
                                <label class="form-label">
                                    {{ translate('messages.Long Description') }} ({{ strtoupper($lang) }})
                                </label>
                                <textarea class="form-control position-relative ckeditor" data-id="mail-body" name="description[]">{{ $translate[$lang]['description'] ?? '' }}</textarea>
                            </div>
                ...
                <div class="row g-4">
                    <div class="col-lg-4 col-md-6">
                        <div>
                            ...
                            <div class="global-image-upload ratio-1 border-dashed rounded-10 bg-light position-relative max-w-100 overflow-hidden border-dashed h-130 d-center has-image">
                                <input type="file" name="thumbnail" accept="image/*" style="position: absolute; width: 100%; height: 100%; opacity: 0; cursor: pointer;">
                                ...
                                <img class="global-image-preview" src="{{ $service->thumbnail_full_path }}" data-default="{{ $service->thumbnail_full_path }}" alt="Preview" style="max-height: 100%; max-width: 100%;" />
                    ...
                    <div class="col-lg-8 col-md-6">
                        <div>
                            ...
                            <div class="global-image-upload ratio-1 border-dashed rounded-10 bg-light position-relative max-w-100 overflow-hidden border-dashed h-130 d-center has-image">
                                <input type="file" accept="image/*" name="cover_image" style="position: absolute; width: 100%; height: 100%; opacity: 0; cursor: pointer;">
                                <div class="global-upload-box">
                                    <div class="upload-content text-center" style="display: none;">
                                        <img src="{{asset('Modules/Service/public/assets/img/admin/do-upload.png')}}" alt="upload-your-image" class="mb-2">
                                        <div class="d-grid align-items-center gap-1 justify-content-center">
                                ...
                                <img class="global-image-preview" src="{{ $service->cover_image_full_path }}" data-default="{{ $service->cover_image_full_path }}" alt="Preview" style="max-height: 100%; max-width: 100%;" />
                                <div class="overlay-icons">
                                    <button type="button" class="btn btn--info p-2 action-btn view-icon" title="View" data-toggle="modal" data-target="#imageShowingMOdal">
                                        <i class="tio-invisible"></i>
                                    </button>
                        ...
                        <div class="form-group mb-0">
                            ...
                            <div id="sub-category-selector">
                                <select name="sub_category_id" class="form-control js-select2-custom"
                                    ...
                                    <option value="" selected disabled>
                                        {{ translate('messages.Select sub category') }}
                                    </option>
                    ...
                    <div class="col-sm-6">
                        <div class="form-group mb-0">
                            ...
                            <div class="d-flex align-items-center border rounded overflow hidden">
                                <input type="number" name="tax" value="{{ $service->tax }}" placeholder="Ex : 5" class="form-control w-100 border-0" required>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-6">
                        <div class="form-group mb-0">
                            ...
                            <input type="number" name="min_bidding_price" value="{{ $service->min_bidding_price }}" class="form-control h--45px" placeholder="100" required>
                        </div>
                    </div>
                    <div class="col-sm-6">
                        <div class="form-group mb-0 select-couting position-relative">
                            ...
                            <input type="text" class="form-control" name="tags" value="{{ $service->tags->pluck('tag')->implode(', ') }}" placeholder="{{translate('messages.search_tags')}}" data-role="tagsinput">
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="card mb-20 variation-card-wrapper">
            <div class="p-20 border-bottom">
            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table id="variation-update-table" class="table m-0 align-middle align-middle-cus table-custom-space tr-hover">
                        ...
                        <tbody id="variation-update-tbody">
                            @include('service::admin.service-management.partials._update-variant-data',['variants'=>$service->variations,'zones'=>$zones])
                        </tbody>
                    </table>
                    <div id="new-variations-table" style="display: none;">
                        <div class="mt-3 mb-3">
                            <span class="badge badge-primary">
                                {{ translate('messages.new_variation') }}
                            </span>
                        </div>
                        <table class="table m-0 align-middle align-middle-cus table-custom-space tr-hover" id="variation-table">
                            <tbody></tbody>
                        </table>
                    </div>
                </div>
        ...
        <div class="btn--container justify-content-end mt-3">
            ...
            <button type="submit" class="btn min-w-120 btn--primary call-demo">{{ translate('messages.update') }}</button>
        </div>
    </form>
            ...
            function ajax_switch_category(route) {
                $.get({
                    ...
                    success: function (response) {
                        ...
                        // $('#sub-category-selector').trigger('change');
                        $('#category-wise-zone').html(response.template_for_zone);
                        $('#variation-table tbody').html(response.template_for_variant);
                        $('#variation-update-table tbody').html(response.template_for_update_variant);
                    },
                    complete: function () {
                    },
                });
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.get({
                        ...
                        success: function (response) {
                            $('#variation-update-tbody').html(response.template);

                            if (response.success) {
                                toastr.success(response.message ?? "{{translate('variation_removed_successfully')}}");
                        ...
                        error: function () {
                            toastr.error("{{translate('request_failed_try_again')}}");
                        }
                    });
                }
            })
