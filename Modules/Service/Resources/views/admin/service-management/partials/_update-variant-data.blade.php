        ...
        {{-- <tr>
            ...
            <td>
                <a class="btn btn-sm btn--danger service-ajax-remove-variant"
                   data-route="{{ route('admin.service.service.ajax-delete-db-variant',[$item,$variants->first()->service_id]) }}"
         ...
         <tr>
            <td class="text-title" style="background-color: #f9fafc;">
                {{str_replace('-',' ',$item)}}
                <input name="variants[]" value="{{$item}}" class="d-none">
            </td>
            <td class="max-w-170px" style="background-color: #f9fafc;">
                <input type="number" value="{{$variants->where('price','>',0)->where('variant_key',$item)->first()->price??0}}" class="form-control h-35px bg-white" id="default-set-{{$key}}"
                       onkeyup="set_values('{{$key}}')" min="0.00001" step="any" required>
            </td>
            @foreach($zones as $zone)
                <td>
                    <input type="number" name="{{$item}}_{{$zone->id}}_price" value="{{$variants->where('zone_id',$zone->id)->where('variant_key',$item)->first()->price??0}}"
