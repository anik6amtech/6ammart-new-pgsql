...
@section('notification_setup_service')
    active
@endsection
...
@section('content')
    <div class="content container-fluid">
        ...
        <ul class="nav nav-tabs border-0 nav--tabs nav--pills mb-4">
            <li class="nav-item">
                <a class="nav-link {{  request()?->type == 'provider' ?  'active' : '' }} " href="{{ route('admin.business-settings.service.notification-setup' ,['type' =>  'provider'])  }}">{{ translate('provider') }}</a>
            </li>
            <li class="nav-item">
                <a class="nav-link {{ request()?->type == 'customer' ?  'active' : '' }}"   href="{{ route('admin.business-settings.service.notification-setup' ,['type' =>  'customer'])  }}">{{ translate('Customer') }}</a>
            </li>
            <li class="nav-item">
                <a class="nav-link {{ request()?->type == 'serviceman' ?  'active' : '' }} "  href="{{ route('admin.business-settings.service.notification-setup' ,['type' =>  'serviceman'])  }}">{{ translate('serviceman') }}</a>
            </li>
        </ul>
                                        ...
                                        <div>
                                            ...
                                            @else
                                                ...
                                                <label class="toggle-switch toggle-switch-sm" data-toggle="tooltip"
                                                    ...
                                                    @endif >
                                                    <input type="checkbox"
                                                           id="push_notification_{{$item->key == 'terms_&_conditions_update' ? 'terms_and_conditions_update' : $item->key}}"
                                                ...
                                                <form action="{{route('admin.business-settings.service.update-notification',['key'=> $item->key ,'user_type' => $item->type ,'type' => 'push_notification'])}}" method="post" id="push_notification_{{$item->key == 'terms_&_conditions_update' ? 'terms_and_conditions_update' : $item->key}}_form">
                                                    @csrf
                                                </form>
                                            @endif
                                        </div>
                                    ...
                                    <div class="d-flex justify-content-center">
                                        <div>
                                            ...
                                            @else
                                                ...
                                                <form action="{{route('admin.business-settings.service.update-notification',['key'=> $item->key ,'user_type' => $item->type ,'type' => 'Mail'])}}" method="post" id="mail_{{$item->key}}_form">
                                                    @csrf
                                                </form>
                                            @endif
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            @else
                                               ...
                                               <span class="toggle-switch-indicator"></span>
                                                ...
                                                <form action="{{route('admin.business-settings.service.update-notification',['key'=> $item->key,'user_type' => $item->type ,'type' => 'SMS'])}}" method="post" id="SMS_{{$item->key}}_form">
                                                    @csrf
                                                </form>
                                            @endif
