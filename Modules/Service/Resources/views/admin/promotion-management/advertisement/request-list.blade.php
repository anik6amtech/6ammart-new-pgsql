...
<div class="content container-fluid">
    ...
    <ul class="nav nav-tabs border-0 nav--tabs nav--pills mb-4">
        <li class="nav-item">
            <a class="nav-link  {{ !request()?->type  ? 'active' : '' }}" href="{{ route('admin.service.advertisement.requestList') }}">{{ translate('New_Request') }}</a>
        </li>
        <li class="nav-item">
            <a class="nav-link {{ request()?->type == 'update-requests' ? 'active' : '' }} " href="{{ route('admin.service.advertisement.requestList',['type'=> 'update-requests']) }}">{{ translate('Update_Request') }}</a>
        </li>
        <li class="nav-item">
            <a class="nav-link {{ request()?->type == 'denied-requests' ? 'active' : '' }} " href="{{ route('admin.service.advertisement.requestList',['type'=> 'denied-requests']) }}">{{ translate('Denied_Requests') }}</a>
        </li>
    </ul>
                ...
                <table class="font-size-sm table table-borderless table-thead-bordered table-nowrap table-align-middle card-table min-h-225px">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td> <a href="{{ route('admin.service.advertisement.show',[$add->id ,'request_page_type'=> request()?->type ?? 'pending-requests']) }}"> {{ $add->id }}</a></td>
                            ...
                            <td>
                                <div class="dropdown dropdown-2">
                                    ...
                                    <ul class="dropdown-menu "dir="ltr">
                                        <a class="dropdown-item d-flex gap-2 align-items-center" href="{{ route('admin.service.advertisement.show',[$add->id ,'request_page_type'=> request()?->type ?? 'pending-requests']) }}">
                                            <i class="tio-visible-outlined"></i>
                                            {{ translate('View Ads') }}
                                        </a>

                                        @if ($add->status == 'denied' || $add->active == 0)
                                        <a class="dropdown-item d-flex gap-2 align-items-center" href="{{ route('admin.service.advertisement.edit',[$add->id ,'request_page_type'=> request()?->type ?? 'pending-requests']) }}">
                                            <i class="tio-edit"></i>
                                        ...
                                        data-approve_url={{   route('admin.service.advertisement.status',['status' => 'approved' ,'id' => $add->id ,'approved' => 1]) }}
                                        data-edit_url={{  route('admin.service.advertisement.edit',[$add->id ,'request_page_type'=> isset($request_page_type) ]) }}
                                        ...
                                        <form id="data-add-{{ $add->id }}_form" action="{{ route('admin.service.advertisement.status',['status' => 'paused' ,'id' => $add->id]) }}" method="get">
                                            @csrf
                                            @method('get')
                                        ...
                                        @if ($add->status != 'pending')
                                        <a class="dropdown-item d-flex gap-2 align-items-center" href="{{ route('admin.service.advertisement.destroy',$add->id) }}">
