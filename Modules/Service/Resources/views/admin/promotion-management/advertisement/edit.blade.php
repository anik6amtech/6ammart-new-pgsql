...
@section('title',Request::is('admin/service/advertisement/copy-advertisement/*') ? translate('New_Advertisement')  : translate('Advertisement_Edit'))
...
<div class="content container-fluid">
    ...
    <h1 class="page-header-title mb-3">
        {{ Request::is('admin/service/advertisement/copy-advertisement/*') ? translate('New_Advertisement')  : translate('Advertisement_Edit') }}
    </h1>
    ...
    <div class="card mb-20">
        <div class="card-body p-30">
            <form id="create-add-form"  method="post" enctype="multipart/form-data" >
                @csrf
                @if (Request::is('admin/service/advertisement/copy-advertisement/*'))
                @method("POST")
                        ...
                        <div class="mb-20">
                            ...
                            @if (isset($advertisement->store))
                                <option value="{{ $advertisement->store_id }}" selected="selected">
                                    {{ $advertisement->store->company_name }} ({{ $advertisement->store->zone->name }})
                                </option>
                            @endif
                            </select>
                        </div>
...
<script>
            $(document).on('ready', function() {
                    ...
                    $('#create-add-form').on('submit', function (event) {
                        ...
                        $.post({
                            url: `{{ Request::is('admin/service/advertisement/copy-advertisement/*') ? route('admin.service.advertisement.copyAddPost',$advertisement?->id) : route('admin.service.advertisement.update',$advertisement?->id) }}`,
                            ...
                            success: function (data) {
                                ...
                                if (data.errors) {
                                    ...
                                    setTimeout(function () {
                                        location.href = '{{route('admin.service.advertisement.index')}}';
                                    }, 2000);
                                }
                            }
                        });
