...
@section('title', translate('add_coupon'))
...
<div class="content container-fluid">
    ...
    <div class="card">
        <div class="card-body p-20">
            <form action="{{ route('admin.service.coupon.store') }}" method="post">
                @csrf
                <div class="row g-4">
                    <div class="col-lg-12">
                            ...
                            <div class="lang_form" id="default-form">
                                <div class="mb-20">
                                    ...
                                    <input type="text" class="form-control" name="coupon_title[]" required
                                        value="{{ old('coupon_title.0') }}" placeholder="{{ translate('title') }}" maxlength="255"
                                ...
                                <div class="d-none lang_form" id="{{ $lang }}-form">
                                    <div class="mb-20">
                                        ...
                                        <input type="text" class="form-control" name="coupon_title[]"
                                            value="{{ old('coupon_title.'.$key) }}" placeholder="{{ translate('title') }}" maxlength="255"
                                            data-preview-text="preview-title">
                                    </div>
                    ...
                    <div class="col-lg-6">
                        <div class="form-group mb-0">
                            ...
                            <input type="text" class="form-control h--45px" name="coupon_code" value="{{ old('coupon_code') }}"
                                   id="couponCode" placeholder="NEW2025" required>
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="form-group mb-0">
                            ...
                            <select class="form-control w-100" name="coupon_type"
                                ...
                                @foreach(COUPON_TYPES as $index=>$coupon_type)
                                    <option value="{{$index}}" {{ old('coupon_type') == $index ? 'selected' : '' }}>{{$coupon_type}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="mb-30">
                            ...
                            <select class="js-select theme-input-style w-100" id="customer-select"
                                ...
                                @foreach($customers as $key=>$customer)
                                    <option value="{{$customer->id}}" {{ in_array($customer->id, old('customer_user_ids', [])) ? 'selected' : '' }}>{{$customer->f_name .' '. $customer->l_name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-6" id="select_category_wrapper">
                        <div class="form-group mb-0">
                    ...
                    <div class="col-lg-6 mb-3 " style="display: none;" id="select_service_wrapper">
                        <div class="form-group mb-0">
                            ...
                            <select name="service_ids[]" id="select_service"
                                ...
                                <option value="all" {{ in_array('all', old('service_ids', [])) ? 'selected' : '' }}>{{translate('messages.all')}} </option>
                        ...
                        <div class="form-group mb-0 select-couting position-relative">
                            ...
                            <select required name="zone_ids[]" id="select_zone"
                                ...
                                @foreach($zones as $zone)
                                    <option
                                        value="{{$zone->id}}" {{ old('zone_ids') && in_array($zone->id, old('zone_ids')) ? 'selected' : '' }}>
                                        {{$zone->name}}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <div class="d-flex align-items-center restriction-time rounded border">
                                ...
                                <select class="form-select w-auto bg-light border-0 h--45px px-2"
                                        ...
                                        id="coupon_amount_type"
                                        required>
                                    <option value="percentage" {{ old('discount_amount_type', 'percentage') == 'percentage' ? 'selected': '' }}>%</option>
                    ...
                    <div class="col-lg-6" id="max-discount-box">
                        <div class="form-group mb-0">
                            ...
                            <input type="number" class="form-control h--45px" name="max_discount_amount" id="discount_max" value="{{ old('max_discount_amount') }}" placeholder="100">
                        </div>
                    </div>
                    <div class="col-lg-6">
                         <div class="form-group mb-0">
                            ...
                            <div class="position-relative date-picker__custom">
                                <input type="text" class="form-control date-range-picker2" name="coupon_duration" value="{{ old('coupon_duration') }}" data-placeholder="{{ translate('messages.03/05/2025 - 03/08/2025') }}" autocomplete="off">
                    ...
                    <div class="col-lg-6">
                        <div class="form-group mb-0">
                            ...
                            <input type="number" class="form-control h--45px" name="min_purchase" id="discount_min" value="{{ old('min_purchase') }}" placeholder="Ex: 5" step="0.01" min="0">
                        </div>
                    </div>
                    <div class="col-lg-6" id="limit_per_user__div">
                        <div class="form-group mb-0">
                            <label for="limit_per_user" class="form-label font-normal fz--14px">
                                {{ translate('Limit for same user') }} <span class="text-danger">*</span>
                            </label>
                            <input type="number" class="form-control h--45px" name="limit_per_user" id="limit_per_user" value="{{ old('limit_per_user') }}" placeholder="10" min="1">
                        </div>
                    </div>
                    <div class="col-lg-12">
...
@push('script_2')
    ...
    <script>
        ...
        //Select2 initialization for all custom selects
        $(document).ready(function () {
            ...
            // ensure customer-select uses select2 with placeholder
            $("#customer-select").select2({
                placeholder: "{{translate('Select_customer')}}",
                width: '100%'
            });

            // Restore UI state based on old values
            var oldDiscountType = '{{ old('discount_type', 'category') }}';
            if (oldDiscountType === 'service') {
                $('#select_category_wrapper').hide();
                $('#select_service_wrapper').show();
                $('#select_service').prop('required', true);
                $('#select_category').prop('required', false);
            } else if (oldDiscountType === 'mixed') {
                $('#select_category_wrapper').show();
                $('#select_service_wrapper').show();
                $('#select_service').prop('required', true);
                $('#select_category').prop('required', true);
            ...
            // If any select has 'all' selected on load, select all options (mimic change behavior)
            ['#select_category','#select_service','#select_zone'].forEach(function(sel){
                var $el = $(sel);
                if ($el.length) {
                    var vals = $el.val() || [];
                    if (vals.includes && vals.includes('all')) {
                        $el.find('option').not(':disabled').prop('selected', 'selected');
            ...
            // Trigger coupon-type change to set customer/limit visibility based on old value
            $('#coupon-type').trigger('change');

            // Coupon amount type and limit logic
            ...
            function checkCouponLimit() {
                if ($couponType.val() === 'percentage') {
                    ...
                    if ($couponInput.val() && parseFloat($couponInput.val()) > 100) {
                        $couponInput.val(100);
                    }
                } else {
            ...
            function toggleMaxDiscount() {
                if ($couponType.val() === 'amount') {
                    ...
                    $('#discount_max').prop('required', false);
                } else {
                    $('#max-discount-box').show();
                    $('#discount_max').prop('required', true);
                }
            }
            ...
            // ensure correct initial selection for discount type
            checkCouponLimit();
            toggleMaxDiscount();

            // If server returned old discount_amount_type, ensure select shows it
            var oldAmountType = '{{ old('discount_amount_type', 'percentage') }}';
            $couponType.val(oldAmountType).trigger('change');
        });
    </script>
@endpush
