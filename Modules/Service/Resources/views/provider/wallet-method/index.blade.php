        ...
        <div class="card">
            <div class="card-header py-2">
                <div class="search--button-wrapper">
                    <h3 class="card-title">
                        {{ translate('disbursement_methods') }}<span class="badge badge-soft-secondary"  id="countfoods">{{ $provider_withdrawal_methods->total() }}</span>
                    </h3>
                    <form >
                        <!-- Search -->
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table id="datatable"
                        ...
                        <tbody id="set-rows">
                        @foreach($provider_withdrawal_methods as $k=>$e)
                            <tr>
                                <th scope="row">{{$k+$provider_withdrawal_methods->firstItem()}}</th>
                                    ...
                                    <div class="d-flex">
                                        <div>
                                            <label class="toggle-switch toggle-switch-sm mr-2" data-toggle="tooltip" data-placement="top" title="{{ translate('messages.make_default_method') }}" for="statusCheckbox{{$e->id}}">
                                                <input type="checkbox" data-url="{{route('provider.service.wallet-method.default',[$e['id'],$e->is_default?0:1])}}" class="toggle-switch-input redirect-url" id="statusCheckbox{{$e->id}}" {{$e->is_default?'checked':''}}>
                                ...
                                <td>
                                    ...
                                    @if (auth('vendor_employee')->id()  != $e['id'])
                                        <div class="btn--container justify-content-center">
                                            ...
                                            <form action="{{route('provider.service.wallet-method.delete',[$e['id']])}}"
                    ...
                    @if(count($provider_withdrawal_methods) === 0)
            ...
            <div class="card-footer">
                <div class="page-area">
                    <table>
                        <tfoot>
                        {!! $provider_withdrawal_methods->links() !!}
                        </tfoot>
                    </table>
                </div>
            </div>
        ...
        <div class="modal fade" id="balance-modal" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{route('provider.service.wallet-method.store')}}" method="post">
...
@push('script_2')
    <script>
        ...
        $('#withdraw_method').on('change', function () {
            ...
            $.ajax({
                url: "{{route('provider.service.wallet-method.method-list')}}" + "?method_id=" + method_id,
                data: {},
                processData: false,
                contentType: false,
                type: 'get',
