...
<?php
    if (env('APP_MODE') == 'demo') {
        $site_direction = session()->get('site_direction_provider');
    }else{
        $site_direction = session()->has('provider_site_direction')?session()->get('provider_site_direction'):'ltr';
    }
    $country=\App\Models\BusinessSetting::where('key','country')->first();
    $countryCode= strtolower($country?$country->value:'auto');

    $providerId = \App\CentralLogics\Helpers::get_provider_id();
    $provider = Modules\Service\Entities\ProviderManagement\Provider::findOrFail($providerId);
    $moduleType = $provider?->module?->module_type;
?>
{{-- {{ dd($countryCode) }} --}}
...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('Modules/Service/public/assets/css/services.css') }}">
    <link rel="stylesheet" href="{{ asset('Modules/Service/public/assets/css/webfonts/uicons-regular-rounded.css') }}">
    <link rel="stylesheet" href="{{ asset('Modules/Service/public/assets/css/webfonts/uicons-solid-rounded.css') }}">
    @stack('css_or_js')
...
<!-- Builder -->
@include('service::provider.layouts.partials._front-settings')
<!-- End Builder -->

<!-- JS Preview mode only -->
@include('service::provider.layouts.partials._header')

@include("service::provider.layouts.partials._sidebar")
<!-- END ONLY DEV -->
...
<!-- End Content -->
    ...
    <!-- Footer -->
@include('service::provider.layouts.partials._footer')
<!-- End Footer -->
...
@stack('script_2')
<script src="{{ asset('Modules/Service/public/assets/js/services.js') }}"></script>
<audio id="myAudio">
...
"use strict";
    ...
    function startFCM() {
        messaging
            ...
            .then(function(token) {
                @php($provider_id=\App\CentralLogics\Helpers::get_provider_id())
                // Send the token to your backend to subscribe to topic
                subscribeTokenToBackend(token, 'provider_panel_{{$provider_id}}_message');
    ...
    function getUrlParameter(sParam) {
        ...
        function conversationList() {
            $.ajax({
                url: "{{ route('provider.service.message.list') }}",
        ...
        function conversationView() {
            ...
            let user_id = getUrlParameter('user');
            let url= '{{url('/')}}/provider/message/view/'+conversation_id+'/' + user_id;
            $.ajax({
                url: url,
                success: function(data) {
        ...
        @if(\App\CentralLogics\Helpers::provider_employee_module_permission_check('booking') && $order_notification_type == 'manual')
        ...
        $('.check-order').on('click',function (){
            if(order_type){
                location.href = '{{url('/')}}/provider/booking/list?booking_status=pending&service_type=all';
            }
        });
        startFCM();
        conversationList();
                    ...
                    success: function (response) {
                        if (response.length === 0) {
                            ...
                            $('.search-list-item').click(function () {
                                ...
                                $.ajax({
                                    type: 'POST',
                                    url: '{{ route('provider.clicked.route') }}',
                                    data: {
                                        routeName: routeName,
                                        routeUri: routeUri,
