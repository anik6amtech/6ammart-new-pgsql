...
@section('title',translate('Booking Status'))
        ...
        <div class="pb-3 d-flex justify-content-between align-items-center gap-3 flex-wrap">
            ...
            <div class="d-flex flex-wrap flex-xxl-nowrap gap-3">
                <div class="d-flex flex-wrap gap-3">
                    ...
                    @if($provider_can_edit_booking && in_array($booking['booking_status'], ['accepted', 'ongoing']))
                    ...
                    @endif
                    <a href="{{route('provider.service.booking.single_invoice',[$booking->id])}}" class="btn d-flex align-items-center gap-1 btn--primary"  target="_blank">
                                            ...
                                            <div class="timeline-step completed">
                                                ...
                                                <div class="timeline-info">
                                                    ...
                                                    <p class="timeline-text">By
                                                        -
                                                        {{ isset($booking->booking->customer) ? Str::limit($booking->booking->customer->f_name . ' ' . $booking->booking->customer->l_name, 30) : translate('Not_Available') }}
                                                    </p>
                                                </div>
                                            </div>
                                                ...
                                                <div class="timeline-step completed">
                                                    ...
                                                    <div class="timeline-info">
                                                        ...
                                                        <p class="timeline-text">{{ translate('By') }}
                                                            @if (isset($status_history->provider))
                                                                -
                                                                {{ Str::limit($status_history?->provider?->company_name, 30) }}
                                                            @else
                                                                -
                                                                {{ isset($status_history->user) ? Str::limit($status_history->user->f_name . ' ' . $status_history->user->l_name, 30) : '' }}
                                                            @endif
                                                        </p>
                                                    </div>
                                                </div>
                                            ...
                                            @if($serviceAtProviderPlace == 1 && $booking->booking_status != 'pending' && $booking->booking_status != 'completed'  && $booking->booking_status != 'canceled')
                                                ...
                                                @if($serviceAtCustomer == 1 && $serviceAtProvider == 1)
                                                    <a href="#0" type="button"
                                                       ...
                                                       style="--size: 30px"
                                                       data-toggle="modal"
                                                       data-target="#repeatServiceLocationModal--{{ $booking['id'] }}" data-placement="top">
                                                        <i class="tio-edit"></i>
                                                    </a>
                                                @endif
                                            @endif
...
{{--                                            @endif--}}
        ...
        <div class="modal-dialog modal-xl modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body px-4">
                    ...
                    <form action="{{ route('provider.service.booking.service.update_repeat_booking_service') }}" method="POST"
    ...
    @include('service::provider.BookingModule.partials.details._repeat-ongoing-service-location-modal')
    ...
    <script type="text/javascript">
        ...
        $("#booking_status").change(function () {
            ...
            if (parseInt(booking_status) !== 0) {
                var route = '{{route('provider.service.booking.status_update',[$booking->id])}}' + '?booking_status=' + booking_status + '&is_repeat=1';
        ...
        function updateServiceman(servicemanId) {
            ...
            const route = '{{ url('provider/booking/serviceman-update') }}' + '/' + bookingId + '&is_repeat=1';
        ...
        function payment_status_change(paymentStatus) {
            let route = '{{route('provider.service.booking.payment_update',[$booking->id])}}' + '?payment_status=' + paymentStatus + '&is_repeat=1';
        ...
        function service_schedule_update() {
            ...
            var route = '{{route('provider.service.booking.schedule_update',[$booking->id])}}' + '?service_schedule=' + service_schedule + '&is_repeat=1';
                    ...
                    if (componentId === 'booking_status' && updatedValue === 'completed') {
                        if (evidence_status) {
                            ...
                            $('#uploadPictureForm').on('submit', function (e) {
                                ...
                                $.ajax({
                                    url: "{{route('provider.service.booking.evidence_photos_upload',[$booking->id])}}?is_repeat=1",
                                    data: formData,
                                    processData: false,
                                    contentType: false,
                                    type: 'POST',
        ...
        function update_component(componentId, updatedValue) {
            ...
            if (componentId === 'booking_status') {
                ...
                $("#payment_status__span").html(updatedValue === 'paid' ? '{{translate('paid')}}' : '{{translate('unpaid')}}');
                if (updatedValue === 1) {
