<div class="modal fade" id="serviceLocationModal--{{$booking['id']}}" tabindex="-1"
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <form action="{{route('provider.service.booking.change-service-location', [$booking['id']])}}" method="POST">
            @csrf
                            ...
                            <div class="media gap-2 flex-wrap p-3 rounded bg-card mt-3">
                                @if (!$booking?->is_guest && $booking?->customer)
                                    <img width="58" height="58"
                                         ...
                                         src="{{ $booking?->customer?->image_full_url }}"
                                ...
                                @endif
                                    <div class="media-body d-flex justify-content-between">
                                        <div>
                                            <h5 class="c1 mb-3">
                                                @if (!$booking?->is_guest && $booking?->customer)
                                                    <span class="c1 updated_customer_name">{{ Str::limit($booking?->customer->fullName, 30) }}</span>
                                                @else
                                                    <span class="updated_customer_name">{{ Str::limit($booking?->customer->fullName ?? '', 30) }}</span>
                                                @endif
                                            </h5>
                                            <ul class="list-info">
                                                @if ($booking?->customer?->phone)
                                                    <li>
                                                        <i class="tio-call"></i>
                                                        <a href="tel:{{ $booking?->customer?->phone }}" id="updated_customer_phone">{{ $booking?->customer?->phone }}</a>
                                                    </li>
                                                @endif
                                                <li>
                                                    <i class="tio-map"></i>
                                        ...
                                        @if( $booking->service_location == 'customer' )
                                        <div class="btn-group">
                                            <div data-toggle="modal"
                                                 data-target="#customerAddressModal--{{ $booking['id'] }}"
                                                 data-placement="top">
...
{{--                                        </a>--}}
                                        <ul class="list-info">
                                            <li>
                                                <i class="tio-call"></i>
                                                <a href="tel:{{ $booking->provider->company_phone ?? '' }}">{{ $booking->provider->company_phone ?? '' }}</a>
                                            </li>
                                            <li>
                                                <i class="tio-map"></i>
                            ...
                            @if( $booking->service_location == 'customer' )
                            <div class="modal-footer d-flex justify-content-end gap-3 border-0 pt-0 pb-4 m-4">
                                <button type="button" class="btn btn--secondary" data-dismiss="modal" aria-label="Close">{{translate('close')}}</button>
