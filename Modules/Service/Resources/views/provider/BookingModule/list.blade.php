...
@section('content')
    ...
    @if(business_config('provider_suspend_on_exceed_cash_limit', 'service_business_settings')->value && Request()->user('provider')->is_suspended == 1)
        ...
        <div class="alert alert-danger alert-danger__custom">
            <div class="media gap-3 align-items-center">
                <div class="alert-close-btn cursor-pointer">
                    <i class="tio-clear-circle"></i>
                </div>
                <div class="media-body">
                    <h5 class="text-capitalize">{{translate('Attention Please')}}!</h5>
                    <p class="text-dark fs-12 mb-0">
                        {{translate('Your limit to hold cash is exceeded. Your account has been suspended until you pay the due. You will not receive any new booking requests from now')}}
                    </p>
                </div>
            </div>
                       ...
                       <div class="d-flex flex-wrap align-items-center gap-3">
                           <div class="hs-unfold mr-2">
                               ...
                               <div id="usersExportDropdown" class="hs-unfold-content dropdown-unfold dropdown-menu dropdown-menu-sm-right hs-unfold-content-initialized hs-unfold-css-animation animated hs-unfold-hidden" data-hs-target-height="131.516" data-hs-unfold-content="" data-hs-unfold-content-animation-in="slideInUp" data-hs-unfold-content-animation-out="fadeOut" style="animation-duration: 300ms;">
                                   ...
                                   <a id="export-excel" class="dropdown-item" href="{{route('provider.service.booking.download', request()->query())}}">
                        ...
                        <div class="table-responsive">
                            <table id="example" class="table m-0 align-middle align-middle-cus table-custom-space tr-hover">
                                ...
                                <tr>
                                    ...
                                    <td class="text-title">
                                        <a href="{{ route($booking->is_repeated ? 'provider.service.booking.repeat_details' : 'provider.service.booking.details', [$booking->id, 'web_page' => 'details']) }}">
                                        {{ $booking->id }}
                                    ...
                                    <td>
                                        <div class="table-actions d-flex gap-2">
                                            <a href="{{ route($booking->is_repeated ?  'provider.service.booking.repeat_details' : 'provider.service.booking.details', [$booking->id, 'web_page' => 'details']) }}" type="button"
                                               class="btn action-btn btn--primary btn-outline-primary"
                                               style="--size: 30px">
                                                <i class="tio-visible"></i>
                                            </a>
                                            <a href="{{ route($booking->is_repeated ?  'provider.service.booking.full_repeat_invoice' : 'provider.service.booking.invoice', $booking->id) }}" type="button" class="action-btn btn--varify btn-outline-varify"
                                            ...
                                            @if($booking->booking_status == 'pending')
                                                ...
                                                <button
                                                    type="button"
                                                    class="btn action-btn btn-outline-danger form-alert"
                                                    style="--size: 30px"
                                ...
                                @empty
                                    {{-- <tr class="text-center">
                                        <td colspan="10">{{translate('no data available')}}</td>
                                    </tr> --}}
                                @endforelse
                                </tbody>
                            </table>
