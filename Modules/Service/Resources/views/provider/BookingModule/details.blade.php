        ...
        <div class="pb-3 d-flex justify-content-between align-items-center gap-3 flex-wrap">
            ...
            <div class="d-flex flex-wrap flex-xxl-nowrap gap-3">
                <div class="d-flex flex-wrap gap-3">
                    ...
                    @endif
                    <a href="{{route('provider.service.booking.invoice',[$booking->id])}}" class="btn d-flex align-items-center gap-1 btn--primary"  target="_blank">
                                                ...
                                                <td class="text-wrap ps-lg-3">
                                                    <div class="d-flex flex-column">
                                                        @if(isset($detail->service))
                                                            <a href="{{route('provider.service.service.service-details',[$detail->service->id])}}" class="font-semibold text-title fz--14px line--limit-2 lh--12">
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    @else
                                        <div class="d-flex align-items-center gap-2">
                                            <button type="button"
                                               class="btn w-100 btn--cancel booking-action"
                                               data-action="ignore"
                                               data-id="{{ $booking['id'] }}"
                                            ...
                                            <button type="button"
                                               class="btn w-100 btn--accept booking-action"
                                               data-action="accept"
                                               data-id="{{ $booking['id'] }}"
                                               data-message="{{ translate('Are you sure you want to accept this booking?') }}"
                                               data-title="{{ translate('Accept Booking') }}"
                                        ...
                                        <script>
                                            $(document).ready(function() {
                                                $('.booking-action').on('click', function() {
                                                    ...
                                                    Swal.fire({
                                                        ...
                                                        if (result.value) {
                                                            const url = action === 'accept'
                                                                ? '{{ route('provider.service.booking.accept', '') }}/' + bookingId
                                                                : '{{ route('provider.service.booking.ignore', '') }}/' + bookingId;
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        Insert Here')}}</h4>
                                                            <div class="d-flex gap-2 gap-sm-3 align-items-end justify-content-center">
                                                                <input class="otp-field" type="number" pattern="\d*" name="otp_field[]" maxlength="1" autofocus required>
                                                                <input class="otp-field" type="number" pattern="\d*" name="otp_field[]" maxlength="1" required>
                                                                <input class="otp-field" type="number" pattern="\d*" name="otp_field[]" maxlength="1" required>
                                                                <input class="otp-field" type="number" pattern="\d*" name="otp_field[]" maxlength="1" required>
                                                                <input class="otp-field" type="number" pattern="\d*" name="otp_field[]" maxlength="1" required>
                                                                <input class="otp-field" type="number" pattern="\d*" name="otp_field[]" maxlength="1" required>
    ...
    <div class="modal fade" id="servicemanModal" tabindex="-1" aria-labelledby="servicemanModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content modal-content-data1" id="modal-data-info1">
                @include('service::provider.BookingModule.partials.details.serviceman-info-modal-data')
            </div>
        </div>
    </div>

    @include('service::provider.BookingModule.partials.details._service-modal')

    @include('service::provider.BookingModule.partials.details._service-location-modal')
...
@push('script_2')
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            otpFields.forEach((field, index) => {
                ...
                field.addEventListener('input', function(e) {
                    ...
                    // If more than one digit, keep only the first one
    ...
    <script type="text/javascript">
        ...
        $("#evidence-photoss").spartanMultiImagePicker({
            ...
            maxFileSize: '',
            placeholderImage: {
                image: '{{asset('public/assets/admin/img/100x100/2.jpg')}}',
                width: '100%',
            },
        ...
        function resend_otp() {
            ...
            $.ajax({
                url: "{{route('provider.service.booking.otp.resend')}}",
                dataType: 'json',
                data: {
        ...
        $("#service_selector__select").on('change', function () {
            ...
            const route = '{{route('provider.service.booking.service.ajax-get-variant')}}' + '?service_id=' + serviceId + '&zone_id=' + "{{$booking->zone_id}}";
        ...
        $("#add-service").on('click', function () {
            ...
            $.ajax({
                type: 'GET',
                url: "{{route('provider.service.booking.service.ajax-get-service-info')}}" + '?' + query_string,
                data: {},
                processData: false,
                contentType: false,
                beforeSend: function () {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.reassign) {
                        toastr.success('{{ translate('Successfully reassign serviceman') }}');
                    }else {
                        toastr.success('{{ translate('Successfully assign serviceman') }}');
                    }
                    setTimeout(function() {
                        location.reload()
                    }, 600);
