        ...
        <div class="chat-user-info w-100 d-flex align-items-center">
            ...
            <div class="chat-user-info-content">
                <h5 class="mb-0 text-capitalize">
                    {{isset($user['first_name']) ? ($user['first_name'].' '.$user['last_name']) : ($user['f_name'].' '.$user['l_name'])}}</h5>
...
<script>
    ...
    $('#reply-form-vnd').on('submit', function() {
        $('button[type=submit], input[type=submit]').prop('disabled',true);
            ...
            $.post({
                url: '{{ route('provider.service.message.store', ['user_id'=>$user->id,'user_type'=>$user_type]) }}',
