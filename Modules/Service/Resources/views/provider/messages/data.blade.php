...
@php($user= $conv->sender_type == 'provider' ? $conv->receiver :  $conv->sender)
@if (isset($user ) && $conv->last_message)
    ...
    <div
        ...
        onclick="viewConvs('{{route('provider.service.message.view',['conversation_id'=>$conv->id,'user_id'=>$user->id])}}','customer-{{$user->id}}','{{ $conv->id }}','{{ $user->id }}')"
        id="customer-{{$user->id}}">
