                ...
                <div class="col-12">
                    ...
                    <div class="card mb-2">
                        <div class="card-body">
                            ...
                            <form action="{{route('provider.service.report.booking')}}" method="POST">
                                @csrf
                                <div class="row">
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                <div class="card p-20 flex-grow-1">
                                    <div class="d-flex gap-2 justify-content-center align-items-center flex-wrap">
                                        <img width="35" class="avatar" src="{{asset('public/assets/admin')}}/img/not-pen.png" alt="">
                                        <div class="">
                                            <h2 class="text-dark fs-24 m-0">{{$bookingsCount['total_bookings']}}</h2>
                                    ...
                                    <div class="booking-statis-count d-flex flex-wrap justify-content-between gap-2 mt-3">
                                        <div class="d-flex flex-column align-items-center gap-2 fz-12">
                                            <span class="lh-1 font-semibold text-danger">{{$bookingsCount['canceled']}}</span>
                                            <span class="text-opacity10 fs-12 lh--1">{{translate('Canceled')}}</span>
                                        </div>
                                        <div class="d-flex flex-column align-items-center gap-2 fz-12">
                                            <span class="lh-1 font-semibold text-success">{{$bookingsCount['accepted']}}</span>
                                            <span class="text-opacity10 fs-12 lh--1">{{translate('Accepted')}}</span>
                                        </div>
                                        <div class="d-flex flex-column align-items-center gap-2 fz-12">
                                            <span class="lh-1 c1 font-semibold">{{$bookingsCount['ongoing']}}</span>
                                            <span class="text-opacity10 fs-12 lh--1">{{translate('On_Going')}}</span>
                                        </div>
                                        <div class="d-flex flex-column align-items-center gap-2 fz-12">
                                            <span class="lh-1 font-semibold text-success">{{$bookingsCount['completed']}}</span>
                                            <span class="text-opacity10 fs-12 lh--">{{translate('Completed')}}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card p-20 flex-grow-1">
                                    <div class="d-flex gap-2 align-items-center justify-content-center flex-wrap">
                                        <img width="35" class="avatar" src="{{asset('public/assets/admin')}}/img/phone-cart.png" alt="">
                                        <div class="">
                                            <h2 class="text-dark fs-24">{{\App\CentralLogics\Helpers::format_currency($bookingAmount['total_booking_amount'])}}</h2>
                                            <span class="fs-12">{{translate('Total_Booking_Amount')}}</span>
                                        </div>
                                    </div>
                                    <div class="d-flex flex-wrap justify-content-between gap-2 mt-3">
                                        <div class="d-flex flex-column align-items-center gap-2 fz-12">
                                            <span class="text-danger font-semibold">{{\App\CentralLogics\Helpers::format_currency($bookingAmount['total_unpaid_booking_amount'])}}</span>
                                            <span class="text-opacity lh--1 fs-12 gap-1 d-flex align-items-center">{{translate('Due_Amount')}}
                                        ...
                                        <div class="d-flex flex-column align-items-center gap-2 fz-12">
                                            <span class="text-success font-semibold">{{\App\CentralLogics\Helpers::format_currency($bookingAmount['total_paid_booking_amount'])}}</span>
                                            <span class="text-opacity lh--1 fs-12 gap-1 d-flex align-items-center">{{translate('Already_Settled')}}
                                                {{--<i class="material-symbols-outlined" data-bs-toggle="tooltip" data-bs-placement="top"
                    ...
                    <div class="card mt-20">
                        <div class="card-body">
                                ...
                                <div class="d-flex flex-wrap align-items-center gap-3">
                                    ...
                                    {{-- <div class="d-flex flex-wrap align-items-center gap-3">
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu z--2 bg-white dropdown-menu-lg dropdown-menu-right">
                                                <li>
                                                    <a class="dropdown-item" href="{{route('provider.service.report.booking.download').'?'.http_build_query($queryParams)}}">
                                                        {{translate('Excel')}}
                                                    </a>
                                                </li>
                                            </ul>
                                    ...
                                    <div class="hs-unfold mr-2">
                                        ...
                                        <div id="usersExportDropdown"
                                            ...
                                            <a id="export-excel" class="dropdown-item" href="{{route('provider.service.report.booking.download', request()->query())}}">
                            ...
                            <div class="table-responsives pb-0">
                                <table class="table align-middle-cus table-borderless m-0">
                                    <thead class="text-nowrap z--1" data-bg-color="#f3f8f8ff">
                                    <tr>
                                        <th>{{translate('SL')}}</th>
                                    ...
                                    @forelse($filteredBookings as $key=>$booking)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{route('provider.service.booking.details', [$booking->id,'web_page'=>'details'])}}">
                                                    {{$booking['id']}}
                                                </a>
                                            </td>
                                            <td>
                                                @if(isset($booking->customer) && $booking->is_guest == 0)
                                                    ...
                                                    <a class="fz-12 text--clr" href="tel:{{$booking->customer->phone??''}}">{{$booking->customer->phone??''}}</a>
                                            ...
                                            <td>
                                                @if($booking['total_campaign_discount_amount'] > $booking['total_discount_amount'])
                                                    <p class="mb-1">{{\App\CentralLogics\Helpers::format_currency($booking['total_campaign_discount_amount'])}}</p>
    ...
    <script>
        ...
        $(document).ready(function () {
            $('#booking-status').on('change', function() {
                location.href = "{{route('provider.service.report.booking')}}" + "?booking_status=" + this.value;
            });
        });
