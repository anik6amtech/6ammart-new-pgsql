<div class="">
    <div>
        <div class="text-center mb-4 pb-2">
            ...
            @if ($provider_subscription?->package_id ==  $package->id)
        ...
        <div class="change-plan-wrapper align-items-center">
            @if ($provider_business_model == 'commission'  )
            <div class="__plan-item">
                <div class="inner-div">
                    <div class="text-center">
                        <h3 class="title">{{ translate('commission')  }}</h3>
                        <h2 class="price">{{  $admin_commission }} %</h2>
                        {{-- <div class="day-count">{{ $provider_subscription?->package?->validity }} {{ translate('days') }}</div> --}}
                    </div>
                </div>
            </div>
            ...
            @elseif(!in_array($provider_business_model,['commission','none']))

            <div class="__plan-item {{ !$provider_subscription  || $provider_subscription?->package_id ==  $package->id ?  'active' : '' }}">
                <div class="inner-div">
                    <div class="text-center">
                        <h3 class="title">{{ $provider_subscription?->package?->package_name  }}</h3>
                        <h2 class="price">{{  \App\CentralLogics\Helpers::format_currency($provider_subscription?->package?->price) }}</h2>
                        <div class="day-count">{{ $provider_subscription?->package?->validity }} {{ translate('days') }}</div>
                    </div>
                </div>
            </div>
            ...
            @if ($provider_subscription?->package_id !==  $package->id || $provider_business_model == 'commission' )
            ...
            <div class="row g-3">
                ...
                <div class="col-md-{{ $pending_bill > 0 ? '3' :'4' }}">
                    <div class="subscription__plan-info">
                        ...
                        </div> <h4 class="subtitle">  {{  $provider_business_model != 'commission' &&  $provider_subscription?->package_id ==  $package->id ? translate('Renew') :  translate('Migrate_to_new_plan') }}  </h4> </div>
                </div>
            </div>
        </div>
        ...
        @endif
        <form action="{{ route('provider.service.subscriptionackage.packageBuy') }}" method="post">
            @csrf
            @method('POST')
                <input type="hidden" value="{{ $package->id }}" name="package_id">
                <input type="hidden" value="{{ $provider_id }}" name="store_id">
                <input type="hidden" value="{{ $provider_subscription?->package_id ==  $package->id ? 'renew' : 'payment' }}" name="type">
        ...
        <div class="row g-3">
{{--            TODO: add wallet payment option--}}
{{--            @if ($balance > 0)--}}
...
{{--            @endif--}}
        ...
        <div class="btn--container justify-content-end mt-3">
            ...
            @if ( $provider_business_model != 'commission' && $provider_subscription?->package_id ==  $package->id)
