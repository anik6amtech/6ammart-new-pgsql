...
@section('title', translate('messages.Provider Subscription'))
...
<div class="content container-fluid">
            ...
            <div class="flex-grow-1">
                <div class="d-flex align-items-start">
                    ...
                    <div class="w-0 flex-grow pl-2">
                        <h1 class="page-header-title">{{ $provider->name }} {{translate('Business_Plan')}} &nbsp; &nbsp;
    ...
    @if ($provider->store_all_sub_trans_count > 0)
    ...
    <div class="js-nav-scroller hs-nav-scroller-horizontal mb-4">
        <ul class="nav nav-tabs border-0 nav--tabs nav--pills">
            ...
            <li class="nav-item">
                <a href="{{ route('provider.service.subscriptionackage.subscriberTransactions',$provider->id) }}" class="nav-link">{{ translate('Transactions') }}</a>
            </li>
            <li class="nav-item">
                <a href="{{ route('provider.service.subscriptionackage.subscriberWalletTransactions') }}" class="nav-link">{{ translate('Subscription_Refunds') }}</a>
            </li>
        </ul>
    </div>
            ...
            <div class="__bg-F8F9FC-card __plan-details">
                <div class="d-flex flex-wrap flex-md-nowrap justify-content-between __plan-details-top">
                    <div class="w-100">
                        ...
                        <h4 class="title mt-2"><span class="text-180">{{ $provider->commission > 0 ?  $provider->commission :  $admin_commission }} %</span> {{ translate('messages.Commission_per_order') }}</h4>                        <div class="info-text ">
                            {{ translate('provider will pay') }} {{ $provider->commission > 0 ?  $provider->commission :  $admin_commission }}% {{ translate('commission to') }} <strong>{{ $business_name }}</strong> {{ translate('from each order'.'. You will get access of all the features and options  in '.'provider panel , app and interaction with user.') }}
                        </div>
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center py-2">
                <div class="flex-grow-1">
                    <div class="d-flex align-items-start">
                        <img src="{{asset('/public/assets/admin/img/store.png')}}" width="24" alt="img">
                        <div class="w-0 flex-grow pl-2">
                            <h1 class="page-header-title">{{ $provider->name }} {{translate('Subscription')}} &nbsp; &nbsp;
                                @if($provider?->store_sub_update_application?->status == 0)
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                @else
                                    <span class="form-check-label text-dark">
                                        {{ $provider?->store_sub_update_application?->package?->max_order }}
                                        {{ translate('messages.Bookings') }}
                                        <small>(
{{--                                            {{ $provider?->store_sub_update_application?->max_order - nextBookingCount($provider->id) }} {{ translate('left') }}--}}
                                            {{ $provider?->store_sub_update_application?->max_order }} {{ translate('left') }}
                                        )</small>
                                    </span>
                                @endif
                            </div>
                        ...
                        <div>
                            <div class="d-flex align-items-center gap-2">
                                ...
                                @if ( $provider?->store_sub_update_application?->max_product == 'unlimited' )
                                    <span class="form-check-label text-dark">{{ translate('messages.unlimited_service_category') }}</span>
                                @else
                                    <span class="form-check-label text-dark"> {{ $provider?->store_sub_update_application?->max_product }} {{
                                            translate('messages.service_category') }} <small>({{ $provider?->store_sub_update_application?->max_product - $provider?->subscribed_services?->count() }} {{ translate('left') }}) </small></span>
                                @endif
                            </div>
                        </div>
                ...
                {{-- {{ dd($provider?->store_sub_update_application) }} --}}
                <div class="btn--container justify-content-end mt-3">
                    @if ( $provider?->store_sub_update_application?->is_canceled == 0 && $provider?->store_sub_update_application?->status == 1  )
                        <button type="button"  data-url="{{route('provider.service.subscriptionackage.cancelSubscription',$provider?->id)}}" data-message="{{translate('If_you_cancel_the_subscription,_after_')}} {{  Carbon\Carbon::now()->diffInDays($provider?->store_sub_update_application?->expiry_date_parsed->format('Y-m-d'), false) }} {{ translate('days_the_you_will_no_longer_be_able_to_run_the_business_before_subscribe_a_new_plan.') }} "
                ...
                <div class="modal-body px-4 pt-0">
                    <div>
                        ...
                        <div class="plan-slider owl-theme owl-carousel owl-refresh">
                            ...
                            <div class="__plan-item hover {{ $provider->business_model == 'commission'  ? 'active' : ''}} ">
                                <div class="inner-div">
                                    <div class="text-center">
                                    ...
                                    <div class="py-5 mt-4">
                                        <div class="info-text text-center">
                                            {{ translate('provider will pay') }} {{  $provider->commission > 0 ?  $provider->commission :  $admin_commission }}% {{ translate('commission to') }} {{ $business_name }} {{ translate('from each order'.'. You will get access of all the features and options  in '.'provider panel , app and interaction with user.') }}
                                        </div>
                                    </div>
                                    <div class="text-center">
                            ...
                            <div class="__plan-item hover {{ $provider?->store_sub_update_application?->package_id == $package->id  && $provider->business_model != 'commission'  ? 'active' : ''}}">
                                <div class="inner-div">
                                    <div class="text-center">
                                    ...
                                    <ul class="info">
                                        ...
                                        <li>
                                            <i class="tio-checkmark-circle"></i> <span>  {{$provider?->module->module_type == 'rental' && addon_published_status('Rental') ? translate('messages.unlimited_trips') :  translate('messages.Unlimited_Orders') }} </span>
                                        </li>
                                        @else
                                        <li>
                                            <i class="tio-checkmark-circle"></i> <span>  {{ $package->max_order }} {{$provider?->module->module_type == 'rental' && addon_published_status('Rental') ? translate('messages.trips') : translate('messages.Orders') }} </span>
                                        </li>
                                        @endif
                                    ...
                                    <div class="text-center">
                                        ...
                                        @if (  $provider?->business_model != 'commission'  &&  $provider->package_id == $package->id)
                                        <button data-id="{{ $package->id }}"  data-url="{{route('provider.service.subscriptionackage.packageView',[$package->id,$provider->id ])}}"
...
@push('script_2')
    <script>
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.post({
                        ...
                        data: {
                            id: '{{ $provider->id }}',
                            subscription_id:'{{ $provider?->store_sub_update_application?->id }}',
                        },
                        beforeSend: function () {
                            $('#loading').show()
                        },
        ...
        var url = '{{ route('provider.service.subscriptionackage.packageView',[$provider?->store_sub?->package_id,$provider->id ]) }}';
        package_pay(url);
