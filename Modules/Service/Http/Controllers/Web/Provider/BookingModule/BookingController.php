...
namespace Modules\Service\Http\Controllers\Web\Provider\BookingModule;
...
class BookingController extends Controller
{
    ...
    public function __construct(Booking $booking, BookingRepeat $bookingRepeat, BookingRepeatDetails $bookingRepeatDetail, BookingRepeatHistory $bookingRepeatHistory, BookingIgnore $bookingIgnore, BookingStatusHistory $bookingStatusHistory, BookingScheduleHistory $bookingScheduleHistory, SubscribedService $subscribedService, Category $category, Zone $zone, Serviceman $serviceman, Provider $provider, SubscribedService $subscribed_service, User $user, customerAddress $userAddress, BookingDetail $bookingDetail)
    {
    ...
    public function index(Request $request): Renderable
    {
        ...
        $packageSubscriber = StoreSubscription::where('store_id',  $providerId)
            ->where('store_type', 'service_provider')
            ->orderBy('created_at', 'desc')
            ->first();

       $endDate = $packageSubscriber?->expiry_date;
       $canceled = $packageSubscriber?->is_canceled;
...
//       $scheduleBookingEligibility = nextBookingEligibility($providerId);
       $scheduleBookingEligibility = true;
        ...
        $serviceLocations = getProviderSettings(providerId: $providerId, key: 'service_at_provider_location', type: 'provider_config') ?? 0;
        if($serviceLocations == '1') {
            $serviceLocations = ['provider', 'customer'];
        } else {
        ...
        $bookings = $this->booking->with(['customer'])
           ...
           ->when($request['booking_status'] == 'pending', function ($query) use ($packageSubscriber, $canceled, $scheduleBookingEligibility, $isPackageEnded, $request, $maxBookingAmount, $serviceAtProviderPlace, $serviceLocations) {
               ...
               if ($packageSubscriber && $request->user('provider')->business_model != 'commission') {
        ...
        return view('service::provider.BookingModule.list', compact('bookings', 'categories', 'subCategories', 'queryParams', 'filterCounter'));
    }
    ...
    public function details($id, Request $request)
    {
        ...
        if ($booking) {
            if($booking['zone_id'] != $request->user('provider')->zone_id) {
                Toastr::error(translate(DEFAULT_204['message']));
                return redirect(route('provider.service.booking.list', ['booking_status' => 'accepted','service_type'=>'all']));
            }

            $booking->service_address = $booking?->service_address_location
                ? json_decode($booking->service_address_location)
                : $booking->service_address;

            $provider = $request->user('provider');

            if (!is_null($booking['provider_id']) && $booking['provider_id'] != $provider->id) {
                if ($booking['booking_status'] != 'pending') {
                    Toastr::error(translate(ACCESS_DENIED['message']));
                    return redirect()->route('provider.service.booking.list', [
            ...
            if (!$provider->subscribed_services->contains('sub_category_id', $booking->sub_category_id)) {
                Toastr::error(translate(ACCESS_DENIED['message']));
        ...
        if ($request->web_page == 'details') {
            $servicemen = $this->serviceman
                ->withCount('bookings')
                ->where('is_active', 1)
                ->where('service_provider_id', $this->provider->where('id', $request->user('provider')->id)->first()->id)
                ->latest()
                ->get();
            ...
            return view('service::provider.BookingModule.details', compact('booking', 'servicemen', 'webPage', 'customerAddresses', 'category', 'subCategory', 'services', 'customerAddress', 'zones'));
            ...
            $customerAddress = $this->userAddress->find($booking['service_address_id']);
            $zones = Zone::where('status',1)->get();
            return view('service::provider.BookingModule.status', compact('booking', 'servicemen','webPage', 'customerAddresses', 'category', 'subCategory', 'zones', 'services'));
        }
    }
    ...
    public function repeatDetails($id, Request $request): Renderable|RedirectResponse
    {
        ...
        if ($booking->repeat->isNotEmpty()) {
            ...
            $booking['upComing'] = collect($booking['repeats'])->filter(function ($repeat) use ($nextService) {
                ...
                return in_array($repeat['booking_status'], ['accepted', 'pending']) && $repeat['id'] !== $nextService['id'];
            })->values()->toArray();
        ...
        if ($webPage == 'details') {
            return view('service::provider.BookingModule.repeat.details', compact('booking', 'servicemen', 'webPage', 'customerAddress', 'services', 'zones', 'category', 'subCategory'));

        }elseif ($webPage == 'service_log'){
            return view('service::provider.BookingModule.repeat.service-log', compact('booking', 'servicemen', 'webPage', 'customerAddress', 'services', 'zones', 'category', 'subCategory'));
    ...
    public function repeatSingleDetails($id, Request $request): Renderable|RedirectResponse
    {
        ...
        $booking = $this->bookingRepeat->with(['booking', 'detail.service' => function ($query) {
            $query->withTrashed();
        }, 'detail.service', 'scheduleHistories', 'statusHistories', 'booking.service_address', 'booking.customer', 'booking.provider', 'serviceman'])->find($id);
        ...
        $customerAddress = $this->userAddress->find($booking->booking['service_address_id']);
        $zones = Zone::where('status',1)->withoutGlobalScope('translate')->get();

        if ($request->web_page == 'details') {
            return view('service::provider.BookingModule.repeat.single-details', compact('booking', 'servicemen', 'webPage', 'customerAddress', 'services', 'zones', 'category', 'subCategory'));

        }elseif ($request->web_page == 'status') {
            return view('service::provider.BookingModule.repeat.status', compact('booking', 'webPage', 'servicemen', 'customerAddress', 'category', 'subCategory', 'services', 'zones'));
        }
    }
    ...
    public function requestAccept(Request $request, string $bookingId): RedirectResponse
    {
        ...
        if (isset($booking)) {
            ...
            if ($provider?->is_suspended == 1 && business_config('provider_suspend_on_exceed_cash_limit', 'service_business_settings')->value) {
            ...
            $bookingStatusHistory->changed_by = $request->user('provider')->id;
            ...
            DB::transaction(function () use ($bookingStatusHistory, $booking, $request) {
                ...
                if ($booking->repeat->isNotEmpty()){
                    foreach ($booking->repeat as $repeat) {
                        ...
                        $repeatBookingStatusHistory->changed_by = $request->user('provider')->id;
    ...
    public function requestIgnore(Request $request, string $bookingId): RedirectResponse
    {
        ...
        if (isset($booking)) {
            ...
            if ($ignoreList){
                ...
                return redirect(route('provider.service.booking.list', ['booking_status'=>'pending','service_type'=>'all']));
            }
            ...
            if (!empty($booking->provider_id)){
               ...
               $fcmToken = $booking?->customer?->cm_firebase_token ?? null;
            ...
            DB::transaction(function () use ($bookingIgnore, $booking, $repeatBookings, $provider) {

                if($provider?->is_valid_subscription == 1 && $provider?->store_sub?->max_order != "unlimited"){
                    $provider?->store_sub?->increment('max_order' , 1);
                }
    ...
    public function statusUpdate($bookingId, Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            ...
            'otp_field' => ((business_config('otp_for_complete_service', 'service_business_settings'))->value == 1 && $request->booking_status == 'completed') ? 'required' : 'nullable',
        ]);
        ...
        if (isset($booking) && !$request->has('is_repeat')) {
            ...
            if ($request->booking_status == 'completed' && (business_config('otp_for_complete_service'))?->value == 1) {
            ...
            if ($request->booking_status == 'accepted') {
                if ($provider?->is_suspended == 1 && business_config('provider_suspend_on_exceed_cash_limit')->value) {
            ...
            if ($booking->isDirty('booking_status')) {
                if (!is_null($booking->repeat)) {
                    foreach ($booking->repeat->whereIn('booking_status', ['pending', 'accepted', 'ongoing']) as $bookingRepeat) {
                        ...
                        $bookingRepeatStatusHistory->changed_by = $request->user('provider')->id;
    ...
    public function resendOtp(Request $request): JsonResponse
    {
        ...
        if (!isset($booking) && $request->has('is_repeat')) {
            ...
            if ($repeat){
                $fcmToken = $repeat?->booking?->customer?->cm_firebase_token;
        ...
        $fcmToken = $booking?->customer?->cm_firebase_token;
    ...
    public function scheduleUpdate($bookingId, Request $request): JsonResponse
    {
        ...
        if (isset($booking) && !$request->has('is_repeat')) {
            ...
            $bookingScheduleHistory->changed_by = $request->user('provider')->id;
        ...
        if (isset($bookingRepeat)) {
            ...
            $bookingRepeatScheduleHistory->changed_by = $request->user('provider')->id;
    ...
    public function servicemanUpdate($bookingId, Request $request): JsonResponse
    {
        ...
        if ($booking && !$request->has('is_repeat')) {
            ...
            if ($booking->repeat && $booking->repeat->count()) {
                foreach ($booking->repeat->whereIn('booking_status', ['pending', 'accepted', 'ongoing']) as $repeatBooking) {
                    if ($repeatBooking->serviceman_id) {
                        $reassign = true;
                    }else{
                        $reassign = false;
                    }
                    $repeatBooking->serviceman_id = $request->serviceman_id;
                    $repeatBooking->save();
                }
            }
            ...
            $servicemen = $this->serviceman
                ->where('is_active', 1)
                ->where('service_provider_id', $request->user('provider')->id)
            ...
            return response()->json([
                'reassign' => $reassign,
                'view' => view(
                    'service::provider.BookingModule.partials.details.serviceman-info-modal-data',
                    compact('servicemen', 'booking', 'search')
                )->render()
            ]);
        }
    ...
    public function download(Request $request): string|StreamedResponse
    {
        ...
        $maxBookingAmount = business_config('maximum_booking_amount')?->value ?? 0;
    ...
    public function invoice($id, Request $request): Renderable
    {
        $booking = $this->booking->with(['detail.service' => function ($query) {
            $query->withTrashed();
        }, 'customer', 'provider', 'serviceman', 'status_histories'])->find($id);

        $booking->service_address = $booking->service_address_location != null ? json_decode($booking->service_address_location) : $booking->service_address;

        return view('service::provider.BookingModule.invoice', compact('booking'));
    }
    ...
    public function fullBookingInvoice($id): Renderable
    {
        $booking = $this->booking->with(['detail.service' => function ($query) {
            $query->withTrashed();
        }, 'customer', 'provider', 'serviceman', 'status_histories','repeat'])->find($id);

        $booking->service_address = $booking->service_address_location != null ? json_decode($booking->service_address_location) : $booking->service_address;

        return view('service::provider.BookingModule.repeat.invoice', compact('booking'));
    }
    ...
    public function fullBookingSingleInvoice($id): Renderable
    {
        ...
        return view('service::provider.BookingModule.repeat.single-invoice', compact('booking'));
    }
    ...
    public function ajaxGetServiceInfo(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'view' => view('service::provider.BookingModule.partials.details.table-row', compact('data'))->render()
        ]);
    }
    ...
    public function changeServiceLocation($bookingId, Request $request)
    {
        ...
        if ($serviceAtProvider == 0 && $request->service_location == 'provider') {
