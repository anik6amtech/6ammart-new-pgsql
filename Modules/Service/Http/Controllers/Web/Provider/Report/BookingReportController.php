...
class BookingReportController extends Controller
{
    ...
    public function getBookingReport(Request $request): Renderable
    {
        ...
        return view('service::provider.report.booking', compact('zones', 'categories', 'subCategories', 'filteredBookings', 'bookingsCount', 'bookingAmount', 'chartData', 'queryParams'));
    }
    ...
    public function getBookingReportDownload(Request $request): string|StreamedResponse
    {
        ...
        return (new FastExcel($filteredBookings))->download(time().'-booking-report.xlsx', function ($booking) {
            return [
                ...
                'Provider Name' => isset($booking->provider) ? ($booking->provider->first_name . ' ' . $booking->provider->last_name) : '',
                'Provider Phone' => isset($booking->provider) ? ($booking->provider->phone??'') : '',
                'Provider Email' => isset($booking->provider) ? ($booking->provider->email??'') : '',

                'Booking Amount' => Helpers::format_currency($booking['total_booking_amount']),
                'Service Discount' => Helpers::format_currency($booking['total_discount_amount']),
                'Coupon Discount' => Helpers::format_currency($booking['total_coupon_amount']),
                'VAT / Tax' => Helpers::format_currency($booking['total_tax_amount']),
            ];
        });
    }
