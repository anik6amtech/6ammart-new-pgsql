...
namespace Modules\Service\Http\Controllers\Web\Provider\BidModule;
...
class PostController extends Controller
{
    ...
    public function index(Request $request): Renderable|RedirectResponse
    {
        ...
        $query_param = ['type' => $request['type'] ?? 'all', 'search' => $request['search']];
        ...
        $coordinates = $request->user('provider')->coordinates ?? null;
        ...
        $bid_offers_visibility_for_providers = business_config('see_other_providers_offers', 'service_business_settings')?->value;
        $type = $request['type'] ?? 'all';
        $search = $request['search'];

        $this->post->where('is_checked', 0)->update(['is_checked' => 1]);

        return view('service::provider.BidModule.list', compact('posts', 'bid_offers_visibility_for_providers', 'type', 'search'));
    }
    ...
    public function details(Request $request, $post_id): Renderable|RedirectResponse
    {
        ...
        $bid_offers_visibility_for_providers = business_config('see_other_providers_offers', 'service_business_settings')?->value;
        return view('service::provider.BidModule.details', compact('post', 'bid_offers_visibility_for_providers', 'distance'));
    }
    ...
    public function updateStatus(Request $request, $id): Redirector|RedirectResponse|Application
    {
        ...
        Toastr::success(translate(DEFAULT_UPDATE_200['message']));
        return redirect(route('provider.service.booking.post.list', ['type' => 'all']));
    }
