...
class TransactionReportController extends Controller
{
    ...
    public function getTransactionReport(Request $request): Renderable
    {
        ...
        //card data
        $adminAccount = getUserAccount($admin->id, 'admin');
        ...
        $filteredTransactions = $this->transaction
            ->with(['booking', 'from_provider', 'to_provider','repeat'])
            ...
            ->when($request->has('zone_ids'), function ($query) use ($request) {
                ...
                })->orWhereHas('to_provider', function ($query) use ($request) {
                    $query->whereIn('zone_id', $request['zone_ids']);
                })->orWhereHas('from_provider', function ($query) use ($request) {
                    $query->whereIn('zone_id', $request['zone_ids']);
                });
            })
            ->when($request->has('provider_ids'), function ($query) use ($request) {
                $query->whereHas('to_provider', function ($query) use ($request) {
        ...
        return view('service::admin.report.transaction', compact('zones', 'providers', 'filteredTransactions', 'adminAccount', 'commission_earning', 'adminTotalEarning','extra_fee', 'queryParams'));
    }
    ...
    public function downloadTransactionReport(Request $request)
    {
        ...
        return (new FastExcel($filteredTransactions))->download(time() . '-provider-report.xlsx', function ($transaction) {
            return [
                ...
                'Transaction To (full name)' => isset($transaction->to_provider) ? $transaction->to_provider->first_name . ' ' . $transaction->to_provider->last_name : null,
                'Transaction To (phone)' => isset($transaction->to_provider) ? $transaction->to_provider->phone : null,
                'Transaction To (email)' => isset($transaction->to_provider) ? $transaction->to_provider->email : null,
                'Debit' => \App\CentralLogics\Helpers::format_currency($transaction->debit),
                'Credit' => \App\CentralLogics\Helpers::format_currency($transaction->credit),
                'Transactional Balance' => \App\CentralLogics\Helpers::format_currency($transaction->balance),
            ];
        });
    }
