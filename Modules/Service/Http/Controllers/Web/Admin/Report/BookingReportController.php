...
class BookingReportController extends Controller
{
    ...
    public function __construct(Zone $zone, Provider $provider, Category $categories, Service $service, Booking $booking, UserAccount $account, User $user, Transaction $transaction, BookingDetailsAmount $bookingDetailsAmount)
    {
        $this->zone = $zone;
    ...
    public function getBookingReport(Request $request): Renderable
    {
        ...
        $zones = $this->zone->active()->select('id', 'name')->get();
        ...
        return view('service::admin.report.booking', compact('zones', 'providers', 'categories', 'subCategories', 'filtered_bookings', 'bookings_count', 'booking_amount', 'chart_data', 'queryParams'));
    }
    ...
    public function getBookingReportDownload(Request $request): string|StreamedResponse
    {
        ...
        return (new FastExcel($filtered_bookings))->download(time() . '-booking-report.xlsx', function ($booking) {
            return [
                ...
                'Customer Name' => isset($booking->customer) && $booking->is_guest == 0 ? ($booking->customer->f_name . ' ' . $booking->customer->l_name) : $booking?->service_address?->contact_person_name,
                'Customer Phone' => isset($booking->customer) && $booking->is_guest == 0 ? ($booking->customer->phone??'') : $booking?->service_address?->contact_person_number,
                'Customer Email' => isset($booking->customer) && $booking->is_guest == 0 ? ($booking->customer->email??'') : '',
                'Provider Name' => isset($booking->provider) && isset($booking->provider) ? ($booking->provider->first_name . ' ' . $booking->provider->last_name) : '',
                'Provider Phone' => isset($booking->provider) && isset($booking->provider) ? ($booking->provider->phone ?? '') : '',
                'Provider Email' => isset($booking->provider) && isset($booking->provider) ? ($booking->provider->email ?? '') : '',

                'Booking Amount' => \App\CentralLogics\Helpers::format_currency($booking['total_booking_amount']),
                'Service Discount' => \App\CentralLogics\Helpers::format_currency($booking['total_discount_amount']),
                'Coupon Discount' => \App\CentralLogics\Helpers::format_currency($booking['total_coupon_amount']),
                'VAT / Tax' => \App\CentralLogics\Helpers::format_currency($booking['total_tax_amount']),
            ];
        });
    }
