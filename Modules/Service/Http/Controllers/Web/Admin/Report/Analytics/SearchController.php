...
namespace Modules\Service\Http\Controllers\Web\Admin\Report\Analytics;
...
use Modules\Service\Entities\ServiceManagement\SearchedData;
...
class SearchController extends Controller
{
    ...
    public function getKeywordSearchAnalytics(Request $request): Renderable
    {
        ...
        $searches = DB::table('service_searched_data')
            ->select('service_searched_data.attribute_id',
                DB::raw('SUM(service_searched_data.volume) AS total_volume'),
                DB::raw('SUM(service_searched_data.response_data_count) AS total_response_data_count'),
                'service_recent_searches.keyword')
            ->join('service_recent_searches', 'service_searched_data.attribute_id', '=', 'service_recent_searches.id')
            ->when($request->has('search'), function ($query) use ($request) {
                $keys = explode(' ', $request['search']);
                foreach ($keys as $key) {
                    return $query->where('service_recent_searches.keyword', 'like', '%' . $key . '%');
                }
            })
            ->groupBy('service_searched_data.attribute_id', 'service_recent_searches.keyword')
            ->paginate(pagination_limit())->appends($queryParams);

        return view('service::admin.analytics.search.keyword', compact('queryParams', 'graph_data', 'searches', 'search', 'zoneWiseVolumes', 'total'));
    }
    ...
    public function getCustomerSearchAnalytics(Request $request): Renderable
    {
        ...
        foreach ($topCustomer as $item) {
            if ($item->user) {
                $graph_data['top_customers'][] = $item->user->f_name . ' ' . $item->user->l_name;
        ...
        $customers = $this->customer
            ...
            ->leftJoin('service_searched_data', 'users.id', '=', 'service_searched_data.user_id')
            ->groupBy('users.id')
            ->selectRaw('users.id, sum(service_searched_data.response_data_count) as total_response_data_count, sum(service_searched_data.volume) as total_volume')
       ...
       // dd($customers[0]);
        ...
        foreach ($customers as $customer) {
            ...
            $customer->profile_image = User::find($customer->id)?->image_full_url;
            $customer->name = User::find($customer->id)?->f_name . ' ' . User::find($customer->id)?->l_name;
        }

        return view('service::admin.analytics.search.customer', compact('queryParams', 'topCustomer', 'topServices', 'graph_data', 'customers', 'search', 'total'));
    }
