...
class OverviewReportController extends Controller
{
    ...
    public function getBusinessOverviewReport(Request $request)
    {
        ...
        $amounts = $this->bookingDetailsAmount
            ...
            ->when(isset($group_by_deterministic), function ($query) use ($group_by_deterministic) {
                $query->select(
                    ...
                    DB::raw($group_by_deterministic.'(service_booking_details_amounts.created_at) '.$group_by_deterministic)
                );
            })
        ...
        return view('service::admin.report.business.overview', compact('zones', 'categories', 'sub_categories', 'amounts', 'chart_data', 'all_expenses', 'deterministic', 'queryParams','all_expenses_and_earnings'));
    }
