...
namespace Modules\Service\Http\Controllers\Web\Admin\BookingModule;
...
use Modules\Service\Entities\BookingModule\BookingAdditionalInformation;
...
use Modules\Service\Entities\BookingModule\BookingRepeatHistory;
...
use Modules\Service\Exports\BookingExport;
...
class BookingController extends Controller
{
    ...
    private CustomerAddress $userAddress;
    ...
    public function index(Request $request): Renderable
    {
        ...
        return view('service::admin.BookingModule.list', compact('bookings', 'zones', 'categories', 'subCategories', 'queryParams', 'filterCounter'));
    }
    ...
    public function bookingVerificationList(Request $request): Factory|View|Application
    {
        ...
        $maxBookingAmount = (business_config('maximum_booking_amount', 'service_business_settings'))?->value ?? 0;
        ...
        return view('service::admin.BookingModule.verification-list', compact('bookings', 'zones', 'categories', 'subCategories', 'queryParams', 'filterCounter', 'type'));
    }
    ...
    public function bookingOfflinePaymentList(Request $request): Factory|View|Application
    {
        ...
        return view('service::admin.BookingModule.offline-payment-list', compact('bookings', 'zones', 'categories', 'subCategories', 'queryParams', 'filterCounter'));
    }
    ...
    public function details($id, Request $request): Renderable|RedirectResponse
    {
        ...
        if ($request->web_page == 'details') {
            ...
            $servicemen = $this->serviceman
                ->withCount('bookings')
                ->where('is_active', 1)
                ->where('service_provider_id', $booking?->provider_id)
            ...
            $zones = Zone::where('status',1)->withoutGlobalScope('translate')->get();
           ...
           foreach ($allProviders as $provider) {
            ...
            return view('service::admin.BookingModule.details', compact('zoneCenter', 'currentZone', 'centerLat', 'centerLng', 'area', 'booking', 'servicemen', 'webPage', 'customerAddress', 'services', 'zones', 'category', 'subCategory', 'providers', 'sort_by', 'currentlyAssignProvider'));
            ...
            foreach ($allProviders as $provider) {
                ...
                if (in_array($booking->service_location, (array)$serviceLocation)) {
                    $providers[] = $provider;
                }
            }
            ...
            $sort_by = 'default';
            return view('service::admin.BookingModule.status', compact('booking', 'webPage', 'servicemen', 'customerAddress', 'category', 'subCategory', 'services', 'providers', 'zones', 'sort_by', 'currentlyAssignProvider'));
        }
    ...
    public function repeatDetails($id, Request $request): Renderable|RedirectResponse
    {
        ...
        if ($webPage == 'details') {
            return view('service::admin.BookingModule.repeat.details', compact('zoneCenter', 'currentZone', 'centerLat', 'centerLng', 'area', 'booking', 'servicemen', 'webPage', 'customerAddress', 'services', 'zones', 'category', 'subCategory', 'providers', 'sort_by'));

        }elseif ($webPage == 'service_log'){
            return view('service::admin.BookingModule.repeat.service-log', compact('zoneCenter', 'currentZone', 'centerLat', 'centerLng', 'area', 'booking', 'servicemen', 'webPage', 'customerAddress', 'services', 'zones', 'category', 'subCategory', 'providers', 'sort_by'));
    ...
    public function repeatSingleDetails($id, Request $request): Renderable|RedirectResponse
    {
        ...
        $booking = $this->bookingRepeat->with(['booking', 'detail.service' => function ($query) {
            $query->withTrashed();
        }, 'detail.service', 'scheduleHistories.user', 'statusHistories.user', 'booking.service_address', 'booking.customer', 'booking.provider', 'serviceman'])
            ->find($id);
        ...
        if ($request->web_page == 'details') {
            return view('service::admin.BookingModule.repeat.single-details', compact('zoneCenter', 'currentZone', 'centerLat', 'centerLng', 'area', 'booking', 'servicemen', 'webPage', 'customerAddress', 'services', 'zones', 'category', 'subCategory', 'providers', 'sort_by'));

        }elseif ($request->web_page == 'status') {
            return view('service::admin.BookingModule.repeat.status', compact('booking', 'webPage', 'servicemen', 'customerAddress', 'category', 'subCategory', 'services', 'providers', 'zones', 'sort_by'));
        }
    }
    ...
    public function statusUpdate($bookingId, Request $request): JsonResponse
    {
        ...
        if ($booking && !$request->has('is_repeat')) {
            ...
            DB::transaction(function () use ($booking, $status, $request) {
                if ($booking->repeat) {
                    foreach ($booking->repeat->whereIn('booking_status', ['pending', 'accepted', 'ongoing']) as $repeat) {
                        ...
                        $this->logBookingStatusHistory($repeat->id, $status, $request->user('admin')->id, $booking->id);
                    }
                ...
                if($status == 'canceled'){
                    if( $booking?->provider?->is_valid_subscription == 1 && $booking?->provider?->store_sub?->max_order != "unlimited"){
                        $booking?->provider?->store_sub?->increment('max_order' , 1);
                    }
                }

                $booking->save();
                $this->logBookingStatusHistory(null, $status, $request->user('admin')->id, $booking->id);
            });
    ...
    private function updateRepeatBookingStatus($repeatBooking, string $status, Request $request): JsonResponse
    {
        ...
        if ($repeatBooking->isDirty('booking_status')) {
            DB::transaction(function () use ($repeatBooking, $status, $request) {
                ...
                $repeatBooking->save();
                $this->logBookingStatusHistory($repeatBooking->id, $status, $request->user('admin')->id, $repeatBooking->booking_id);
    ...
    public function upComingBookingCancel($bookingId, Request $request): RedirectResponse
    {
        ...
        if (isset($repeatBooking)){
            ...
            $bookingStatusHistory->changed_by = $request->user('admin')->id;
            $bookingStatusHistory->user_type = 'admin';
    ...
    public function verificationUpdate($bookingId, Request $request): JsonResponse
    {
        ...
        if (isset($booking)) {
            ...
            if (isset($booking->provider_id)) {
                $fcmToken = Provider::whereId($booking->provider_id)->first()->fcm_token ?? null;
                $language_key = $this->provider->whereId($booking->provider_id)->first()?->current_language_key;
                if (!is_null($fcmToken) && (!business_config('provider_suspend_on_exceed_cash_limit')->value || $booking?->provider?->is_suspended == 0)) {
                    $title = get_push_notification_message('provider_new_service_request_arrived', $language_key);
                ...
                $providers = Provider::whereIn('id', $provider_ids)->where('zone_id', $booking->zone_id)->get();
                foreach ($providers as $provider) {
                    ...
                    if (!is_null($fcmToken) && (!business_config('provider_suspend_on_exceed_cash_limit')->value || $provider?->is_suspended == 0)) device_notification($fcmToken, $title, null, null, $booking->id, 'booking');
                }
            }
            return response()->json(response_formatter(DEFAULT_STATUS_UPDATE_200), 200);
        }
    ...
    public function verificationStatus($bookingId, Request $request): RedirectResponse
    {
        ...
        if (isset($booking) && $request->status == 'deny') {
            ...
            if (isset($booking->provider_id)) {
                ...
                foreach ($providers as $provider) {
                    $fcmToken = $provider->fcm_token ?? null;
                    $title = get_push_notification_message('provider_booking_accepted', $provider?->current_language_key);
                    if (!is_null($fcmToken) && (!business_config('provider_suspend_on_exceed_cash_limit')->value || $provider?->is_suspended == 0)) device_notification($fcmToken, $title, null, null, $booking->id, 'booking');
                }
            }
    ...
    public function paymentUpdate($bookingId, Request $request): JsonResponse
    {
        ...
        $repeatBooking = $this->bookingRepeat->where('id', $bookingId)->first();
        if (isset($booking) && !$request->has('is_repeat')) {
    ...
    public function scheduleUpdate($bookingId, Request $request): JsonResponse
    {
        ...
        if (isset($booking) && !$request->has('is_repeat')) {
            ...
            $bookingScheduleHistory->changed_by = $request->user('admin')->id;
            $bookingScheduleHistory->user_type = 'admin';
        ...
        if (isset($bookingRepeat)) {
            ...
            $bookingRepeatScheduleHistory->changed_by = $request->user('admin')->id;
            $bookingRepeatScheduleHistory->user_type = 'admin';
    ...
    public function servicemanUpdate(Request $request): JsonResponse
    {
        ...
        if (isset($booking) && !$request->has('is_repeat')) {
            ...
            $search = $request->search;
            $servicemen = $this->serviceman
                ->where('service_provider_id', $bookingRepeat?->provider_id)
                ->when($request->has('search'), function ($query) use ($request) {
                    $keys = explode(' ', $request['search']);
                    return $query->where(function ($query) use ($keys) {
                        foreach ($keys as $key) {
            ...
            return response()->json([
                'view' => view('service::admin.BookingModule.partials.details.serviceman-info-modal-data', compact('servicemen', 'booking', 'search'))->render()
            ]);
        }
    ...
    public function download(Request $request)
    {
        ...
        $providerAssigned = $request->input('provider_assigned');

        $maxBookingAmount = (business_config('maximum_booking_amount'))->value;
        $bookings = $this->booking
            ->with(['customer', 'category', 'subCategory', 'provider', 'serviceman'])
            ->search($search, ['id'])
            ->when($bookingStatus != 'all', function ($query) use ($bookingStatus, $maxBookingAmount, $request) {
                $query->when($bookingStatus == 'pending', function ($query) use ($maxBookingAmount) {
                    $query->adminPendingBookings($maxBookingAmount);
            ...
            ->when($serviceType && $serviceType != 'all', function ($query) use ($serviceType) {
                return $query->ofRepeatBookingStatus($serviceType === 'repeat' ? 1 : 0);
            })
            ->when($providerAssigned == 'assigned', function ($query) {
                $query->where(function ($subQuery) {
                    $subQuery->whereNotNull('provider_id')
                        ->orWhereHas('repeat', function ($q) {
                            $q->whereNotNull('provider_id');
            ...
            ->when($providerAssigned == 'unassigned', function ($query) {
                $query->where(function ($subQuery) {
                    $subQuery->whereNull('provider_id')
        ...
        if ($bookings->isEmpty()) {
            Toastr::error(translate('No data available for export'));
            return back();
        }
    ...
    public function downloadVerificationList(Request $request)
    {
        $request->validate([
            'booking_status' => 'in:' . implode(',', array_column(BOOKING_STATUSES, 'key')) . ',all',
            'type' => 'in:pending,denied'
    ...
    public function invoice($id, Request $request): Renderable
    {
        ...
        return view('service::admin.BookingModule.invoice', compact('booking'));
    }
    ...
    public function fullBookingInvoice($id): Renderable
    {
        ...
        return view('service::admin.BookingModule.repeat.invoice', compact('booking'));
    }
    ...
    public function fullBookingSingleInvoice($id): Renderable
    {
        ...
        return view('service::admin.BookingModule.repeat.single-invoice', compact('booking'));
    }
    ...
    public function customerFullBookingInvoice($id, $lang): Renderable
    {
        ...
        return view('service::admin.BookingModule.fullbooking-invoice', compact('booking'));
    }
    ...
    public function customerFullBookingSingleInvoice($id, $lang): Renderable
    {
        ...
        return view('service::admin.BookingModule.fullbooking-single-invoice', compact('booking'));
    }
    ...
    public function ajaxGetServiceInfo(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'view' => view('service::admin.BookingModule.partials.details.table-row', compact('data'))->render()
        ]);
    }
    ...
    public function verifyOfflinePayment(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'deny_reason' => 'required_if:status,denied|string|max:500',
        ]);
        ...
        try {
            ...
            $booking = $this->booking->with('booking_offline_payments', 'customer')->findOrFail($request->booking_id);
            ...
            if ($offlinePayment) {
                $offlinePayment->payment_status = $isApproved ? 'approved' : 'denied';

                if ($isApproved) {
                    // Process payment for approved status
                    placeBookingTransactionForDigitalPayment($booking);
                    $message = translate('Payment has been approved successfully');

                    // Send approval notification
                    $title = get_push_notification_message('user_offline_payment_approved', $booking->customer?->current_language_key);
                    if ($booking->customer?->cm_firebase_token && $title) {
                        device_notification(
                            $booking->customer->cm_firebase_token,
                            $title,
                            null,
                            null,
                            $booking->id,
                    ...
                    $offlinePayment->denied_note = $request->deny_reason;
                    $message = translate('Payment has been denied');

                    // Send denial notification
                    $title = get_push_notification_message('user_offline_payment_denied', $booking->customer?->current_language_key);
                    if ($booking->customer?->cm_firebase_token && $title) {
                        device_notification(
                            $booking->customer->cm_firebase_token,
                            $title,
                ...
                throw new \Exception(translate('No offline payment record found for this booking'));
            }
            ...
            return response()->json([
                'message' => $message,
                'redirect_url' => route('admin.service.booking.details', ['id' => $booking->id])
            ]);

        } catch (\Exception $e) {
            DB::rollBack();
            \Log::error('Offline payment verification failed: ' . $e->getMessage());

            return response()->json([
                'message' => translate('Failed to process payment verification'),
                'error' => $e->getMessage()
            ], 500);
        }
    }
    ...
    public function reBookingDetails(Request $request, $id)
    {
        ...
        if ($request->web_page == 'details') {
            ...
            return view('service::admin.BookingModule.rebooking-details', compact('zoneCenter', 'currentZone', 'centerLat', 'centerLng', 'area', 'booking', 'servicemen', 'webPage', 'customerAddress', 'services', 'zones', 'category', 'subCategory', 'providers', 'sort_by'));
        } elseif ($request->web_page == 'status') {
            ...
            $providers = $this->provider
                ...
                ->ofApproval(1)->ofStatus(1)->get();
            $sort_by = 'default';
            return view('service::admin.BookingModule.service-log', compact('booking', 'webPage', 'servicemen', 'customerAddress', 'category', 'subCategory', 'services', 'providers', 'zones', 'sort_by'));
        }
    ...
    {
        return view('service::admin.BookingModule.rebooking-ongoing');
    }
    ...
    public function switchPaymentMethod($bookingId, Request $request)
    {
        ...
        Toastr::success(translate(PAYMENT_METHOD_UPDATE_200['message']));
        return back();
    ...
    public function changeServiceLocation($bookingId, Request $request)
    {
        ...
        if ($request->service_location == 'customer') {
            ...
            $updatedAddress = array_merge($existingAddress, [
                "latitude" => $request->latitude ?? $existingAddress['lat'] ?? null,
                "longitude" => $request->longitude ?? $existingAddress['lon'] ?? null,
            ...
            $updateData = [
                ...
                'service_address_location' => json_encode($updatedAddress),
            ];
        ...
        if (isset($user) && $user?->cm_firebase_token && $user?->status) {
            try {
                device_notification($user?->cm_firebase_token, translate('service location updated'), null, null, $booking->id, 'booking', null, null, null, null, $repeatOrRegular);
            }catch (\Exception $exception) {
                //
            }
        }
    ...
    public function availableProviderList(Request $request): JsonResponse
    {
       ...
       foreach ($allProviders as $provider) {
        ...
        return response()->json([
            'view' => view('service::admin.BookingModule.partials.details.provider-info-modal-data', compact('providers', 'booking', 'search', 'sort_by'))->render()
        ]);
    }

    public function reassignProvider(Request $request): JsonResponse
    {
        $changedBy = $request->user('admin')->id;
        ...
        if ($bookingRepeat) {
            ...
            return response()->json([
                'view' => view('service::admin.BookingModule.partials.details.provider-info-modal-data', [
                    'providers' => $providers,
                    'booking' => $bookingRepeat,
                    'search' => $search,
                    'sortBy' => $sort_by,
    ...
    private function sendProviderNotification($providerId, $bookingId, $type): void
    {
        ...
        if ($provider) {
           ...
           $bookingNotificationStatus = business_config('booking_notification')?->value ?? 0;
