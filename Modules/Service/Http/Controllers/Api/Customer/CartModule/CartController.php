...
namespace Modules\Service\Http\Controllers\Api\Customer\CartModule;
...
use Modules\Service\Traits\CartModule\AddedToCartTrait;
...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request): JsonResponse
    {
        ...
        $this->addedToCartUpdate($customerUserId, $request['service_id'], !$this->isCustomerLoggedIn, $this->currentModule()?->id);
        ...
        $variation = $this->variation
            ->where(['zone_id' => $this->zoneId(), 'service_id' => $request['service_id']])
        ...
        if (isset($variation)) {
            ...
            $cart->module_id = $this->currentModule()?->id;
            ...
            return response()->json(response_formatter(SERVICE_ADD_CART_SUCCESS_200), 200);
        }
    ...
    public function list(Request $request): JsonResponse
    {
        ...
        $cart = $this->cart
            ...
            ->with(['customer', 'provider', 'provider.schedules', 'category', 'sub_category', 'service'])
        ...
        foreach ($cart as $cartItem) {
            ...
            if ($cartItem?->provider) {
                ...
                $cartItem->provider->instantBookingEligibility = instantBookingEligibility($providerId);
                $cartItem->provider->repeatBookingEligibility = repeatBookingEligibility($providerId);
    ...
    public function updateQty(Request $request, string $id): JsonResponse
    {
        ...
        $cart = $this->cart
            ...
            ->with(['customer', 'provider', 'category', 'sub_category', 'service'])
            ->where(['customer_id' => $customerUserId])
            ->latest()
        ...
        return response()->json(response_formatter(SERVICE_CART_UPDATE_QUANTITY_SUCCESS_200, ['total_cost' => $totalCost,'referral_amount' => $referralAmount, 'wallet_balance' => with_decimal_point($walletBalance), 'cart' => $cart]), 200);
    }
    ...
    public function updateProvider(Request $request): JsonResponse
    {
        ...
        return response()->json(response_formatter(SERVICE_CART_UPDATE_PROVIDER_SUCCESS_200), 200);
    }
    ...
    public function remove(Request $request, string $id): JsonResponse
    {
        ...
        if (!isset($cart)) {
            return response()->json(response_formatter(SERVICE_CART_FAILED_TO_REMOVE_204), 200);
        }

        $this->cart->where('id', $id)->delete();

        return response()->json(response_formatter(SERVICE_CART_REMOVED_FROM_CART_200), 200);
    }
    ...
    public function rebookAddToCart(Request $request): JsonResponse
    {
        ...
        foreach ($booking->detail as $detail) {
            ...
            if ($serviceData) {
                $this->addedToCartUpdate($customerUserId, $detail->service_id, !$this->isCustomerLoggedIn, $this->currentModule()?->id);

                $variation = $this->variation
                    ->where(['zone_id' => $this->zoneId(), 'service_id' => $detail->service_id])
