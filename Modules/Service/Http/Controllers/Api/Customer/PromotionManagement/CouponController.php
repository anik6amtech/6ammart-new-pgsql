...
class CouponController extends Controller
{
    ...
    public function applyCoupon(Request $request): JsonResponse
    {
        ...
        $checkCoupon = $this->coupon->where(['coupon_code' => $request['coupon_code']])->latest()->first();
        if (!isset($checkCoupon)) {
            return response()->json(response_formatter(COUPON_INVALID), 403);
        }
        ...
        if (!$zoneCheck) return response()->json(response_formatter(COUPON_NOT_VALID_FOR_ZONE), 403);
        ...
        foreach ($couponQuery->with(['discount.discount_types'])->get() as $coupon) {
            ...
            if ($coupon->discount->discount_type == 'category') {
                ...
                if (collect($typeWiseId['category_ids'])->intersect($categoryIds)->isEmpty()) {
                    return response()->json(response_formatter(COUPON_NOT_VALID_FOR_CATEGORY), 403);
                }
            }
            ...
            if ($coupon->discount->discount_type == 'service') {
                ...
                if (collect($typeWiseId['service_ids'])->intersect($serviceIds)->isEmpty()) {
                    return response()->json(response_formatter(COUPON_NOT_VALID_FOR_SERVICE), 403);
                }
            }
        ...
        if ($coupon->coupon_type == 'first_booking') {
            ...
            if ($bookings > 1) {
                return response()->json(response_formatter(COUPON_IS_VALID_FOR_FIRST_TIME), 403);
            }
            ...
            $couponCustomer = $this->couponCustomer
                ...
                ->exists();
            if (!$couponCustomer) {
                return response()->json(response_formatter(COUPON_NOT_VALID_FOR_CART), 403);
            }
        }
        ...
        foreach ($cartItems as $item) {
            if (in_array($item->service_id, $discountedIds) || in_array($item->category_id, $discountedIds)) {
                ...
                if($applicableDiscount > $subtotal){
                    $applicableDiscount = $subtotal;
                }
                $couponDiscountAmount = booking_discount_calculator($coupon->discount, ($subtotal - $applicableDiscount));
                
                // $tax = round(((($cartItem->service_cost - $applicableDiscount - $couponDiscountAmount) * $service['tax']) / 100) * $cartItem['quantity'], 2);
                $tax = round((($subtotal - $applicableDiscount - $couponDiscountAmount) * $service['tax']) / 100, 2);
    ...
    public function removeCoupon(Request $request): JsonResponse
    {
        ...
        foreach ($cartItems as $cart) {
            ...
            $applicableDiscount = ($campaignDiscount >= $basicDiscount) ? $campaignDiscount : $basicDiscount;
            if($applicableDiscount > $cart->service_cost){
                $applicableDiscount = $cart->service_cost;
            }
            $serviceCostWithoutDiscount = $cart->service_cost - $applicableDiscount;
