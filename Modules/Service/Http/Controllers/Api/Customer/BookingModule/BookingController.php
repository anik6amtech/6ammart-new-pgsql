...
namespace Modules\Service\Http\Controllers\Api\Customer\BookingModule;
...
use Modules\Service\Entities\BookingModule\BookingOfflinePayment;
...
class BookingController extends Controller
{
    ...
    protected OfflinePaymentMethod $offlinePayment;
    private BookingRepeat $bookingRepeat;
    private bool $isCustomerLoggedIn;
    private mixed $customerUserId;

    public function __construct(Booking $booking, BookingStatusHistory $bookingStatusHistory, Request $request, OfflinePaymentMethod $offlinePayment, BookingRepeat $bookingRepeat)
    {
    ...
    public function placeRequest(Request $request): JsonResponse
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'guest_id' => $this->isCustomerLoggedIn ? 'nullable' : 'required',
        ...
        if (is_null($request['service_address_id'])) {
            $request['service_address_id'] = $this->add_address(json_decode($request['service_address'],true), null, !$this->isCustomerLoggedIn, 'customer');
        }

        $minimumBookingAmount = (float)(business_config('minimum_booking_amount', 'service_business_settings'))?->value;
        $totalBookingAmount = cart_total($customerUserId, $this->currentModule()->id ?? null) + getServiceFee();
        ...
        if ($request['payment_method'] == 'wallet_payment') {
            if (!isset($request['post_id'])) {
                $response = $this->placeBookingRequest(userId: $customerUserId, request: $request, transactionId: 'wallet_payment', newUserInfo: $newUserInfo, moduleId: $this->currentModule()->id ?? null);
            } else {
                ...
                $response = $this->placeBookingRequestForBidding($customerUserId, $request, 'wallet_payment', $data, $this->currentModule()->id ?? null);
            ...
            if (!isset($request['post_id'])) {
                $response = $this->placeBookingRequest(
                    $customerUserId, $request, 'offline-payment',
                    newUserInfo: $newUserInfo,
                    isGuest: !$this->isCustomerLoggedIn,
                    moduleId: $this->currentModule()->id ?? null
                );
            } else {
                $postBid = PostBid::with(['post'])
                    ->where('post_id', $request['post_id'])
                ...
                $response = $this->placeBookingRequestForBidding($customerUserId, $request, 'offline_payment', $data, $this->currentModule()->id ?? null);
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $bookings = $this->booking
            ->with(['customer', 'repeat', 'provider' => function ($query) {
                $query->select('id', 'first_name', 'last_name', 'phone', 'logo', 'cover_image');
            }])
...
//            }
    ...
    public function singleDetails(Request $request, string $id): JsonResponse
    {
        $booking = $this->bookingRepeat->with([
            'detail.service', 'scheduleHistories', 'statusHistories', 'booking.customer', 'provider', 'serviceman'
        ])->where(['id' => $id])->first();
    ...
    public function track(Request $request, $id): JsonResponse
    {
        ...
        $booking = $this->booking
            ->with(['detail.service', 'schedule_histories.user', 'status_histories.user', 'customer', 'provider', 'zone', 'serviceman'])
            ->where(['id' => $id])
    ...
    public function statusUpdate(Request $request, string $booking_id): JsonResponse
    {
        ...
        if (isset($booking)) {
            ...
            $bookingStatusHistory->user_type = 'customer';
...
//            dd($request['booking_status'] == 'canceled' && $booking->repeat->isNotEmpty());
                if ($request['booking_status'] == 'canceled' && $booking->repeat->isNotEmpty()){
                    foreach ($booking->repeat as $repeat) {
                        ...
                        $repeatBookingStatusHistory->user_type = 'customer';
...
//            });
    ...
    public function storeOfflinePaymentData(Request $request): JsonResponse
    {
        ...
        $fields = array_column($offlinePaymentData->method_informations, 'field_name');
        $customerInformation = (array)json_decode(base64_decode($request['customer_information']), true);
        ...
        if ($existingPayment) {
            ...
            $existingPayment->method_name = OfflinePaymentMethod::find($request['offline_payment_id'])?->method_name;
            $existingPayment->customer_information = $customerInformation;
            $existingPayment->payment_status = 'pending';
            $existingPayment->customer_note = $request['customer_note'];
            ...
            $bookingOfflinePayment->method_name = OfflinePaymentMethod::find($request['offline_payment_id'])?->method_name;
            $bookingOfflinePayment->customer_information = $customerInformation;
            $bookingOfflinePayment->payment_status = 'pending';
            $bookingOfflinePayment->customer_note = $request['customer_note'];
