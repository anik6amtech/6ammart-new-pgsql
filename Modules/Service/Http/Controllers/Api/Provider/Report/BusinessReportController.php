...
class BusinessReportController extends Controller
{
    ...
    public function getBusinessOverviewReport(Request $request)
    {
        ...
        $amounts = $this->booking_details_amount
            ...
            ->join('service_bookings', 'service_booking_details_amounts.booking_id', '=', 'service_bookings.id')
            ->when(isset($groupByDeterministic), function ($query) use ($groupByDeterministic) {
                $query->select(
                    ...
                    DB::raw('sum(service_bookings.extra_fee) as extra_fee'),

                    DB::raw($groupByDeterministic.'(service_booking_details_amounts.created_at) '.$groupByDeterministic)
                );
            })
        ...
        if($deterministic == 'month') {
            ...
            foreach ($months as $month) {
                ...
                foreach ($amounts as $item) {
                    if ($item['month'] == $month) {
                        $chartData['earnings'][] = with_decimal_point($item['provider_earning'] - $item['extra_fee']);
        ...
        if($deterministic == 'month') {
            ...
            foreach ($months as $month) {
                ...
                foreach ($amounts as $key=>$item) {
                    if ($item['month'] == $month) {
                        $chartData['net_profit'][] = with_decimal_point($item['provider_earning'] - ($item['discount_by_provider'] + $item['coupon_discount_by_provider'] + $item['campaign_discount_by_provider'] + $item['extra_fee']));
                        $chartData['total_earning'][] = with_decimal_point($item['provider_earning'] - $item['extra_fee']);
