...
namespace Modules\Service\Http\Controllers\Api\Provider\Report;
...
class BookingReportController extends Controller
{
    ...
    public function getBookingReport(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'zone_ids.*' => $request->has('zone_ids') && !is_null($request['zone_ids'][0]) ? 'integer' : '',
            'category_ids' => $request->has('category_ids') && !is_null($request['category_ids'][0]) ? 'array' : '',
            'category_ids.*' => $request->has('category_ids') && !is_null($request['category_ids'][0]) ? 'integer' : '',
            'sub_category_ids' => $request->has('sub_category_ids') && !is_null($request['sub_category_ids'][0]) ? 'array' : '',
            'sub_category_ids.*' => $request->has('sub_category_ids') && !is_null($request['sub_category_ids'][0]) ? 'integer' : '',
        ...
        $filteredBookings = self::filterQuery($this->booking, $request)
            ->with(['customer:id,f_name,l_name', 'provider'])
        ...
        $bookingAmount['total_paid_booking_amount'] = $bookingsForAmount->where('payment_method', '!=', 'cash_after_service')->where('booking_status', 'completed')->where('is_paid', 1)->sum('total_booking_amount');
