...
class ServiceController extends Controller
{
    ...
    public function review(Request $request, string $service_id): JsonResponse
    {
        ...
        $reviews = $this->review->with(['booking.detail', 'provider', 'customer', 'reviewReply', 'service'])
            ...
            ->where('provider_id', auth('provider')->user()->id)
            ->latest()
            ->paginate($request['limit'], ['*'], 'offset', $request['offset'])->withPath('');
    ...
    public function servicesBySubcategory(Request $request): JsonResponse
    {
        ...
        $services = $this->service->with(['variations', 'category.zonesBasicInfo', 'service_discount', 'category.category_discount'])->latest()
            ->where('module_id', $this->currentModule()?->id)
    ...
    public function reviewReply(Request $request): JsonResponse
    {
        ...
        $providerUserId = auth('provider')->user()->id;
    ...
    public function updateSubscription(Request $request): JsonResponse
    {
        ...
        $packageSubscriber = StoreSubscription::where('store_id',  $request->user('provider')->id)
        ...
        foreach ($request['sub_category_id'] as $id) {
            $subscribedService = $this->subscribed_service::where('sub_category_id', $id)->where('provider_id',  $request->user('provider')->id)->first();
