...
{
    private $bankDetail, $provider, $account, $user, $serviceman, $ignoredPost;
   ...
   protected Transaction $transaction;
    ...
    public function __construct(
        SubscribedService $subscribedService, Provider $provider, Serviceman $serviceman, Booking $booking, Review $review, Post $post, IgnoredPost $ignoredPost,
        Advertisement $advertisement, Transaction $transaction, Notification $notification)
    {
...
//        $this->account = $account;
    ...
    public function dashboard(Request $request, Transaction $transaction, SubscribedService $subscribedService, Serviceman $serviceman): JsonResponse
    {
       ...
       if (in_array('top_cards', $request['sections'])) {
           $account = getUserAccount($request->user('provider')->id, PROVIDER);
           $data[] = ['top_cards' => [
               ...
               'total_subscribed_services' => $this->subscribedService->where('provider_id',  $request->user('provider')->id)
                   ...
                   ->ofStatus(1)
                   ->count(),
               'total_service_man' => $this->serviceman->where(['service_provider_id' =>  $request->user('provider')->id])->count(),
               'total_booking_served' => $request->user('provider')?->bookings('completed')->count()
           ]];
       }

       if (in_array('earning_stats', $request['sections'])) {
           $allTransactions = $transaction->where(['to_user_id' =>  $request->user('provider')->id, 'to_user_type' => PROVIDER])->where('credit', '>', 0)
        ...
        if (in_array('recent_bookings', $request['sections'])) {
            ...
            $recentBookings = $this->booking->with(['detail.service' => function ($query) {
                    ...
                    ->where(function ($query) use ($request) {
                        $query->where('provider_id', $request->user('provider')->id)
                            ->orWhereHas('repeat', function ($subQuery) use ($request) {
                                $subQuery->where('provider_id', $request->user('provider')->id);
                            });
                    })
                ...
                ->latest()->take(5)->get();
            $recentNotBooking = [];
            $recentBookings = $request->user('provider')?->is_suspended == 0 || !business_config('provider_suspend_on_exceed_cash_limit', 'service_business_settings')->value ? $recentBookings : $recentNotBooking;
        ...
        if (in_array('serviceman_list', $request['sections'])) {
            $servicemanList = $this->serviceman->where('is_active', 1)
        ...
        if (in_array('additional_info_count', $request['sections'])) {
            ...
            $advertisementCount = $this->advertisement->with(['attachments'])
                ->where('store_id', $request->user('provider')?->id)
                ->count();
            ...
            $recentNotBooking = [];
            $pendingBookingCount = $request->user('provider')?->is_suspended == 0 || !business_config('provider_suspend_on_exceed_cash_limit', 'service_business_settings')->value ? $pendingBookingCount : $recentNotBooking;
...
//            'email' => 'required',
            ...
            'company_phone' => 'required|unique:service_providers,id,' .  $request->user('provider')->id,
            // 'company_address' => 'required',
            'logo' => 'image|mimes:jpeg,jpg,png,gif|max:10000',
            'cover_photo' => 'image|mimes:jpeg,jpg,png,gif|max:10000',
        ...
        if (count($data) < 1) {
            $validator->getMessageBag()->add('translations', translate('messages.company name and company address in english is required'));
        }
        ...
        if ($request->has('cover_photo')) {
            $provider->cover_image = file_uploader('provider/cover-image/', 'png', $request->file('cover_photo'), $provider->cover_image);
        }
...
//        $provider->email = $request->email;
        ...
        foreach ($data as $key=>$item) {
            Translation::updateOrInsert(
                ['translationable_type' => 'Modules\Service\Entities\ProviderManagement\Provider',
                    'translationable_id' => $provider->id,
                    'locale' => $item['locale'],
                    'key' => $item['key']],
                ['value' => $item['value']]
            );
    ...
    public function updateFcmToken(Request $request): JsonResponse
    {
        ...
        $customer = $this->user::find( $request->user('provider')->id);
    ...
    public function notifications(Request $request): JsonResponse
    {
        ...
        $pushNotification = $this->notification->active()->where('tergat', PROVIDER)
            ->where(function ($q) {
                $q->where('zone_id', auth('provider')->user()->zone_id)
    ...
    public function adjust(Request $request): JsonResponse
    {
        ...
        if ($receivable == $payable){
            ...
            withdrawRequestAcceptForAdjustTransaction( $request->user('provider')->id, $receivable);
