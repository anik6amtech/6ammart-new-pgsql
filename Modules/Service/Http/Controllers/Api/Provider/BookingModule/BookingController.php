...
namespace Modules\Service\Http\Controllers\Api\Provider\BookingModule;
...
class BookingController extends Controller
{
    ...
    public function __construct(Booking $booking, BookingRepeat $bookingRepeat, BookingRepeatDetails $bookingRepeatDetail, BookingStatusHistory $bookingStatusHistory, BookingScheduleHistory $bookingScheduleHistory, SubscribedService $subscribedService, BookingDetail $bookingDetail, BookingIgnore $bookingIgnore, BookingRepeatHistory $bookingRepeatHistory, BookingDetailsAmount $booking_details_amount)
    {
        ...
        try {
            $this->subscribedSubCategories = $subscribedService->where(['provider_id' => auth('provider')->user()->id])
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $bookings_count['pending'] = $this->booking
            ...
            ->where(function ($q) use ($providerId, $provider, $maxBookingAmount) {
                ...
                /* ->where('provider_id', $providerId)
                ->orWhere(function ($query) use ($provider, $maxBookingAmount) {
        ...
        $bookings = $this->booking
            ...
            ->when($request['booking_status'] == 'all', function ($query) use ($serviceAtCustomerPlace, $providerId, $request, $maxBookingAmount, $serviceAtProviderPlace) {
                ...
                $query->where(function ($query) use ($providerId) {
                    ...
                    ->orWhere(function ($query) use ($serviceAtCustomerPlace, $providerId, $request, $maxBookingAmount, $serviceAtProviderPlace) {
            ...
            ->when($request['booking_status'] == 'pending', function ($query) use ($serviceAtCustomerPlace, $providerId, $request, $maxBookingAmount, $serviceAtProviderPlace) {
                $query->providerPendingBookings($request->user('provider'), $maxBookingAmount)
                    ->whereDoesntHave('ignores', function ($query) use ($providerId) {
                        $query->where('provider_id', $providerId);
                    })
        ...
        $bookingSummary = $this->getProviderCompletedBookingSummary($request->user('provider')->id);

        $bookings_count['todays_completed_booking'] = $bookingSummary['todays_completed_booking'];
        $bookings_count['this_week_completed_booking'] = $bookingSummary['this_week_completed_booking'];
        $bookings_count['this_month_completed_booking'] = $bookingSummary['this_month_completed_booking'];
    ...
    public function download(Request $request): JsonResponse
    {
        ...
        $bookings = $this->booking->where('provider_id',  $request->user('provider')->id)
    ...
    public function show(Request $request, string $id): JsonResponse
    {
        ...
        $booking = $this->booking->with([
            'detail.service',  'schedule_histories', 'status_histories', 'customer',
            'provider', 'zone', 'serviceman', 'booking_partial_payments', 'booking_offline_payments',
        ...
        if (isset($booking)) {
           ...
           if ($offlinePayment) {
            ...
            foreach ($booking->schedule_histories as $key => $history) {
                if ($history->user_type === 'customer' && $history->user) {
                    $userInfo= [
                        'full_name' => $history->user->f_name . ' ' . $history->user->l_name,
                        'phone' => $history->user->phone,
                        'email' => $history->user->email,
                        'image' => $history->user->image_full_url,
                    ];
                } elseif ($history->user_type === 'provider' && $history->provider) {
                    $userInfo= [
                        'full_name' => $history->provider->company_name,
                        'phone' => $history->provider->company_phone,
                        'email' => $history->provider->company_email,
                        'image' => $history->provider->logo_full_path,
                    ];
                } elseif ($history->user_type === 'serviceman' && $history->serviceman) {
                    $userInfo= [
                        'full_name' => $history->serviceman->first_name . ' ' . $history->serviceman->last_name,
                        'phone' => $history->serviceman->phone,
                        'email' => $history->serviceman->email,
                        'image' => $history->serviceman->profile_image_full_path,
                    ];
                } else {
                ...
                unset($history->serviceman);
                $userInfo['id'] = $history->changed_by;
                $userInfo['user_type'] = $history->user_type;
                $booking->schedule_histories[$key]['user'] = $userInfo;
            }

            foreach ($booking->status_histories as $key => $history) {
                if ($history->user_type === 'customer' && $history->user) {
                    $userInfo= [
                        'full_name' => $history->user->f_name . ' ' . $history->user->l_name,
                        'phone' => $history->user->phone,
                ...
                $userInfo['id'] = $history->changed_by;
                $userInfo['user_type'] = $history->user_type;
                $booking->status_histories[$key]['user'] = $userInfo;
            }
    ...
    public function singleDetails(Request $request, string $id): JsonResponse
    {
        $booking = $this->bookingRepeat->with([
            'detail.service', 'scheduleHistories', 'statusHistories', 'booking.customer', 'booking.provider', 'serviceman'
        ...
        if (isset($booking)) {
            ...
            $scheduleHistories = $booking?->scheduleHistories;
            foreach ($scheduleHistories ?? [] as $key => $history) {
                if ($history->user_type === 'customer' && $history->user) {
                    $userInfo= [
                        'full_name' => $history->user->f_name . ' ' . $history->user->l_name,
                        'phone' => $history->user->phone,
                ...
                $userInfo['id'] = $history->changed_by;
                $userInfo['user_type'] = $history->user_type;
                $scheduleHistories[$key]['user'] = $userInfo;
            }

            $searchHistories = $booking?->statusHistories;
            foreach ($searchHistories ?? [] as $key => $history) {
                ...
                $userInfo['id'] = $history->changed_by;
                $userInfo['user_type'] = $history->user_type;
                $searchHistories[$key]['user'] = $userInfo;
            }
            $booking->schedule_histories = $scheduleHistories;
            $booking->status_histories = $searchHistories;
    ...
    public function requestAccept(Request $request, string $bookingId): JsonResponse
    {
        ...
        if (isset($booking)) {
            ...
            $booking->booking_type = $request->user('provider')->business_model;
...
//                $repeats['repeats'] = $sortedRepeats->values()->toArray();
                $repeats['repeats'] = $repeats->repeat;

//                $nextService = collect($repeats['repeats'])
                $nextService = $repeats['repeats']
...
//                    $nextService = collect($repeats['repeats'])
    ...
    public function statusUpdate(Request $request, string $bookingId): JsonResponse
    {
        ...
        if (isset($booking)) {
            ...
            if ($request['booking_status'] == 'completed') {
               if (business_config('otp_for_complete_service')?->value == 1 && $booking->booking_otp != $request['booking_otp']) {
    ...
    public function singleBookingStatusUpdate(Request $request, string $repeatId): JsonResponse
    {
        ...
        if (isset($booking)) {
            $evidence_photos = [];
            if ($request['booking_status'] == 'completed') {
                if (business_config('otp_for_complete_service', 'service_business_settings')?->value == 1 && $booking->booking_otp != $request['booking_otp']) {
                    return response()->json(response_formatter(OTP_VERIFICATION_FAIL_403), 200);
                }
...
//                    $nextService = collect($repeats['repeats'])
    ...
    public function notificationSend(Request $request): JsonResponse
    {
        ...
        if (!isset($booking)) {
            if ($bookingRepeat) {
                $fcmToken = $bookingRepeat?->booking?->customer?->cm_firebase_token;
                $title = get_push_notification_message('user_otp', $bookingRepeat?->booking?->customer?->current_language_key) . ' ' . $bookingRepeat->booking_otp;
        ...
        $title = get_push_notification_message('user_otp', $booking?->customer?->current_language_key) . ' ' . $booking->booking_otp;
    ...
    public function updateBooking(Request $request): JsonResponse
    {
        ...
        $providerEditAccess = (boolean)business_config('provider_can_edit_booking', 'service_business_settings')?->value;
    ...
    public function updateBookingRepeat(Request $request): JsonResponse
    {
        ...
        if (empty($request->input('booking_repeat_id'))) {
            ...
            if ($findRepeatId && $findRepeatId->repeat->isNotEmpty()) {
                $sortedRepeats = $findRepeatId->repeat->sortBy('id');
