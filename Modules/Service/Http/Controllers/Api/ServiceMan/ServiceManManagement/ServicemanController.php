...
namespace Modules\Service\Http\Controllers\Api\ServiceMan\ServiceManManagement;
...
class ServicemanController extends Controller
{
   ...
   private Notification $notification;
    ...
    (
        Booking $booking, Serviceman $serviceman, User $employee,
    ...
    public function changePassword(Request $request): JsonResponse
    {
        ...
        $this->employee->where('id', $request->user('serviceman')->id)->update(['password' => bcrypt($request->password)]);
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $serviceman = $this->serviceman->where(['id' => $request->user('serviceman')->id])->with(['provider:id,zone_id'])
        ...
        if ($serviceman) {
            ...
            $serviceman->current_language_key = $current_language;
            $serviceman->save();
    ...
    {
        $serviceman = $this->serviceman::find($request->user('serviceman')->id);
        if (!isset($serviceman)) {
        ...
        if (Serviceman::where('email', $request['email'])->where('id', '!=', $serviceman->id)->exists()) {
        ...
        if ($request->has('profile_image')) {
            $serviceman->profile_image = file_uploader('provider/serviceman/', 'png', $request->file('profile_image'), $serviceman->profile_image);;
        }
    ...
    public function pushNotifications(Request $request): JsonResponse
    {
        ...
        $pushNotification = $this->notification->active()->where('tergat', SERVICEMAN)
            ->where(function ($q) {
                $q->where('zone_id', auth('serviceman')->user()->provider->zone_id)
    ...
    public function update_fcm_token(Request $request)
    {
        ...
        Serviceman::where(['id' => $dm['id']])->update([
    ...
    {
        if (auth('api')->user()){
            $serviceman = $this->employee::find($request->user('serviceman')->id);
