...
namespace Modules\Service\Http\Controllers\Api\ServiceMan\BookingModule;
...
class BookingController extends Controller
{
    ...
    public function statusUpdate(Request $request, string $bookingId): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            ...
            'booking_otp' => ((business_config('otp_for_complete_service', 'service_business_settings'))->value == 1 && $request->booking_status == 'completed') ? 'required' : 'nullable',
        ...
        if (isset($booking)) {
            ...
            $evidencePhotos = [];
            if ($bookingStatus == 'completed' && (business_config('otp_for_complete_service', 'service_business_settings'))?->value == 1) {
                if ($booking->booking_otp != $request['booking_otp']) {
                    return response()->json(response_formatter(OTP_VERIFICATION_FAIL_403), 200);
                }
            }

            if ($bookingStatus == 'completed' && (business_config('service_complete_photo_evidence', 'service_business_settings'))?->value == 1) {
            ...
            $bookingStatusHistory->changed_by = $request->user('serviceman')->id;
            $bookingStatusHistory->booking_status = $request['booking_status'];
            $bookingStatusHistory->user_type = 'serviceman';
...
//                    $nextService = collect($repeats['repeats'])
    ...
    public function bookingDetails(Request $request, string $id): JsonResponse
    {
        $booking = $this->booking->with([
            'detail.service', 'schedule_histories', 'status_histories', 'customer', 'provider', 'zone', 'serviceman', 'booking_partial_payments'
        ...
        if (isset($booking)) {
           ...
           if ($offlinePayment) {
            ...
            return response()->json(response_formatter(DEFAULT_200, [
                'booking' => $booking,
                'provider_serviceman_can_cancel_booking' => (int)provider_config('serviceman_can_cancel_booking', 'provider_config', $request->user('serviceman')->service_provider_id)?->value,
                'provider_serviceman_can_edit_booking' => (int)provider_config('serviceman_can_edit_booking', 'provider_config', $request->user('serviceman')->service_provider_id)?->value,
            ]), 200);
        }
        return response()->json(response_formatter(DEFAULT_204), 200);
    }
    ...
    public function singleBookingDetails(Request $request, string $id): JsonResponse
    {
        $booking = $this->bookingRepeat->with([
            'detail.service', 'scheduleHistories', 'statusHistories', 'booking.service_address', 'booking.customer', 'booking.provider', 'serviceman'
        ])->where(function ($query) use ($request) {
            return $query->where('serviceman_id', $request->user('serviceman')->id)->orWhereNull('provider_id');
        })->where(['id' => $id])->first();
        ...
        if (isset($booking)) {
            return response()->json(response_formatter(DEFAULT_200, [
                'booking' => $booking,
                'provider_serviceman_can_cancel_booking' => (int)business_config('serviceman_can_cancel_booking', 'service_business_settings')?->value,
                'provider_serviceman_can_edit_booking' => (int)business_config('serviceman_edit_booking', 'service_business_settings')?->value,
            ]), 200);
        }
        return response()->json(response_formatter(DEFAULT_204), 200);
    }
    ...
    public function bookingList(Request $request): JsonResponse
    {
        ...
        $bookings = $this->booking->with(['subCategory:id,name', 'repeat'])
            ...
            ->when(($request['booking_status'] == 'all' && $request['type'] == 'history'), function ($query) use ($request) {
                $query->where(function ($subQuery) use ($request) {
                    $subQuery->whereIn('booking_status', ['completed', 'canceled'])
                        ->orWhereHas('repeat', function ($repeatQuery) use ($request) {
                            $repeatQuery->whereIn('booking_status', ['completed', 'canceled']);
                        });
                });
            })
            ->latest()
    ...
    public function updateBooking(Request $request): JsonResponse
    {
        ...
        $editAccess = (boolean)provider_config('serviceman_can_edit_booking', 'provider_config', $booking->provider_id)?->value;
    ...
    public function updateBookingRepeat(Request $request): JsonResponse
    {
        ...
        $editAccess = (int)business_config('serviceman_edit_booking', 'service_business_settings')?->value;
        $request['service_info'] = collect(json_decode($request['service_info'], true));
        $serviceInfoValidated = $request['service_info']?->first()['quantity'] ?? null;
