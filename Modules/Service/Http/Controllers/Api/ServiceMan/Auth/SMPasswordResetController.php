...
class SMPasswordResetController extends Controller
{
...
//
            ...
            $verification_data= DB::table('password_resets')->where('email', $serviceman['email'])->first();
            ...
            DB::table('password_resets')->updateOrInsert(['email' => $serviceman['email']],
            [
                'token' => $token,
                'created_at' => now(),
            ]);
            ...
            try {
                $mailResponse=null;
                if (config('mail.status') && Helpers::getNotificationStatusData('serviceman','verification','mail_status')) {
                    Mail::to($serviceman['email'])->send(new \App\Mail\DmPasswordResetMail($token,$serviceman['first_name']));
                ...
                if (Helpers::getNotificationStatusData('serviceman','verification','sms_status')) {
    ...
    public function verify_token(Request $request)
    {
        ...
        $user = Serviceman::where('phone', $request->phone)->first();
    ...
    public function reset_password_submit(Request $request)
    {
        ...
        if(env('APP_MODE') == 'dev')
        {
            if($request['reset_token'] == "123456")
            {
                DB::table('service_servicemen')->where(['phone' => $request['phone']])->update([
        ...
        if (isset($data)) {
            if ($request['password'] == $request['confirm_password']) {
                DB::table('service_servicemen')->where(['email' => $data->email])->update([
                    'password' => bcrypt($request['confirm_password'])
                ]);
    ...
    public function firebase_auth_verify(Request $request)
    {
        ...
        $user = Serviceman::Where(['phone' => $request->phoneNumber])->first();
