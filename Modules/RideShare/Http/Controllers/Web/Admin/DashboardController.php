...
class DashboardController extends BaseController
{
    ...
    public function __construct(
        ...
        // SupportSavedReplyServiceInterface $supportSavedReplyService,
        // ChannelConversationServiceInterface $channelConversationService,
    ...
    public function dashboardOld(Request $request): Application|Factory|View|RedirectResponse|JsonResponse
    {
    ...
    public function dashboard(Request $request): View|Collection|LengthAwarePaginator|null|callable|RedirectResponse
    {
        $module_type = Config::get('module.current_module_type');
        if ($module_type == 'settings') {
            return redirect()->route('admin.business-settings.business-setup');
        ...
        $transactions = $this->transactionService->getBy(criteria: ['user_id' => \auth('admin')->user()->id], orderBy: ['created_at' => 'desc'])->take(5);
        $recentTrips = $this->tripRequestService->getBy(relations: ['customer', 'vehicle', 'vehicleCategory'], orderBy: ['created_at' => 'desc'], limit: 6, offset: 1);
        ...
        $superAdminAccount = null;
        $customers = $this->customerService->getBy(criteria: ['status' => true])->count();
        $drivers = $this->driverService->getBy(criteria: ['status' => true])->count();
        $totalTrips = $this->tripRequestService->getBy(criteria: ['type' => RIDE_REQUEST])->count();
        $pendingTrips = $this->tripRequestService->getBy(criteria: ['current_status' => PENDING])->count();
        $ongoingTrips = $this->tripRequestService->getBy(whereInCriteria: ['current_status' => [ONGOING, ACCEPTED]])->count();
        $completedTrips = $this->tripRequestService->getBy(criteria: ['current_status' => COMPLETED])->count();
        $cancelledTrips = $this->tripRequestService->getBy(criteria: ['current_status' => CANCELLED])->count();
        ...
        return view("ride-share::admin.dashboard-{$module_type}")->with(compact(
            'zones',
            'transactions',
            'superAdminAccount',
            'customers',
    ...
    public function leaderBoardDriver(Request $request)
    {
        ...
        return response()->json(view('ride-share::admin.partials.dashboard-partials._leader-board-driver', compact('leadDriver'))->render());
    }
    ...
    public function zoneWiseStatistics(Request $request)
    {
        ...
        return response()
            ->json(view('ride-share::admin.partials.dashboard-partials._areawise-statistics', ['trips' => $data['zoneTripsByDate'], 'totalCount' => $data['totalTrips']])
                ->render());
    }
    ...
    public function heatMap(?Request $request)
    {
        ...
        if (array_key_exists('date_range', $request->all()) && $request['date_range']) {
            ...
            $dateRange = $date['start']->format('m/d/Y') . ' - ' . $date['end']->format('m/d/Y');
        } else {
            $withCountQuery = [
                'tripRequest as ride_request' => [
                    ['type', '=', RIDE_REQUEST]
        ...
        return view('ride-share::admin.maps.heat-map', compact('zones', 'totalRideRequests', 'totalParcelRequests', 'markers', 'polygons', 'centerLat', 'centerLng', 'dateRange'));
    }
    ...
    public function heatMapOverview(Request $request)
    {
        ...
        $centerLng = $lngSum / ($totalPoints == 0 ? 1 : $totalPoints);
        return response()
            ->json(view('ride-share::admin.maps.partials._heat-map-overview-map', compact('polygons', 'markers', 'centerLat', 'centerLng'))
                ->render());
    }
    ...
    public function heatMapCompare(Request $request)
    {
        ...
        return view('ride-share::admin.maps.heat-map-compare',
