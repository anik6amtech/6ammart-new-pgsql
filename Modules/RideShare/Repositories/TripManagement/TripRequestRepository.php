...
class TripRequestRepository
{
    ...
    public function getBy(string $column, int|string $value, array $attributes = []): mixed
    {
        ...
        return $this->trip
            ...
            ->when(($attributes['withAvgRelation'] ?? null),
                fn($query) => $query->withAvg($attributes['withAvgRelation'], $attributes['withAvgColumn']))
            ->when(($attributes['withCountRelation'] ?? null),
                fn($query) => $query->withCount($attributes['withCountRelation']))
    ...
    public function store(array $attributes): Model
    {
        try {
            ...
            if(isset($attributes['intermediate_coordinates']) && ($attributes['intermediate_coordinates'] != '')) {
            ...
            //throw $th;
            DB::rollback();
            abort(403, message: $e->getMessage().$e->getLine());
        }
     ...
     * @return Model
     */
    public function update(array $attributes, string $id): array|Model
    {
    ...
    public function updateRelationalTable($attributes): mixed
    {
        ...
        if ($attributes['ride_cancellation_reason'] ?? null) {
            $trip->ride_cancellation_reason = $attributes['ride_cancellation_reason'];
            $trip->save();
        }
