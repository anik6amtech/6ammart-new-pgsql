...
namespace Modules\RideShare\Traits\TripManagement;
...
trait CommonTrait
{
    ...
    public function calculateFinalFare($trip, $fare): array
    {
        ...
        $fee = RideRequestFee::query()->firstWhere('ride_request_id', $trip->id);
        $time = RideRequestTime::query()->firstWhere('ride_request_id', $trip->id);

        $bid_on_fare = FareBidding::where('ride_request_id', $trip->id)->where('is_ignored', 0)->first();
        ...
        $idle_fee_buffer = (double)get_cache('min_idle_fee_time') ?? 0;
        ...
        $delay_fee_buffer = (double)get_cache('min_delay_fee_time') ?? 0;
        ...
        if ($tripRequest['type'] == 'parcel') {
            ...
            $estimated_fare = $tripFare->map(function ($trip) use ($routes, $tripRequest, $area_id, $beforeCreate, $zone) {
                ...
                return [
                    ...
                    'estimated_fare' => round($est_fare, $points),
                    'base_estimated_fare' => round($est_fare - $vat, $points),
                    'vat' => round($discountFareVat, $points),
                    'discount_fare' => round($discountEstFare, $points),
                    'base_discount_fare' => round($discountEstFare - $discountFareVat, $points),
                    'discount_amount' => round(($discount ? $discount['discount_amount'] : 0), $points),
                    'coupon_applicable' => $coupon,
    ...
    public function checkZoneExtraFare($zone)
    {
        $extraFareFee = 0;
        $extraFareReason = "";
        if ($zone->increased_delivery_fee_status) {
            $extraFareFee = $zone->increased_delivery_fee;
            $extraFareReason = $zone->increase_delivery_charge_message;
        }
        if ($extraFareFee > 0) {
            return [
