...
<div class="customer-details-media d-flex">
    <img src="{{$driver?->image_full_url}}"
         alt="" width="40" height="40">
    <div class="customer-details-media-content">
        <div class="d-flex gap-2">
            <h6>
                <a href="{{ route("admin.users.delivery-man.preview", ['id' => $driver?->id, 'tab' => 'ride_list']) }}">
        ...
        <div class="border rounded p-3">
            <ul class="customer-details-media-info-card-body-list">
                ...
                <li>
                    ...
                    <span>:</span>
                    <span class="value title-color">{{$driver?->rider_vehicle?->category?->name}}</span>
                </li>
                <li>
                    <span class="key">{{translate("Vehicle Number")}}</span>
                    <span>:</span>
                    <span class="value title-color">{{$driver?->rider_vehicle?->licence_plate_number}}</span>
                </li>
                <li>
                    <span class="key">{{translate("Vehicle Brand")}}</span>
                    <span>:</span>
                    <span class="value title-color">{{ $driver?->rider_vehicle?->brand?->name }}</span>
                </li>
                <li>
                    <span class="key">{{translate("Vehicle Model")}}</span>
                    <span>:</span>
                    <span class="value title-color">{{$driver?->rider_vehicle?->model?->name}}</span>
                </li>
            </ul>
            @if($trip)
                ...
                @if($trip?->driverSafetyAlertPending)
                    ...
                    <div class="w-100 bg-white py-2">
                        <button type="button"
                                ...
                                data-url="{{ route('admin.ride-share.safety-alert.ajax-mark-as-solved', $trip?->driverSafetyAlertPending->id) }}">
                ...
                <div class="border rounded p-3">
                        ...
                        <li>
                            ...
                            <span>
                                ...
                                <a href="{{route('admin.ride-share.ride.show', ['type' => ALL, 'id' => $otherTrip->id, 'page' => 'summary'])}}"
                                   target="_blank">
                                <img src="{{asset('Modules/RideShare/public/assets/img/ride-share/up-right-arrow-square.svg')}}"
                                     class="svg"
                                     alt="">
                                </a>
                            </span>
                    ...
                    @if($otherTrip?->driverSafetyAlertPending)
                        ...
                        <div class="w-100 bg-white py-2">
                            <button type="button"
                                    class="btn btn-primary fw-semibold text-uppercase w-100 d-flex justify-content-center markAsSolvedBtn"
                                    data-url="{{ route('admin.ride-share.safety-alert.ajax-mark-as-solved', $otherTrip?->driverSafetyAlertPending->id) }}">
                                {{ translate('Mark as Solved') }}
                            </button>
                        </div>
                    @endif
