...
@section('title', translate('messages.fleet_view'))
...
@push('css_or_js')
    ...
    <style>
        ...
        .bg-danger-light {
            --bs-bg-opacity: 0.05;
            background-color: rgba(255, 109, 109, var(--bs-bg-opacity)) !important;
        }
        .fw-bold {
            ...
            <div class="card">
                <div class="card-header d-flex gap-2 flex-column flex-sm-row justify-content-between">
                    <div class="w-100 max-w-299px">
                        <h4>{{translate('User Fleet View')}}</h4>
                        <p>{{translate("Monitor your Riders & customers from here")}}</p>
                    </div>
                    <div class="get-zone-message">
                        @include('ride-share::admin.maps.partials.fleet-map._safety-alert-get-zone-message')
                    </div>
                </div>
                <div class="card-body tab-filter-container">
                    <div class="border-bottom d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
                        ...
                        <ul class="nav d-inline-flex nav--tabs-2 rounded bg-white align-items-center mt-2"
                            id="zone-tab-menu">
                            <li class="nav-item">
                                <a href="{{route('admin.ride-share.fleet-map.fleet-map',['type' => ALL_DRIVER, 'zone_id' => request('zone_id')])}}"
                                   class="nav-link text-capitalize {{request('type') == ALL_DRIVER ? 'active' : ''}}"
                                   data-tab-target="all-driver">{{translate("All Drivers")}}</a>
                            </li>
                            <li class="nav-item">
                                <a href="{{route('admin.ride-share.fleet-map.fleet-map',['type' => DRIVER_ON_TRIP, 'zone_id' => request('zone_id')])}}"
                                   class="nav-link text-capitalize {{request('type') == DRIVER_ON_TRIP ? 'active' : ''}}"
                                   data-tab-target="trip-driver">{{translate("On-trip")}}</a>
                            </li>
                            <li class="nav-item">
                                <a href="{{route('admin.ride-share.fleet-map.fleet-map',['type' => DRIVER_IDLE, 'zone_id' => request('zone_id')])}}"
                            ...
                            <li class="nav-item">
                                <a href="{{route('admin.ride-share.fleet-map.fleet-map',['type' => ALL_CUSTOMER, 'zone_id' => request('zone_id')])}}"
                        ...
                        <form action="{{request()->fullUrl()}}" id="zoneSubmitForm" class="pb-1">
                            <div class="">
                                <select class="js-select-custom js-select-custom-width h-35" name="zone_id" id="selectZone">
                                    @if(count($zones)>0)
                    ...
                    <div class="zone-lists d-flex flex-wrap gap-3">
                                ...
                                <div>
                                    @if(request('type') == ALL_DRIVER)
                                        <div data-tab-type="all-driver">
                                            ...
                                            <form action="" class="search-form search-form_style-two d-flex"
                                                  method="GET">
                                                <div class="input-group search-form__input_group">
                                                <span class="search-form__icon">
                                                    <i class="bi bi-search"></i>
                                                </span>
                                                    <input type="text" class="theme-input-style search-form__input form-control"
                                            ...
                                            <ul class="zone-list">
                                                @include('ride-share::admin.maps.partials.fleet-map._fleet-map-driver-list')
                                            </ul>
                                        </div>
                                    @endif
                                    ...
                                    @if(request('type') == ALL_CUSTOMER)
                                        <div data-tab-type="customer">
                                            ...
                                            <ul class="zone-list">
                                                @include('ride-share::admin.maps.partials.fleet-map._fleet-map-customer-list')
                                            </ul>
                                        </div>
                                    @endif
                        ...
                        <div class="zone-lists__map" id="partialFleetMap">
                            @include('ride-share::admin.maps.partials.fleet-map._fleet-map-view')
                        </div>
                    </div>
                </div>
            </div>
...
@push('script_2')
    <script>
        ...
        <div class="map-clusters-custom-window">
            ...
            <a href="${data.trip}" target="_blank"><p class="m-0">${data.subtitle || ""}</p></a>
        </div>
    `;
            ...
            function fetchModelUpdate() {
                ...
                $.get({
                    url: "{{ route('admin.ride-share.fleet-map.fleet-map-view-using-ajax') }}",
                    dataType: "json",
                    data: requestData,
            ...
            function getListUrl() {
                return @json($type) ===
                'all-customer'
                    ? "{{ route('admin.ride-share.fleet-map.fleet-map-customer-list', $type) }}"
                    : "{{ route('admin.ride-share.fleet-map.fleet-map-driver-list', $type) }}";
            }

            function getSingleViewUrl(id) {
                const baseUrl = @json($type) ===
                'all-customer'
                    ? "{{ route('admin.ride-share.fleet-map.fleet-map-view-single-customer', ':id') }}"
                    : "{{ route('admin.ride-share.fleet-map.fleet-map-view-single-driver', ':id') }}";
            ...
            function getUserDetails(id) {
                const baseUrl =  @json($type) ===
                'all-customer'
                    ? "{{ route('admin.ride-share.fleet-map.fleet-map-customer-details', ':id') }}"
                    : "{{ route('admin.ride-share.fleet-map.fleet-map-driver-details', ':id') }}";
                return baseUrl.replace(':id', id);
    ...
    <script>
       $('.js-select-custom').each(function () {
            ...
            $this.on('select2:open', function () {
                ...
                if ($this.hasClass('js-select-custom-width') && !$container.hasClass('min-w-200')) {
                    $container.addClass('min-w-200');
                }
            });
        });
    </script>
