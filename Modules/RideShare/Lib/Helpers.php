...
if (!function_exists('fileUploader')) {
    function fileUploader(string $dir, string $format, $image = null, $oldImage = null)
    {
        ...
        $imageName = Helpers::upload(dir: $dir, format: $format, image: $image);
...
if (!function_exists('businessConfig')) {
    function businessConfig($key, $settingsType = 'ride_share_business_settings')
    {
        try {
            $config = DataSetting::query()
                ->where('key', $key)
                ->when($settingsType, function ($query) use ($settingsType) {
                    $query->where('type', $settingsType);
                })
                ->first();
...
if (!function_exists('newBusinessConfig')) {
    function newBusinessConfig($key, $settingsType = null)
    {
        $businessSettings = Cache::rememberForever(CACHE_BUSINESS_SETTINGS, function () {
            return DataSetting::all();
        });

        try {
            $config = $businessSettings->where('key', $key)
                ->when($settingsType, function ($query) use ($settingsType) {
                    $query->where('type', $settingsType);
                })
                ->first()?->value;
...
if (!function_exists('responseFormatter')) {
    function responseFormatter($constant, $content = null, $limit = null, $offset = null, $errors = []): array
    {
        ...
        if(gettype($constant) == 'string') {
            $responseConst = [
                'response_code' => $content,
                'message' => translate($constant),
            ];
        } else {
            $responseConst = [
...
if (!function_exists('errorResponseFormatter')) {
    function errorResponseFormatter($message = '', $errors = [], $code = 500): array
    {
        $response = [
            'response_code' => $code,
...
if (!function_exists('get_cache')) {
    ...
    {
        return businessConfig($key)?->value ?? null;
    }
}
...
if (!function_exists('getCustomDateRange')) {
    function getCustomDateRange($dateRange)
    {
        ...
        $startDate = Carbon::make(trim($startDate));
...
if (!function_exists('set_currency_symbol')) {
    ...
    {
        $points = (int)config('round_up_to_digit') ?? 0;
        $position = config('currency_symbol_position') ?? 'left';
        $symbol = \App\CentralLogics\Helpers::currency_symbol() ?? '$';
...
if (!function_exists('getNotification')) {
    function getNotification($key, $lang='en')
    {
        $notification = NotificationMessage::with(['translations'=>function($query)use($lang){
                $query->where('locale', $lang);
            }])->where('module_type','ride-share')->where('key', $key)->where('status', 1)->first();

        if($notification){
            $notification['message'] =  count($notification->translations) > 0 ? $notification->translations[0]->value : $notification['message'];
        }

        $cleanTitle = $notification['key'] ?? ' ';
        // $cleanTitleArray = explode('_', $cleanTitle);
        // unset($cleanTitleArray[0]);
        // $cleanTitle = implode('_', $cleanTitleArray);
        $cleanTitle = preg_replace('/^(customer_|driver_|other_)/', '', $cleanTitle);
...
if (!function_exists('getRoutes')) {
    /*
         function getRoutes(array $originCoordinates, array $destinationCoordinates, array $intermediateCoordinates = [], array $drivingMode = ["DRIVE"])
        {
            ...
            $mapApiKey = Helpers::get_business_data('map_api_key_server');
            ...
            if ($response->successful()) {
                ...
                preg_match('/(\d+)[sS]/', $duration, $matches);
                $durationSec = isset($matches[1]) ? (int)$matches[1] : 0;

                // Convert traffic duration to seconds
                preg_match('/(\d+)[sS]/', $durationInTraffic, $trafficMatches);
    ...
    */
    ...
    function getRoutes(array $originCoordinates, array $destinationCoordinates, array $intermediateCoordinates = [], array $drivingMode = ["DRIVE"])
    {
        ...
        if ($response->successful()) {
            ...
            $responses[0] = [
                'distance' => (double)round(($distance / 1000), 2),
                'distance_text' => number_format(round(($distance / 1000), 2), 2) . ' km',
                'duration' => number_format((($durationSec / 60) / $convert_to_bike), 2) . ' min',
                'duration_sec' => (int)($durationSec / $convert_to_bike),
                'duration_in_traffic' => number_format((($durationInTrafficSec / 60) / $convert_to_bike), 2) . ' min',
                'duration_in_traffic_sec' => (int)($durationInTrafficSec / $convert_to_bike),
...
if (!function_exists('checkPusherConnection')) {
    function checkPusherConnection($event)
    {
        try {
            ...
            $pusher = new Pusher(
                config('broadcasting.connections.pusher.key').'ass',
...
if (!function_exists('textVariableDataFormat')) {
                                    ...
                                    $referralRewardAmount = null, $otp = null, $parcelId = null, $approximateAmount = null)
    {
        ...
        if ($value) {
            ...
            if ($tripId) {
                $data = str_replace("{rideId}", $tripId, $data);
            }
            if ($otp) {
...
if (!function_exists('insertBusinessSetting')) {
    ...
    {
        $data = DataSetting::where('key', $keyName)->where('type', $settingType)->first();
        if (!$data) {
            DataSetting::updateOrCreate(['key' => $keyName, 'type' => $settingType], [
                'value' => $value,
                'created_at' => now(),
                'updated_at' => now(),
            ]);
