...
use Modules\RideShare\Events\RideRequestEvent;
...
use Modules\RideShare\Interface\ZoneManagement\Repository\ZoneRepositoryInterface;
...
class TripRequestService extends BaseService implements TripRequestServiceInterface
{
    ...
    public function __construct(
        ...
        UserLastLocationRepository             $userLastLocation,
    ...
    {
        return $this->tripRequestRepository->get(
            limit: $limit,
            offset: $offset,
    ...
    public function index(array $criteria = [], array $relations = [], array $whereHasRelations = [], array $orderBy = [], int $limit = null, int $offset = null, array $withCountQuery = [], array $appends = [], array $groupBy = [], bool $allModule=false): Collection|LengthAwarePaginator
    {
        ...
        if (array_key_exists('search', $criteria) && $criteria['search'] != '') {
            ...
            $searchData['relations'] = [
                'customer' => ['f_name', 'l_name', 'email', 'phone'],
                'driver' => ['f_name', 'l_name', 'email', 'phone'],
            ];
    ...
    public function getAdminZoneWiseStatistics(array $data)
    {
        ...
        $zones = $this->zoneRepository->getBy(criteria: ['status' => 1]);
    ...
    {
        $criteria = [];
        if (array_key_exists('zone', $data) && $data['zone'] != 'all' && $data['zone'] != null && $data['zone'] != '') {
            $criteria = [
                'zone_id' => $data['zone']
            ];
        }
        ...
        $label = [];
        $points = (int)config('round_up_to_digit') ?? 0;
        switch ($data['date']) {
            case TODAY:
                $label = [
                    '"6:00 am"',
        ...
        return [
            ...
            'totalAdminCommissionAmount' => array_sum($totalAdminCommission),
            'dateType' => translate($data['date'] ?? ALL_TIME)
        ];
    }

    public function getDateZoneWiseEarningStatistics(array $data)
    {
        $zones = $this->zoneRepository->getBy();
    ...
    public function getLeaderBoard(array $data, $limit = null, $offset = null)
    {
        if ($data['user_type'] == CUSTOMER) {
            ...
            $whereHasRelations = ['customer' => []];
        } else {
            $userIdColumn = 'driver_id';
            $whereHasRelations = ['driver' => []];
        }
        $criteria = [];
        ...
        return $this->tripRequestRepository->getLeaderBoard(userType: $userIdColumn, criteria: $criteria, whereBetweenCriteria: $whereBetweenCriteria, whereHasRelations: $whereHasRelations, relations: [$data['user_type']], limit: $limit, offset: $offset);
    }
    ...
    public function makeRideRequest($request, $pickupCoordinates): mixed
    {
        ...
        if (!empty($find_drivers)) {
            $push = getNotification('driver_new_' . $saveTripType);
            $notification = [
            ...
            foreach ($find_drivers as $key => $value) {
                ...
                if ($value->driver?->fcm_token) {
                    ...
                    $notify[$key]['ride_request_id'] = $save_trip->id;
                }
            }
    ...
    {
        $admin_trip_commission = (float)get_cache('ride_commission') ?? 0;
        $vat_percent = (float)get_cache('ride_vat') ?? 1;
        ...
        $push = getNotification('other_coupon_applied');
    ...
    public function handleCancelledTrip($trip, $attributes, $tripId)
    {
        ...
        $push = getNotification('customer_trip_canceled');
        if (!empty($data)) {
            if ($trip->driver_id) {
    ...
    public function handleCompletedTrip($trip, $request, $attributes)
    {
        ...
        $push = getNotification('customer_trip_' . $request->status);
        if (!is_null($trip->driver->fcm_token)) {
            sendDeviceNotification(
                fcm_token: $trip->driver->fcm_token,
                title: translate($push['title']),
    ...
    public function handleDriverStatusUpdate($request, $trip)
    {
        ...
        if ($trip->type == 'parcel') {
            ...
            $action = 'customer_ride_' . $request->status;
        }
    ...
    public function handleRequestActionPushNotification($trip, $user)
    {
        ...
        $this->rejectedDriverRequestService->deleteBy(criteria: ['ride_request_id', $trip->id]);

        return getNotification('customer_driver_on_the_way');
    }
    ...
    {
        $criteria = ['driver_id' => auth('delivery_men')->id(), 'type' => RIDE_REQUEST];
    ...
    public function rideList(array $data = [])
    {
        ...
        $criteria = [
            'driver_id' => auth('delivery_men')->id(),
        ];
    ...
    public function getPendingRide(array $data = []): mixed
    {
        ...
        $relations = [
            ...
            'fare_biddings' => [['driver_id', '=', auth('delivery_men')->id()]],
            'coordinate' => []
    ...
    public function tripOverview(array $data = []): mixed
    {
        ...
        $trips = $this->tripRequestRepository->getBy(criteria: ['driver_id' => auth('delivery_men')->id()], whereBetweenCriteria: $whereBetweenCriteria);
        ...
        if(auth('delivery_men')->check()) {
            $authUserType = DRIVER;
            $authUser = auth('delivery_men')->user();
        } else {
    ...
    public function updateRideStatus(array $data = []): mixed
    {
        ...
        if ($data['status'] == 'completed' || $data['status'] == 'cancelled') {
            ...
            $driverDetails = $this->driverDetailService->findOneBy(criteria: ['user_id' => auth('delivery_men')->id()]);
        ...
        if ($data['cancel_reason'] ?? null) {
            $trip->ride_cancellation_reason = $data['cancel_reason'];
            $trip->save();
        }
        ...
        if ($data['status'] == 'cancelled') {
            ...
            $this->driverLevelUpdateChecker(auth('delivery_men')->user());
    ...
    public function updateTripRequestAction(array $attributes, Model $trip): Model
    {
        ...
        } elseif ($attributes['trip_status'] ?? null)  {
            $trip->tripStatus()->update([$attributes['current_status'] => now()]);
        }
    ...
    public function getLastRideDetails($rider_id) {
        return $this->tripRequestRepository->getLastRideDetails($rider_id);
    }
