...
use Modules\RideShare\Interface\UserManagement\Repository\DriverRepositoryInterface;
use Modules\RideShare\Interface\UserManagement\Repository\UserLevelRepositoryInterface;
...
class DriverService extends BaseService implements DriverServiceInterface
{
    ...
    public function __construct(DriverRepositoryInterface        $userRepository, TripRequestRepositoryInterface $tripRequestRepository,
    ...
    public function show(int|string $id, array $data)
    {
        ...
        $userAccount = getUserAccount($driver->id, DRIVER);

        $commonData = [
            'collectable_amount' => $userAccount->payable_balance > $userAccount->receivable_balance ? ($userAccount->payable_balance - $userAccount->receivable_balance) : 0,
            'pending_withdraw' => $userAccount->pending_balance,
            'already_withdrawn' => $userAccount->total_withdrawn,
            'withdrawable_amount' => $userAccount->receivable_balance > $userAccount->payable_balance ? ($userAccount->receivable_balance - $userAccount->payable_balance) : 0,
            'total_earning' => $userAccount->received_balance,
        ...
        /* if ($tab == 'overview') {
            ...
            if (!empty($driver->rider_vehicle)) {
                //vehicle tab
                $vehicleTripCount = $driver->driverTrips()->where('current_status', 'completed')->where('vehicle_id', $driver->rider_vehicle->id)->count();
                $vehicleRate = ($commonData['completed_trips'] > 0) ? ($vehicleTripCount / $commonData['completed_trips']) * 100 : 0;

                //parcel
                $parcelTripCount = $driver->driverTrips()->where('current_status', 'completed')->where('type', 'parcel')->where('vehicle_id', $driver?->rider_vehicle?->id)->count();
    ...
    private function driverRateInfo($driver)
    {
        ...
        /* $driverTrips = $driver->driverTrips()
            ...
            ->get(); */
        $driverTripCount = $driver->driverTrips()
            ->where('driver_id', $driver->id)
            ->whereIn('current_status', ['completed', 'cancelled'])
            ->where('payment_status', PAID)
            ->count();
        $completedOrderCount = $driver->orders()
            ->where('order_status', 'delivered')
            ->count();

        $driverTotalItemCount = $driverTripCount + $completedOrderCount;

        $driverAvgEarning = ($driver?->userAccount?->received_balance + $driver?->userAccount?->receivable_balance) / ($driverTotalItemCount > 0 ? $driverTotalItemCount : 1);

        //Positive review rate
        $positiveReviewRate = $driver->receivedReviews()
            ->where('ride_type', 'ride_request')
            ->selectRaw('SUM(CASE WHEN rating IN (4,5) THEN 1 ELSE 0 END) / COUNT(*) * 100 AS positive_review_rate')
            ->value('positive_review_rate');

        $positiveDeliveryReviewRate = DMReview::where('delivery_man_id', $driver->id)
            ->selectRaw('SUM(CASE WHEN rating IN (4,5) THEN 1 ELSE 0 END) / COUNT(*) * 100 AS positive_review_rate')
            ->value('positive_review_rate');
        
        if($positiveReviewRate == null && $positiveDeliveryReviewRate == null){
            $positiveReviewRate = 0;
        } elseif ($positiveReviewRate == null && $positiveDeliveryReviewRate != null) {
            $positiveReviewRate = $positiveDeliveryReviewRate;
        } elseif ($positiveReviewRate != null && $positiveDeliveryReviewRate == null) {
            $positiveReviewRate = $positiveReviewRate;
        } else {
            $positiveReviewRate = ($positiveReviewRate + $positiveDeliveryReviewRate) / 2;
        }

        $cancelledOrders = $driver->orders()->where('order_status', 'canceled')->count();
        ...
        $totalItems = $driverTripCount + $cancelledTrips + $cancelledOrders + $completedOrderCount;
        $totalCompletedCount = $completedTrips + $completedOrderCount;
        $totalCancelledCount = $cancelledTrips + $cancelledOrders;
        ...
        $successRate = ($totalItems > 0) ? ($totalCompletedCount / $totalItems) * 100 : 0;
        $cancelRate = ($totalItems > 0) ? ($totalCancelledCount / $totalItems) * 100 : 0;
    ...
    public function export(array $criteria = [], array $relations = [], array $orderBy = [], int $limit = null, int $offset = null, array $withCountQuery = []): Collection|LengthAwarePaginator|\Illuminate\Support\Collection
    {
        return $this->index(criteria: $criteria, relations: $relations, orderBy: $orderBy)->map(function ($item) {
            ...
            $ids = $item->driverTripsStatus->whereNotNull('completed')->pluck('ride_request_id');
