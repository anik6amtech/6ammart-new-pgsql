...
class CustomerService extends BaseService implements CustomerServiceInterface
{
    ...
    public function show(int|string $id, array $data)
    {
        $customer = $this->userRepository
            ->findOneBy(criteria: ['id' => $id, 'user_type' => CUSTOMER], relations: ['customerRides']);
        $otherData = [];
        ...
        if ($tab == 'overview') {
            ...
            $customerRides = $this->tripRequestRepository->getBy(criteria: $tripCriteria, searchCriteria: $searchTripCriteria, orderBy: ['created_at' => 'desc'], limit: paginationLimit(), offset: $data['page'] ?? 1);
            $otherData = [
                'trips' => $customerRides,
                'search' => $data['search'] ?? '',
            ];
    ...
    {
        $totalRequests = $customer->customerRides()->count();
        $totalDigitalPayments = $customer->customerRides()->whereNotIn('payment_method', ['cash', 'wallet'])->count();
        ...
        //total success rate
        $totalSuccessRequest = $customer->customerRides()->where('current_status', 'completed')->count();
        $successPercentage = $totalSuccessRequest == 0 ? 0 : ($totalSuccessRequest / $totalRequests) * 100;

        //total cancel rate
        $totalCancelRequest = $customer->customerRides()->where('current_status', 'cancelled')->count();
        $cancelPercentage = $totalCancelRequest == 0 ? 0 : ($totalCancelRequest / $totalRequests) * 100;

        //trip info of customer details
        $customerLowestFare = $customer->customerRides()->where('current_status', 'completed')->min('paid_fare');
        $customerHighestFare = $customer->customerRides()->where('current_status', 'completed')->max('paid_fare');
    ...
    public function export(array $criteria = [], array $relations = [], array $orderBy = [], int $limit = null, int $offset = null, array $withCountQuery = []): Collection|LengthAwarePaginator|\Illuminate\Support\Collection
    {
        return $this->index(criteria: $criteria, relations: $relations, orderBy: $orderBy)->map(function ($item) {
            ...
            return [
                ...
                'Total Trip' => $item->customerRides->count(),
