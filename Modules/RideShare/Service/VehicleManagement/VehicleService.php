...
class VehicleService extends BaseService implements VehicleServiceInterface
{
    ...
    public function create(array|Request $data): ?Model
    {
        ...
        if (array_key_exists('other_documents', $data)) {
            foreach ($data['other_documents'] as $doc) {
                ...
                $doc_name = fileUploader('vehicle/', $extension, $doc);
                $documents[] = ['img' => $doc_name, 'storage' => Helpers::getDisk()];
            }
        }
        $name = json_decode($data['translations'], true);
        $storeData = [
            'name' => $name[0]['value'],
            ...
            'parcel_weight_capacity' => $data['parcel_weight_capacity'] ?? 0,
        ...
        $vehicle = $this->vehicleRepository->create($storeData);
        foreach ($name as $key=>$item) {
            Translation::updateOrInsert(
                ['translationable_type' => get_class($vehicle),
    ...
    public function updatedByAdmin(int|string $id, array $data = []): ?Model
    {
        ...
        if ($data['other_documents'] ?? null) {
            foreach ($data['other_documents'] as $doc) {
                $extension = $doc->getClientOriginalExtension();
                $documents[] = fileUploader('vehicle/', $extension, $doc);
            }
        }
    ...
    public function updatedByDriver(int|string $id, array|Request $data)
    {
        ...
        $vehicle = $this->vehicleRepository->update($vehicle->id, $updateData);
