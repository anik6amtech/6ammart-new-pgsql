...
use Modules\RideShare\Traits\UnloadedHelpers;
...
class BusinessSettingService extends BaseService implements BusinessSettingServiceInterface
{
        ...
        foreach ($data as $key => $value) {
            if (!is_null($value)) {
                $businessSetting = $this->businessSettingRepository
                    ->findOneBy(criteria: ['key' => $key, 'type' => RIDE_SHARE_BUSINESS_SETTINGS]);
                if ($businessSetting) {
    ...
    {
        if (array_key_exists('update_vehicle_approval', $data)) {
            $value = json_encode($data['update_vehicle_approval']);
            $driverSetting = $this->businessSettingRepository
                ->findOneBy(criteria: ['key' => 'update_vehicle_approval', 'type' => RIDE_SHARE_BUSINESS_SETTINGS]);
            if ($driverSetting) {
                $this->businessSettingRepository
                    ->update(id: $driverSetting->id, data: ['key' => 'update_vehicle_approval', 'type' => RIDE_SHARE_BUSINESS_SETTINGS, 'value' => $value]);
            } else {
                $this->businessSettingRepository
                    ->create(data: ['key' => 'update_vehicle_approval', 'type' => RIDE_SHARE_BUSINESS_SETTINGS, 'value' => $value]);
            }
        }
    ...
    public function storeTripFareSetting(array $data)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if ($driverSetting) {
                $this->businessSettingRepository
                    ->update(id: $driverSetting->id, data: ['key' => $key, 'type' => RIDE_SHARE_BUSINESS_SETTINGS, 'value' => $value]);
            } else {
                $this->businessSettingRepository
                    ->create(data: ['key' => $key, 'type' => RIDE_SHARE_BUSINESS_SETTINGS, 'value' => $value]);
            }
            Cache::put($key, $value);
        }
        $absent_keys = ['ride_otp_confirmation'];
        foreach ($absent_keys as $absent) {
            if (!array_key_exists($absent, $data)) {
                $businessInfo = $this->businessSettingRepository
                    ->findOneBy(criteria: ['key' => $absent, 'type' => RIDE_SHARE_BUSINESS_SETTINGS]);
                if ($businessInfo) {
                    $this->businessSettingRepository->update(id: $businessInfo->id, data: [
                        'key' => $absent,
                        'value' => 0,
    ...
    {
        if(!array_key_exists('safety_feature_after_ride_complete_status', $data)) {
            $data['safety_feature_after_ride_complete_status'] = 0;
        }

        foreach($data as $key => $value) {
            $safetyFeature = $this->businessSettingRepository->findOneBy(criteria: [
                'key' => $key,
                'type' => RIDE_SHARE_BUSINESS_SETTINGS
            ]);
            if ($safetyFeature) {
    ...
    {
        $valueToStore = $data['emergency_'.$data['emergency_number_type'].'_number'];
        ...
        if ($emergencyGovtNumberType) {
            $this->businessSettingRepository->update(
                ...
                data: [
                    'key' => 'emergency_number_type',
                    'type' => RIDE_SHARE_BUSINESS_SETTINGS,
                    'value' => $data['emergency_number_type'],
                ]
            );
        } else {
            $this->businessSettingRepository->create(
        ...
        $emergencyOtherNumbersForCall = json_encode($emergencyOtherNumbersForCall);
