...
<div class="content container-fluid">
    ...
    @php($earning_provider_image=\App\Models\DataSetting::withoutGlobalScope('translate')->where('type','admin_landing_page')->where('key','earning_provider_image')->first())
    ...
    @php($earning_serviceman_image=\App\Models\DataSetting::withoutGlobalScope('translate')->where('type','admin_landing_page')->where('key','earning_serviceman_image')->first())
    ...
    <div class="tab-content">
        <div class="tab-pane fade show active">
            ...
            <form action="{{ route('admin.business-settings.admin-landing-page-settings', 'earning-provider-link') }}" method="POST" enctype="multipart/form-data">
                @php($provider_app_links = \App\Models\DataSetting::where(['key'=>'provider_app_earning_links','type'=>'admin_landing_page'])->first())
                @php($provider_app_links = isset($provider_app_links->value)?json_decode($provider_app_links->value, true):null)

                @csrf
                <h5 class="card-title mb-3">
                    <span class="card-header-icon mr-2"><i class="tio-settings-outlined"></i></span> <span>{{translate('Download_Provider_App_Section')}}</span>
                </h5>
                <div class="card">
                    <div class="card-body">
                        <div class="row g-3">
                            <div class="col-md-7">
                                ...
                                <label class="upload-img-3 m-0 d-block">
                                    <div class="position-relative">
                                    <div class="img">
                                        <img  src="{{\App\CentralLogics\Helpers::get_full_url('earning', $earning_provider_image?->value?? '', $earning_provider_image?->storage[0]?->value ?? 'public','upload_image_4')}}"
                                    ...
                                    <input type="file" name="earning_provider_image"  hidden>
                                    @if (isset($earning_provider_image['value']))
                                            <span id="earning_provider_img" class="remove_image_button remove-image"
                                            data-id="earning_provider_img"
                                            data-title="{{translate('Warning!')}}"
                                            data-text="<p>{{translate('Are_you_sure_you_want_to_remove_this_image_?')}}</p>"
                                                > <i class="tio-clear"></i></span>
                                            @endif
                                ...
                                <div class="__bg-F8F9FC-card">
                                    <div class="form-group mb-md-0">
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            ...
                                            <label  class="toggle-switch toggle-switch-sm m-0">
                                                <input type="checkbox" name="playstore_url_status"
                                                       data-id="play-store-provider-status"
                                                       data-type="toggle"
                                                       data-image-on='{{asset('/public/assets/admin/img/modal')}}/play-store-on.png'
                                                       data-image-off="{{asset('/public/assets/admin/img/modal')}}/play-store-off.png"
                                                       data-title-on="{{translate('Want_to_enable_the_Play_Store_button_for_Provider_App?')}}"
                                                       data-title-off="{{translate('Want_to_disable_the_Play_Store_button_for_Provider_App?')}}"
                                                       data-text-on="<p>{{translate('If_enabled,_the_Provider_app_download_button_will_be_visible_on_the_Landing_page.')}}</p>"
                                                       data-text-off="<p>{{translate('If_disabled,_this_button_will_be_hidden_from_the_landing_page.')}}</p>"
                                                       id="play-store-provider-status" class="status toggle-switch-input dynamic-checkbox-toggle" value="1" {{(isset($provider_app_links) && $provider_app_links['playstore_url_status'])?'checked':''}}>
                                                <span class="toggle-switch-label text mb-0">
                                                    <span class="toggle-switch-indicator"></span>
                                                </span>
                                            </label>
                                        </div>
                                        <input id="playstore_url" type="url" placeholder="{{translate('Ex: https://play.google.com/store/apps')}}" class="form-control h--45px" name="playstore_url" value="{{ $provider_app_links['playstore_url'] ?? ''}}">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            ...
                                            <label class="toggle-switch toggle-switch-sm m-0">
                                                <input type="checkbox" name="apple_store_url_status"
                                                       ...
                                                       data-id="apple-provider-status"
                                                       data-type="toggle"
                                                       data-image-on='{{asset('/public/assets/admin/img/modal')}}/apple-on.png'
                                                       data-image-off="{{asset('/public/assets/admin/img/modal')}}/apple-off.png"
                                                       data-title-on="{{translate('Want_to_enable_the_App_Store_button_for_Provider_App?')}}"
                                                       data-title-off="{{translate('Want_to_disable_the_App_Store_button_for_Provider_App')}}"
                                                       data-text-on="<p>{{translate('If_enabled,_the_Provider_app_download_button_will_be_visible_on_the_Landing_page.')}}</p>"
                                                       data-text-off="<p>{{translate('If_disabled,_this_button_will_be_hidden_from_the_landing_page.')}}</p>"
                                                       id="apple-provider-status" class="status toggle-switch-input dynamic-checkbox-toggle"

                                                       value="1" {{(isset($provider_app_links) && $provider_app_links['apple_store_url_status'])?'checked':''}}>
                                                <span class="toggle-switch-label text mb-0">
                                                    <span class="toggle-switch-indicator"></span>
                                                </span>
                                            </label>
                                        </div>
                                        <input type="url" id="apple_store_url" placeholder="{{translate('Ex: https://www.apple.com/app-store/')}}" class="form-control h--45px" name="apple_store_url" value="{{ $provider_app_links['apple_store_url'] ?? ''}}">
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <form  id="earning_provider_img_form" action="{{ route('admin.remove_image') }}" method="post">
                @csrf
                <input type="hidden" name="id" value="{{  $earning_provider_image?->id}}" >
                {{-- <input type="hidden" name="json" value="1" > --}}
                <input type="hidden" name="model_name" value="DataSetting" >
                <input type="hidden" name="image_path" value="earning" >
                <input type="hidden" name="field_name" value="value" >
            </form>
            <form action="{{ route('admin.business-settings.admin-landing-page-settings', 'earning-serviceman-link') }}" method="POST" enctype="multipart/form-data">
                @csrf
                @php($serviceman_app_links = \App\Models\DataSetting::where(['key'=>'serviceman_app_earning_links','type'=>'admin_landing_page'])->first())
                @php($serviceman_app_links = isset($serviceman_app_links->value)?json_decode($serviceman_app_links->value, true):null)

                <h5 class="card-title mt-3 mb-3">
                    <span class="card-header-icon mr-2"><i class="tio-settings-outlined"></i></span> <span>{{translate('Download_Serviceman_App_Section')}}</span>
                </h5>
                <div class="card">
                    <div class="card-body">
                            ...
                            <div class="col-md-7">
                                ...
                                <label class="upload-img-3 m-0 d-block">
                                    ...
                                    <div class="img">
                                        ...
                                        <img src="{{\App\CentralLogics\Helpers::get_full_url('earning', $earning_serviceman_image?->value?? '', $earning_serviceman_image?->storage[0]?->value ?? 'public','upload_image_4')}}"

                                        data-onerror-image="{{asset('/public/assets/admin/img/upload-4.png')}}" class="vertical-img mw-100 vertical onerror-image" alt="">
                                    </div>
                                        <input type="file" name="earning_serviceman_image"  hidden>
                                            @if (isset($earning_serviceman_image['value']))
                                            <span id="earning_serviceman_img" class="remove_image_button  remove-image"
                                                  data-id="earning_serviceman_img"
                                                  data-title="{{translate('Warning!')}}"
                                                  data-text="<p>{{translate('Are_you_sure_you_want_to_remove_this_image_?')}}</p>"
                                                > <i class="tio-clear"></i></span>
                                            @endif
                                ...
                                <div class="__bg-F8F9FC-card">
                                    <div class="form-group mb-md-0">
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            ...
                                            <label class="toggle-switch toggle-switch-sm m-0">
                                                <input type="checkbox" name="playstore_url_status"
                                                       data-id="play-store-serviceman-status"
                                                       data-type="toggle"
                                                       data-image-on="{{ asset('/public/assets/admin/img/modal/play-store-on.png') }}"
                                                       data-image-off="{{ asset('/public/assets/admin/img/modal/play-store-off.png') }}"
                                                       data-title-on="{{ translate('Want_to_enable_the_Play_Store_button_for_Serviceman_App?') }}"
                                                       data-title-off="{{ translate('Want_to_disable_the_Play_Store_button_for_Serviceman_App?') }}"
                                                       data-text-on="<p>{{ translate('If_enabled,_the_Serviceman_app_download_button_will_be_visible_on_the_Landing_page.') }}</p>"
                                                       data-text-off="<p>{{ translate('If_disabled,_this_button_will_be_hidden_from_the_landing_page.') }}</p>"
                                                       id="play-store-serviceman-status"
                                                       class="status toggle-switch-input dynamic-checkbox-toggle"


                                                       value="1" {{(isset($serviceman_app_links) && $serviceman_app_links['playstore_url_status'])?'checked':''}}>
                                                <span class="toggle-switch-label text mb-0">
                                                    <span class="toggle-switch-indicator"></span>
                                                </span>
                                            </label>
                                        </div>
                                        <input id="playstore_url_dm" type="url" placeholder="{{translate('Ex: https://play.google.com/store/apps')}}" class="form-control h--45px" name="playstore_url" value="{{ $serviceman_app_links['playstore_url'] ?? ''}}">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                ...
                                <div class="__bg-F8F9FC-card">
                                    <div class="form-group mb-md-0">
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            ...
                                            <label class="toggle-switch toggle-switch-sm m-0">
                                                <input type="checkbox" name="apple_store_url_status"
                                                       data-id="apple-serviceman-status"
                                                       data-type="toggle"
                                                       data-image-on="{{ asset('/public/assets/admin/img/modal/apple-on.png') }}"
                                                       data-image-off="{{ asset('/public/assets/admin/img/modal/apple-off.png') }}"
                                                       data-title-on="{{ translate('Want_to_enable_the_App_Store_button_for_Serviceman_App?') }}"
                                                       data-title-off="{{ translate('Want_to_disable_the_App_Store_button_for_Serviceman_App?') }}"
                                                       data-text-on="<p>{{ translate('If_enabled,_the_Serviceman_app_download_button_will_be_visible_on_the_Landing_page.') }}</p>"
                                                       data-text-off="<p>{{ translate('If_disabled,_this_button_will_be_hidden_from_the_landing_page.') }}</p>"
                                                       id="apple-serviceman-status"
                                                       class="status toggle-switch-input dynamic-checkbox-toggle"


                                                       value="1" {{(isset($serviceman_app_links) && $serviceman_app_links['apple_store_url_status'])?'checked':''}}>
                                                <span class="toggle-switch-label text mb-0">
                                                    <span class="toggle-switch-indicator"></span>
                                                </span>
                                            </label>
                                        </div>
                                        <input id="apple_store_url_dm" type="url" placeholder="{{translate('Ex: https://www.apple.com/app-store/')}}" class="form-control h--45px" name="apple_store_url" value="{{ $serviceman_app_links['apple_store_url']?? ''}}">
                                    </div>
                                </div>
                            </div>
                        </div>
             ...
             <form  id="earning_serviceman_img_form" action="{{ route('admin.remove_image') }}" method="post">
                @csrf
                <input type="hidden" name="id" value="{{  $earning_serviceman_image?->id}}" >
                <input type="hidden" name="model_name" value="DataSetting" >
                <input type="hidden" name="image_path" value="earning" >
                <input type="hidden" name="field_name" value="value" >
            </form>
