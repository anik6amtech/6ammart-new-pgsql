    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div>
                ...
                <span>
                    {{translate('messages.rider_reviews')}}
            ...
            <div class="">
                @include('admin-views.delivery-man.partials._review_tab_menu')
            </div>
                        ...
                        <div class="search--button-wrapper justify-content-end">
                            <div class="col-sm-auto min--240">
                                <select name="deliveryman_id"
                                        ...
                                        data-url="{{ url()->full() }}?tab=rides">
                                    <option value="all">{{ translate('messages.All_Riders') }}</option>
                            ...
                            @if(request()->get('search'))
                                <button type="reset" class="btn btn--primary ml-2 location-reload-to-base"
                                        data-url="{{url()->full()}}?tab=rides">{{translate('messages.reset')}}</button>
                            @endif
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="d-block font-size-sm text-body">
                                            @if($review->receivedByDeliveryMan)
                                            <a href="{{route('admin.users.delivery-man.preview',[$review['received_by'], 'tab' => 'ride_list'])}}" target="_blank"
                                               class="media gap-2 align-items-center text-dark">
                                                <img  src="{{ $review->receivedByDeliveryMan?->image_full_url }}"
                                                    class="rounded-circle object-cover" width="48" height="48"
                                                    alt="{{$review->receivedByDeliveryMan?->f_name.' '.$review->receivedByDeliveryMan?->l_name}}">
                                                <div class="meida-body">
                                                    <div
                                                        title="{{$review->receivedByDeliveryMan?->f_name.' '.$review->receivedByDeliveryMan?->l_name}}">{{$review->receivedByDeliveryMan?->f_name.' '.$review->receivedByDeliveryMan?->l_name}}</div>
                                                    <div> {{$review?->receivedByDeliveryMan?->phone}} </div>
                                                </div>
                                            </a>
                                            @else
                                            <div
                                                class="text-muted">{{translate('messages.rider_not_found')}}</div>
                                            @endif

                                        </span>
                                    </td>
                                    <td>
                                        @if ($review->givenByUser)
                                            <a href="{{route('admin.users.customer.ride-share.view',[$review->given_by])}}" target="_blank"
                                               class="text-dark">
                                                {{$review->givenByUser->f_name ?? ""}} {{$review->givenByUser->l_name ?? ""}}
                                            </a>
                                        @else
                                            <div
                                    ...
                                    <td>
                                        <div class="btn--container justify-content-center">
                                            <a class="btn action-btn btn--warning btn-outline-warning view-details" href="#"
                                               ...
                                               data-name="{{$review?->receivedByDeliveryMan?->f_name.' '.$review?->receivedByDeliveryMan?->l_name}}"
                                               data-image="{{ $review?->receivedByDeliveryMan?->image_full_url }}"
                                               data-phone="{{$review?->receivedByDeliveryMan?->phone}}"
