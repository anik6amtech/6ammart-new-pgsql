...
<div class="js-nav-scroller hs-nav-scroller-horizontal mt-2">
    ...
    <ul class="nav nav-tabs mb-3 border-0 nav--tabs nav--pills">
        ...
        <li class="nav-item">
            <a class="nav-link {{request()?->tab == 'ride_list' ? 'active' : ''}}"
                href="{{ route('admin.users.delivery-man.preview', ['id' => $deliveryMan->id, 'tab' => 'ride_list']) }}"
                aria-disabled="true">{{ translate('messages.ride_list') }}</a>
        </li>
        <li class="nav-item">
            <a class="nav-link {{request()?->tab == 'vehicle' ? 'active' : ''}}"
                href="{{ route('admin.users.delivery-man.preview', ['id' => $deliveryMan->id, 'tab' => 'vehicle']) }}"
                aria-disabled="true">{{ translate('messages.vehicle') }}</a>
        </li>
        <li class="nav-item">
