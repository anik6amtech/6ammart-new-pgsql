...
@section('title',translate('Update Rider'))
    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <h1 class="page-header-title text-break">
                ...
                <span>{{translate('messages.update_rider')}}</span>
            </h1>
        </div>
        <!-- End Page Header -->
                            ...
                            data-original-title="{{ translate('messages.Required.')}}"> *
                                ...
                                <div class="col-sm-6">
                                    <div class="text-capitalize">
                                        <label for="" class="mb-2">{{ translate('serve_for') }}</label>
                                        <div class="d-flex flex-wrap form-control gap-3">
                                            @foreach(VEHICLE_CATEGORY_USE_CASE as $type)
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input mx-2 category-use-for-checkbox-{{ $type }}" type="checkbox" value="{{$type}}" name="serve_for[]" {{ $deliveryMan->{'is_' . $type} == 1 ? 'checked' : '' }}>
                                ...
                                <div class="col-sm-6 delivery-required-field">
                                    <div class="form-group mb-0">
                                    ...
                                    <div class="form-group m-0">
                                        ...
                                        <select name="vehicle_id" id="vehicle_select"
                                                ...
                                                @if (isset($deliveryMan->vehicle))
                                                    <option value="{{ $deliveryMan->vehicle->id }}" selected="selected">
                                                        {{ $deliveryMan->vehicle->name }}</option>
                                                @endif
                                        </select>
                                    </div>
                                </div>
...
<script>
    "use strict";
        ...
        $('#vehicle_select').select2({
            ajax: {
                url: '{{ route('admin.users.delivery-man.vehicle.category.all-categories') }}',
                data: function (params) {
                    return {
                        q: params.term, // search term
                        page: params.page,
                        type: $('.form-check-input:checked').map(function () {
                                return $(this).val();
                            }).get().join(',')
                    };
                },
    ...
    $(document).ready(function () {
        $('form.js-validate').on('submit', function (e) {
            ...
            $(this).find('input[required], select[required], textarea[required]').each(function () {
                if (!$(this).val()) {
                    toastr.error('Please fill out the required field: ' + ($(this).attr('placeholder') || $(this).prev('label').text()), {
                        CloseButton: true,
                        ProgressBar: true
                    });
                    $(this).focus();
