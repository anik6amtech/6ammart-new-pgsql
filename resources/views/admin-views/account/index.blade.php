        ...
        <div class="card-body">
            <form action="{{route('admin.transactions.account-transaction.store')}}" method='post' id="add_transaction">
                ...
                <div class="row g-3">
                    <div class="col-lg-4 col-sm-6">
                        ...
                        <label class="form-label" for="type">{{translate('messages.collect_from')}}<span class="input-label-secondary"></span></label>
                            <select name="type" id="type" class="form-control">
                                ...
                                <option value="provider">{{translate('messages.provider')}}</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-lg-4 col-sm-6 user-type-wrapper user-type-store" style="display: none;">
                        <div class="form-group mb-0">
                    ...
                    <div class="col-lg-4 col-sm-6 user-type-wrapper user-type-deliveryman">
                        <div class="form-group mb-0">
                    ...
                    <div class="col-lg-4 col-sm-6 user-type-wrapper user-type-provider" style="display: none;">
                        <div class="form-group mb-0">
                            <label class="form-label" for="provider">{{translate('messages.provider')}}<span class="input-label-secondary"></span></label>
                            <select id="provider" name="provider_id" data-placeholder="{{translate('messages.select_provider')}}" class="form-control" title="Select provider">
                            ...
                            @foreach($account_transaction as $k=>$at)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @elseif($at->provider)
                                        <a href="{{route('admin.service.provider.details',[$at->provider->id,'module_id'=>$at->provider->module_id])}}">{{ $at->provider->first_name }} {{ $at->provider->last_name }}</a>
                                        @else
                                    ...
                                    @if($at->from_type == 'deliveryman')
                                        @php($btnInfo['type'] = translate('DeliveryMan_Info'))
                                        @php($btnInfo['phone'] = $at->deliveryman?->phone)
                                        @php($btnInfo['address'] = $at->deliveryman?->last_location?->location ?? translate('address_not_found'))
                                        @php($btnInfo['latitude'] = $at->deliveryman?->last_location?->latitude ?? 0)
                                        @php($btnInfo['longitude'] = $at->deliveryman?->last_location?->longitude ?? 0)
                                        @php($btnInfo['name'] = $at->deliveryman?->f_name.' '.$at->deliveryman?->l_name)
                                    @elseif($at->from_type == 'provider')
                                        @php($btnInfo['type'] = translate('Provider_Info'))
                                        @php($btnInfo['phone'] = $at->provider?->phone)
                                        @php($btnInfo['address'] = $at->provider?->company_address ?? translate('address_not_found'))
                                        @php($btnInfo['latitude'] = $at->provider?->latitude ?? 0)
                                        @php($btnInfo['longitude'] = $at->provider?->longitude ?? 0)
                                        @php($btnInfo['name'] = $at->provider?->first_name.' '.$at->provider?->last_name)
                                    @else
                                        @php($btnInfo['type'] = translate('Store_Info'))
                                        @php($btnInfo['phone'] = $at->store?->phone)
                                        @php($btnInfo['address'] = $at->store?->address ?? translate('address_not_found'))
                                        @php($btnInfo['latitude'] = $at->store?->latitude ?? 0)
                                        @php($btnInfo['longitude'] = $at->store?->longitude ?? 0)
                                        @php($btnInfo['name'] = $at->store?->name)
                                    @endif
                                    <td>
                                        <div class="btn--container justify-content-center"> <a href="#"
                                            ...
                                            data-type="{{ $btnInfo['type'] }}"
                                            data-phone="{{ $btnInfo['phone'] }}"
                                            data-address="{{ $btnInfo['address'] }}"
                                            data-latitude="{{ $btnInfo['latitude'] }}"
                                            data-longitude="{{ $btnInfo['longitude'] }}"
                                            data-name="{{ $btnInfo['name'] }}"
...
<script src="{{asset('public/assets/admin')}}/js/view-pages/account-index.js?v=1.2"></script>
<script>
    "use strict";
    ...
    $('#provider').select2({
        ajax: {
            url: '{{url('/')}}/admin/service/provider/get-providers',
            data: function (params) {
                return {
                    q: params.term, // search term
                    page: params.page
    ...
    $('#provider').on('change', function() {
        $.get({
            url: '{{url('/')}}/admin/service/provider/get-account-data/'+this.value,
            dataType: 'json',
            success: function (data) {
                $('#account_info').html('({{translate('messages.cash_in_hand')}}: '+data.cash_in_hand+' {{translate('messages.total_earning')}}: '+data.earning_balance+')');
            },
