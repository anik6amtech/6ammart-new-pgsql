                ...
                <div class="col-sm mb-2 mb-sm-0">
                    <div class="d-flex align-items-start">
                        ...
                        <div class="w-0 flex-grow pl-2">
                            <h1 class="page-header-title">{{translate('Subscription Package')}} <small class="ml-2"> {{ $subscriptionackage->module_type == 'rental' ? '('.translate('messages.Rental_Module') .')' : ''}} {{ $subscriptionackage->module_type == 'service' ? '('.translate('messages.Service_Module') .')' : ''}} </small></h1>
                ...
                <div class="card-body">
                    <div class="check--item-wrapper check--item-wrapper-2 mt-0">
                       ...
                       @endif
                        ...
                        @if ($subscriptionackage->module_type == 'service')
                            <div class="check-item">
                                <label class="form-group form-check form--check">
                                    <input type="checkbox" class="form-check-input package-available-feature" {{ $subscriptionackage->scheduled_service == 1 ? 'checked' : '' }}  name="scheduled_service" value="1" >
                                    <span class="form-check-label text-dark">{{ translate('messages.scheduled_service') }}</span>
                                </label>
                            </div>
                            <div class="check-item">
                                <label class="form-group form-check form--check">
                                    <input type="checkbox" class="form-check-input package-available-feature" {{ $subscriptionackage->service_request == 1 ? 'checked' : '' }}  name="service_request" value="1" >
                            ...
                            <div class="check-item">
                                <label class="form-group form-check form--check">
                                    <input type="checkbox" class="form-check-input package-available-feature" {{ $subscriptionackage->reports_and_analytics == 1 ? 'checked' : '' }}  name="reports_and_analytics" value="1" >
                                    <span class="form-check-label text-dark">{{ translate('messages.reports_&_analytics') }}</span>
                                </label>
                            </div>
                            <div class="check-item">
                                <label class="form-group form-check form--check">
                                    <input type="checkbox" class="form-check-input package-available-feature" {{ $subscriptionackage->bidding == 1 ? 'checked' : '' }}  name="bidding" value="1" >
                ...
                <div class="card-header">
                    <div class="w-100 d-flex flex-wrap align-items-start gap-2">
                        ...
                        <div class="w-0 flex-grow">
                            ...
                            <div class="fz-12px">
                                {{-- {{  $subscriptionackage->module_type == 'rental' && addon_published_status('Rental') ? translate('Set_maximum_trip_&_Vehicle_limit_for_this_package') :translate('Set_maximum_order_&_product_limit_for_this_package') }} --}}
                                @if($subscriptionackage->module_type == 'rental')
                                    {{ translate('Set_maximum_trips_&_vehicle_limit_for_this_package') }}
                                @elseif($subscriptionackage->module_type == 'service')
                                    {{ translate('Set_maximum_booking_request_&_sub_category_limit_for_this_package') }}
                                @else
                                    {{ translate('Set_maximum_order_&_product_limit_for_this_package') }}
                                @endif
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="limit-item-card">
                                    <div class="form-group mb-0">
                                        <label class="form-label text-capitalize">
                                            {{-- {{$subscriptionackage->module_type == 'rental' && addon_published_status('Rental') ? translate('Maximum_Trip_Limit'):translate('Maximum_Order Limit') }} --}}
                                            @if($subscriptionackage->module_type == 'rental')
                                                {{ translate('Maximum_Trip_Limit') }}
                                            @elseif($subscriptionackage->module_type == 'service')
                                                {{ translate('Maximum_Booking_Request_Limit') }}
                                            @else
                                                {{ translate('Maximum_Order_Limit') }}
                                            @endif
                                        </label>
                        ...
                        <div class="__bg-F8F9FC-card p-0">
                            <div class="card-body">
                                <div class="limit-item-card">
                                    <div class="form-group mb-0">
                                        <label class="form-label text-capitalize">
                                            {{-- {{$subscriptionackage->module_type == 'rental' && addon_published_status('Rental') ?  translate('Maximum_Vehicle_Limit'):translate('Maximum_Item_Limit') }} --}}
                                            @if($subscriptionackage->module_type == 'rental')
                                                {{ translate('Maximum_Vehicle_Limit') }}
                                            @elseif($subscriptionackage->module_type == 'service')
                                                {{ translate('Maximum_Service_Sub_Category_Limit') }}
                                            @else
                                                {{ translate('Maximum_Product_Limit') }}
                                            @endif
                                        </label>
                                        <div class="d-flex flex-wrap items-center gap-2">
                                            <div class="resturant-type-group p-0">
