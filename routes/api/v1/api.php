...
Route::group(['namespace' => 'Api\V1', 'middleware'=>'localization'], function () {
    ...
    Route::get('deliveryman-faq', 'ConfigController@deliveryman_faq');
    ...
    Route::group(['prefix' => 'delivery-man'], function () {
        ...
        Route::group(['middleware'=>['dm.api']], function () {
            ...
            Route::get('reviews', 'DeliverymanController@get_reviews');
            ...
            Route::group(['prefix' => 'withdraw-request'], function () {
                Route::get('list', 'DeliverymanController@withdraw_list');
                Route::post('create', 'DeliverymanController@request_withdraw');
            });
...
WebSocketsRouter::webSocket('/delivery-man/live-location', DMLocationSocketHandler::class);// routes/api.php

Route::post('/broadcasting/deliveryman-auth', function () {
    $deliveryman = DeliveryMan::where('auth_token', request()->bearerToken())->first();
    if (! $deliveryman) {
        info('deliveryman auth failed. token: '. request()->bearerToken());
        return response()->json(['error' => 'Unauthorized'], 403);
    }
    info('deliveryman auth success. deliveryman id: '. $deliveryman->id, request()->all());
    return [
        'auth' => env('REVERB_APP_KEY').':'.hash_hmac('sha256', request()->socket_id.':'.request()->channel_name, env('REVERB_APP_SECRET'))
    ];
});
Route::post('/broadcasting/user-auth', function () {
    if (! auth('api')->check()) {
        return response()->json(['error' => 'Unauthor`ized'], 403);
    }
    return [
        'auth' => env('REVERB_APP_KEY').':'.hash_hmac('sha256', request()->socket_id.':'.request()->channel_name, env('REVERB_APP_SECRET'))
    ];
