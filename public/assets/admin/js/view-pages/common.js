...
document.querySelectorAll('[name="searchq"]').forEach(function (element) {
    element.addEventListener("input", function (event) {
        if (this.value === "" && window.location.searchq !== "") {
            this.form.submit();
            /* let baseUrl = window.location.origin + window.location.pathname;
            window.location.href = baseUrl; */
        }
...
$(window).on('load', function() {
    $('.alert-danger__custom').show();

    $(document).on('click', '.alert-close-btn', function() {
        $(this).closest('.alert-danger__custom').remove();
    });
});