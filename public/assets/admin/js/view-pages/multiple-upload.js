...
$(document).ready(function () {
    ...
    // Guard: Only initialize image upload logic if the required elements exist on the page
    if (imageContainer && uploadWrapper && inputElement) {
...
$(document).ready(function () {
    ...
    // Render previews for any existing files (e.g., on edit page)
    pdfContainer.find('.pdf-single[data-file-url]').each(function(){
        const $el = $(this);
        const url = ($el.data('file-url') || '').toString().toLowerCase();
        let type = '';
        if (url.endsWith('.pdf')) {
